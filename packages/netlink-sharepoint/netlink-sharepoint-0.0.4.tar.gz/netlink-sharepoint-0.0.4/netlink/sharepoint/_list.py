...
class List(collections.abc.Mapping, abc.ABC):
    """
    Abstract class for Sharepoint list implementation
    ...
    _data contains records (class Item) with the key being the sharepoint ID. For added items, a negative id is used

    Each actual list should be a singleton, at least for the respective Sharepoint site.

    At this point optimistic (as in none) locking is used.

    The item itself keeps information if data has been changed.


    """
    _instances = {}  # this is used to ensure singleton per site (at least per Site-object)
    _title = ''
    ...
    def __new__(cls, *args, **kwargs):
        with cls._lock:
            if cls not in cls._instances:
                cls._instances[cls] = super(List, cls).__new__(cls)
    ...
    def __init__(self, sharepoint_site, lazy: bool = True):
    ...
    def load(self):
        for i in self._sharepoint_list.items.get().execute_query():
