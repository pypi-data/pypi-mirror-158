...
def check_file_exist_and_readable(file_path=None):
    if not os.path.isfile(file_path):
        raise FileNotFoundError('FILE NOT FOUND ERROR: {} is not a valid file path'.format(file_path))
    elif not os.access(file_path, os.R_OK):
        raise FileNotFoundError('FILE CORRUPTED ERROR: {} is not readable'.format(file_path))
