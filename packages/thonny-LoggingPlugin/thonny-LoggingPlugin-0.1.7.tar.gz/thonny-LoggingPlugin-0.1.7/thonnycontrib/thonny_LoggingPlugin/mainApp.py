...
class EventLogger:
    ...
    def __init__(self):
        ...
        self._inDict = {
            ...
            "l1Tests" : {'sequence','filename','tested_line','expected_result','obtained_result','exception'}
        }
    ...
    def _input_processing(self,data,event):
        ...
        if 'filename' in data :
            data['filename'] = re.search("\/[^\/]*$",data['filename']).group()
        ...
        if formatted_log != {}:
            ...
            try :
                if self.sending_logs :
                    self.sending_client.send_xapi_statement(formatted_log)
