...
def test_generating_default_config_file(script_runner):
    ...
    assert (
        "Cannot connect to host localhost:80 ssl:default [Connect call failed ('127.0.0.1', 80)]"
        in ret.stdout
    )
...
def test_version(script_runner):
    ...
    assert "xsget 0.1.0" in ret.stdout
...
def test_extract_urls():
    ...
    for css_path in css_paths:
        config = argparse.Namespace(
            base_url="http://a.com/123", link_css_path=css_path
        )
