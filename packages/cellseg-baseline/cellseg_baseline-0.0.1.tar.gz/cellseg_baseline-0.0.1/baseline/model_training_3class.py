...
def model_training_3class(data_path='./data/Train_Pre_3class/',work_dir='./work_dir',model_name = 'unet', seed=2022,resume=False,num_workers=4,num_class=3,input_size=256,batch_size=8,max_epochs=2000,val_interval=2,epoch_tolerance=100,initial_lr=6e-4):

    #%% set training/validation split
    np.random.seed(seed)
    model_path = join(work_dir, model_name+'_3class')
    os.makedirs(model_path, exist_ok=True)
    run_id = datetime.now().strftime("%Y%m%d-%H%M")
    shutil.copyfile(__file__, join(model_path, run_id + '_' + os.path.basename(__file__)))
    img_path = join(data_path, 'images')
    gt_path = join(data_path, 'labels')
    ...
    for epoch in range(1, max_epochs):
        ...
        for step, batch_data in enumerate(train_loader, 1):
            ...
            labels_onehot = monai.networks.one_hot(labels, num_class) # (b,cls,256,256)
        ...
        if epoch>20 and epoch % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                for val_data in val_loader:
                    ...
                    val_labels_onehot = monai.networks.one_hot(val_labels, num_class)
                    roi_size = (256, 256)
                    sw_batch_size = 4
            ...
            if (epoch - best_metric_epoch) > epoch_tolerance:
                print(f"validation metric does not improve for {epoch_tolerance} epochs! current epoch= {epoch}, best_metric_epoch={best_metric_epoch}")
                break
