    ...
    if poly.geometryType() == "Polygon":
        ...
        for small in poly:
            ...
            if is_point(img_shape):
                ...
                ImageDraw.Draw(img).polygon(img_shape, fill=1, outline=1)
                for holes in shp_geom.asShape(small).holes:
                    img_holes = [
                        tuple(np.round(lon_lat_to_img(point, g)).tolist())
                        for point in holes
                    ]
...
class MaskProcessor:
    ...
    def change_geometry(self):
        for i, area in enumerate(self.data["geos"]["features"]):
            if shp_geom.asShape(area["geometry"]).geometryType() in [
                "Polygon",
                "MultiPolygon",
                "LineString",
                "MultiLineString",
            ]:
                x = shp_geom.asShape(area["geometry"]).buffer(1e-5)  # .buffer(-1e-5)
    ...
    def create_masks(self):
        ...
        for area in self.data["geos"]["features"]:
            ...
            poly = shp_geom.asShape(area["geometry"])
    ...
    def get_compass_area(self, grid, poly, area_id):
        ...
        for area in geo_B["features"]:
            compass_poly = shp_geom.asShape(area["geometry"])
            compass_id = area["id"]
