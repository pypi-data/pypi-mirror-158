...
def err_synth(seq, durs, get_features, get_error,
              ...
              trajectcomps=None, objintervals=None,
    ...
    if do_synthesis:
        LOGGER.debug("Synthesising...")
        trajects_passed = True
        if trajectcomps is not None:
            full_targets = mappings.add_slope_and_duration(seq, durations=durs)
        ...
        if not trajects_passed:
            ...
            if objintervals is None:
                ...
                synth_feats_interval = []
                for i in range(NUM_OBJINTERVAL_SAMPLES):
                    s = seq.copy()
                    for lab_param, prange in objintervals.items():
                        lab, param = lab_param.split("-")
                        s.loc[pd.IndexSlice[:, lab], param] = rng.uniform(prange[0], prange[1])
                    full_targets = mappings.add_slope_and_duration(s, durations=durs)
                    vtl_curves = mappings.target_approximation(full_targets)
                    synth_sound = mappings.synthesise_vtl_curves(vtl_curves)
                    synth_feats = get_features(synth_sound,
                                               articvec=articvec,
                                               warp_alpha=vtln_warpfactor)  # warp if needed
                    synth_feats_interval.append(synth_feats)
                synth_feats = np.array(synth_feats_interval).mean(0)
