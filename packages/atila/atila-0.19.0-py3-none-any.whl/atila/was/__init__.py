...
from skitai.tasks.coroutine.tasks import Mask, Tasks
from skitai.tasks.coroutine.tasks.pth import sp_task
from skitai.tasks.coroutine.tasks.pth.task import Task
...
class WAS (wsgiappservice.WAS):
    ...
    def _set_task_class (self, kargs):
        if "meta" not in kargs:
            kargs ["meta"] = {}
        if "__task_class" not in kargs ["meta"]:
            kargs ["meta"]["__task_class"] = Task
            kargs ["meta"]["__after_request_async"] = after_request_async
        return kargs

    def _insert_cloned_was (self, args, kargs):
        _was = utils.get_cloned_was (self.ID)
    ...
    def ThreadPass (self, target, *args, **kargs):
        ...
        kargs = self._set_task_class (kargs)
