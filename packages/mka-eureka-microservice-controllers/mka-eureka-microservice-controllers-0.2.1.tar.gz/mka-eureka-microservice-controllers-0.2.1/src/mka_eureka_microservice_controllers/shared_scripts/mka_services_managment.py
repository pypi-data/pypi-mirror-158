...
class ServiceController():
    ...
    def ReadFile(file_path_info):
        serviceCallStatus,serviceCallResults= ServiceController.CallEurekaService(Services.FILE_SERVICE_NAME,Services.READ_DF_FILE,data=file_path_info,response_key=Services.FILE_KEY)
        if not serviceCallStatus:
                return False,{'message':serviceCallResults["message"],"code":422}
        json_dataframe=serviceCallResults[0]["file_content"]
        return True,json_dataframe
