...
def mark_old_patients_pending(db):
    ...
    query = f"""
    update
        "prod2-generico"."{patients_metadata_table}"
    set
        "etl-status" = 'pending'
    where
        "etl-status" = 'updating'
...
def update_diagnostic_customer(db):
    query = f"""
        ...
        from
            ...
            from
                "prod2-generico"."{patients_metadata_table}" pm
            inner join "prod2-generico"."redemption" r on
                pm.id = r."patient-id"
            where
...
def process_batch(changed_patients, db, s3, pg_db):
    ...
    patient_bills_2 = patient_bills.sort_values(by=["id", "created-at"])  # soring on patient id and bill-created-at

    patient_bills_2['bill-date'] = patient_bills_2['bill-date'].apply(lambda x: x.strftime("%Y-%m-%d"))
    ...
    patient_first_bill.rename(columns={"bill-id": "first-bill-id", "created-at": "first-bill-date"}, inplace=True)
    ...
    if str(value_max_date) == str(cur_nmz_date):
        if str(cur_nmz_date) != str(patient_val_max_dt):
            ...
            update_q1 = '''
                ...
                from 
                    "prod2-generico"."patients-metadata-2" pm2
                        inner join 
                    "prod2-generico"."customer-value-segment"  cbs
                        on 
        ...
        if str(cur_nmz_date) != str(patient_beh_max_date):
            ...
            update_q1 = '''
                ...
                from 
                    "prod2-generico"."patients-metadata-2" pm2 
                        inner join 
                    "prod2-generico"."customer-behaviour-segment" cbs 
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--batch_size', default=100, type=int, required=False, help="batch size")
