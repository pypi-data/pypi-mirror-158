...
parser.add_argument('-ah', '--api_hit', default="0", type=str, required=False)
...
if env== 'dev':
    mysql_read = MySQL(read_only=False)
else:
    mysql_read = MySQL()
...
if start1=="NULL" and start2=="NULL":
    ...
    if datetime.now(tz=gettz('Asia/Kolkata')).strftime('%H:%M:%S') < '09:30:00':
        ...
        if env=='dev':
            ...
            def ceil_dt(dt, delta):
                return dt + (datetime(1, 1, 1, 0, 0, 0 ,tzinfo=gettz('Asia/Calcutta'))  - dt) % delta + timedelta(minutes=23,seconds=20)
...
if int(write_to_mysql) == 1:
    ...
    try:
        ...
        if int(api_hit)==1:
            ...
            inserted_items = pd.read_sql_query(mysql_inserted_items_query, mysql_read.connection)
