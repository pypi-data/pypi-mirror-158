...
parser.add_argument('-et', '--email_to', default="akshay.bhutada@zeno.health,rajesh.shinde@zeno.health", type=str, required=False)
...
sale_purchase_investor=sale_purchase_investor.drop(["type-1","sub-type-1","sub-type-3","drug-type","drug-category","goodaid-flag","distributor-id","distributor-name","as-ms-pr","net-quantity"],axis=1)
...
email.send_email_file(subject=f"sale purchase report for date for date {end_date}",
                      mail_body=f"PFA sale purchase date for date   {end_date} ",
