...
def main(debug_mode, as_inactive_exclude_types, db, rs_db_write, read_schema,
         write_schema, logger):
    s3 = S3()
    ...
    try:
        ...
        out_of_shelf_group = out_of_shelf_group[
            ['store_id', 'store_name', 'drug_id', 'drug_name', 'type', 'bucket',
             'drug_grade', 'closing_date', 'max_set', 'customer_type',
        ...
        oos_sys = round(
            (df_max_set["oos-count"].sum() / df_max_set["drug-count"].sum()), 4) * 100
        oos_ethical = round(
            (df_ethical["oos-count"].sum() / df_ethical["drug-count"].sum()), 4) * 100
        oos_generic = round(
            (df_generic["oos-count"].sum() / df_generic["drug-count"].sum()), 4) * 100
        oos_others = round(
            (df_others["oos-count"].sum() / df_others["drug-count"].sum()), 4) * 100
