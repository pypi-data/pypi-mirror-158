...
parser.add_argument('-ed', '--end_date', default="2022-06-27", type=str, required=False, help="End date in IST")
...
file_name = f"{store_id}-{start_date}-{end_date}.csv"
uri = s3.save_df_to_s3(df=recon_df[column_order],
                       file_name=f"inventory/ledger/adhoc/{file_name}", index=False)

if local_file_path:
    local_file_path = f"{local_file_path}/{file_name}"
    recon_df.to_csv(path_or_buf=local_file_path)
