...
def ipTSP(
    ...
    if (fml == 'DFJ_Lazy'):
        ...
        res = ipTSPShortestPath()
    elif (fml == 'MultiCommodityFlow'):
        res = ipTSPMultiCommodityFlow()
    elif (fml == 'QAP'):
        res = ipTSPQAP()
    else:
...
def consTSP(
    ...
    def consTSPkNearestNeighbor(k = 1):
        seq = [nodeIDs[0]]
        remain = [nodeIDs[i] for i in range(1, len(nodeIDs))]
        ofv = 0
    ...
    if (algo == 'NearestNeighbor'):
        if (algoArgs == None):
            res = consTSPNearestNeighbor()
        else:
            res = consTSPkNearestNeighbor(algoArgs['k'])
    elif (algo == 'FarthestNeighbor'):
        res = consTSPFarthestNeighbor()
    elif (algo == 'Sweep'):
        res = consTSPSweep()
    elif (algo == 'Random'):
        res = consTSPRandomSeq()
    else:
        pass
    if (res != None):
        return res
    ...
    res = None
    if (algo == 'DepthFirst'):
        res = consTSPDepthFirst()
    elif (algo == 'Christofides'):
        res = consTSPChristofides()
