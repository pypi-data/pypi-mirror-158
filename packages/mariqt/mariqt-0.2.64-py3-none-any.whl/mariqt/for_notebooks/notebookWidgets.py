...
class DoubleMinMaxAndOffsetConverter(ConverterBase):
    ...
    def __init__(self):
        ...
        self.minMaxConverter = DoubleMinMaxConverter()
        self.offsetConverter = DoubleConstOffsetConverter()
        self._widget = widgets.VBox([self.minMaxConverter._widget,self.offsetConverter._widget])

    def convert(self, value: str) -> str:
        value = self.minMaxConverter.convert(value)
        if not value is None:
            value =  self.offsetConverter.convert(value)
        return value

    def setParams(self,params):
        self.minMaxConverter.setParams(params[0])
        self.offsetConverter.setParams(params[1])

    def getParams(self):
        return [self.minMaxConverter.getParams(),self.offsetConverter.getParams()]
