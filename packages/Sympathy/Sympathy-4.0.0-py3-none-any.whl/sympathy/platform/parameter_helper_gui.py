...
from sympathy.platform.widget_library import (
    set_plain_text_to_rich_tooltip as _set_description,
    plain_text_to_rich as _format_description)
...
class ParameterBaseValueWidget(ParameterWidget):
    def __init__(self, parameter_value, editor=None, node=None, parent=None):
        ...
        if self._parameter_value.label:
            ...
            _set_description(
                self._label_widget, self._parameter_value.description)
        if self._parameter_value.description:
            formatted = _format_description(self._parameter_value.description)
            self._editor.setToolTip(formatted)
...
class ParameterValueWidget(ParameterBaseValueWidget):
    def __init__(self, parameter_value, editor=None, node=None, parent=None):
        ...
        self._editor.valueChanged.connect(self._text_changed)
...
class ParameterBooleanWidget(ParameterWidget):
    ...
    def __init__(self, parameter_value, editor=None, node=None, parent=None):
        ...
        if self._parameter_value.description:
            _set_description(self._editor, self._parameter_value.description)
...
class ParameterEditorTextLineWidget(ParameterEditorWidget):
    ...
    def __init__(self, parameter_value, editor_dict, parent=None):
        ...
        if self._parameter_list.type == 'float':
            ...
            line_edit.valueChanged[object].connect(
                self._value_changed)
        else:
            line_edit = sywidgets.ValidatedTextLineEdit()
            line_edit.valueChanged.connect(self._value_changed)
...
class ParameterEditorIntegerWidget(ParameterEditorSpinBoxWidget):
    ...
    def __init__(self, parameter_value, editor_dict, spin_buttons=False,
        ...
        self._value_spinbox.valueChanged[object].connect(
            self._value_changed)
...
class ParameterEditorFloatWidget(ParameterEditorSpinBoxWidget):
    ...
    def __init__(self, parameter_value, editor_dict, spin_buttons=False,
                 parent=None):
        customization = {
            'max': 100.0 if spin_buttons else None,
            'min': 0.0 if spin_buttons else None,
            'step': 1.0,
            'decimals': None,
        }
...
class ParameterEditorFileDialogWidget(ParameterEditorTextLineWidget):
    ...
    def __init__(self, parameter_string, editor_dict, parent=None):
        ...
        # Disabled completer to avoid issues with QFileSystemModel in Qt 6.2.
        # Window is rendered black for a duration which depends on
        # the number of files in the root (setRootPath(root)). Seen on Ubuntu.
...
class ParameterEditorComboboxWidget(ParameterEditorWidget):
    ...
    def __init__(self, parameter_list, editor_dict, parent=None):
        ...
        model = self._dummy_combobox.model()
        search_model = sywidgets.OrderedComboboxSearchFilterModel(model)
        self._list_combobox.setModel(search_model)
    ...
    def _init_gui_from_parameters(self):
        options = list(self._editor_dict.get('options', []) or [])
        self._init_options(options)

    def _init_options(self, options):
        display = self._editor_dict.get('display')
        self.set_options(options, display)
    ...
    def set_options(self, options, display=None, keep_current_display=False):
        index = self._list_combobox.currentIndex()
        option = None
        text = None
        if index >= 0:
            text = self._list_combobox.currentText() or ''
            option = self._get_option(text)
        with block_signals(self._list_combobox):
            self._list_combobox.clear()
        self._options = options
        display_map = {}
        if display is not None and len(display) == len(options):
            display = [d.get('text', '') if isinstance(d, dict) else d
                       for d in display]
            display_map = dict(zip(options, display))
        # Duplicate keys, values are discarded. This allows lookup in both
        # directions and means that duplicates should be avoided.
        if keep_current_display and index >= 0:
            display_map[option] = text
        self._option_map = _flip_dict(display_map)
        self._display_map = _flip_dict(self._option_map)
        include_empty = self._editor_dict.get('include_empty', False)
        if include_empty and '' not in options:
            options.insert(0, '')

        self._add_items(options)
    ...
    def _set_option(self, option):
        ...
        if option not in self._options:
            ...
            text = self._display_map.get(option, text)
            self._insert_item(index, text)
            self._setup_new_item(index, option)
        else:
            try:
    ...
    def _parameter_value_changed(self, parameter_value):
        self._parameter_list.value = parameter_value
        self.valueChanged.emit(self._parameter_list.value)

    def _value_changed(self, value):
        # Avoid unnecessary set and signaling after index change.
        # valueChanged may already be called.
        parameter_value = self._get_option(value)
        if parameter_value != self._parameter_list.value:
            self._parameter_value_changed(parameter_value)
...
class ParameterListEditorComboboxWidget(ParameterEditorComboboxWidget):
    ...
    def _init_gui_from_parameters(self):
        ...
        self._init_options(list(self._available_items))
...
class ParameterListModel(QtCore.QAbstractListModel):
    ...
    def _init_values(self, items, checked):
        checked = dict.fromkeys(sorted(checked), True)
        unchecked = dict.fromkeys(sorted(items), False)
        self._checked_labels = {}
        ...
        self._original_labels = set()
        self._original_labels.update(items)

    def _init_parameter(self):
        self._init_values(self._parameter.list, self._parameter.value_names)
    ...
    def data(self, index, role):
        ...
        if role in (QtCore.Qt.EditRole, QtCore.Qt.DisplayRole):
            ...
            if self._checked_labels[label]:
                res = QtCore.Qt.CheckState.Checked
            else:
                res = QtCore.Qt.CheckState.Unchecked
            res = int(res)
    ...
    def _set_unchecked(self, items):
        # Improved implementation might keep selection, etc.
        checked = self._parameter.value_names
        self.beginResetModel()
        self._init_values(items, checked)
        self.endResetModel()
...
class ParameterEditorListWidget(ParameterEditorWidget):
    ...
    def __init__(self, parameter_list, editor_dict, parent=None):
        ...
        if use_filter:
            ...
            vlayout.addWidget(self._mode_widget)
    ...
    def _init_gui_from_parameters(self):
        if self._use_mode:
            self._mode_widget.set_selected(
                self._parameter_list.multiselect_mode)
        self._init_mode()

    def _add_menu_action(self, text, handler, shortcut=None):
        action = QtGui.QAction(text, parent=self._list_widget)
    ...
    def _mode_changed(self, mode):
        self._parameter_list.multiselect_mode = mode
        self._init_mode()
    ...
    def _set_items(self, items):
        # Tentative api, used in Fx.
        # Partially a workaround to setting .list triggering value_changed.
        self._list_model._set_unchecked(items)
...
class ParameterEditorJsonWidget(ParameterEditorWidget):
    ...
    def __init__(self, parameter_list, editor_dict, customization=None,
        ...
        self._json_lineedit.valueChanged.connect(self._value_changed)
        self._json_lineedit.setText(json.dumps(parameter_list.value))

        if parameter_list.description:
            _set_description(self._json_lineedit, parameter_list.description)
...
class ParameterEditorJsonTableWidget(ParameterEditorWidget):
    ...
    def __init__(self, parameter_list, editor_dict, customization=None,
        ...
        if parameter_list.description:
            _set_description(self._table_editor, parameter_list.description)
        vlayout.addWidget(self._table_editor)
        self.setLayout(vlayout)

    def _validate(self, value):
        return (value and isinstance(value, list) and
                len(value[0]) == len(self._headers) and
                len(set(len(rows) for rows in value)) == 1)
...
class ParameterEditorConnectionWidget(ParameterEditorWidget):
    ...
    def __init__(self, parameter_list, editor_dict, customization=None,
        ...
        self._credentials_widget = credentials.StackedWidget()
        ...
        widgets = [cls(parent=self) for cls in credentials_cls]

        modes = editor_dict.get('modes')
        if modes:
            modes = [parameter_types.CredentialsMode[mode]
                     for mode in modes] + [None]
            widgets = [widget for widget in widgets if widget.mode() in modes]
...
class ParameterEditorAzureWidget(ParameterEditorWidget):
    ...
    def _handle_edit_button_clicked(self, checked):
        ...
        port = credentials.azure_login_server().port()
        self._node.configure_credentials_async(
            connection.identifier(), connection.to_dict(), {'port': port},
            partial(self._handle_async_credentials_received, connection))
    ...
    def _handle_async_credentials_received(self, connection, secrets):
        if secrets is None:
            ...
            # Conditional on exec_() result here?
...
class ParameterListWidget(ParameterWidget):
    def __init__(self, parameter_list, editor=None, node=None, parent=None):
        ...
        if self._parameter_list.label:
            ...
            layout.addItem(QtWidgets.QSpacerItem(10, 1))
            _set_description(
                self._label_widget, self._parameter_list.description)
        if self._parameter_list.description:
            formatted = _format_description(self._parameter_list.description)
            self._editor.setToolTip(formatted)
            self.setToolTip(formatted)
        layout.addWidget(self._editor)
        self.setLayout(layout)
