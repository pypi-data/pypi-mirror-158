...
def nodups(lst):
    return reversed(
        dict.fromkeys(reversed(lst)).keys())
    ...
    def _got_data_request(self, ident, msg):
        ...
        if not error:
            for prev_node in locked_prev:
                if prev_node.is_done_locked():
                    # TODO(erik): these sort of things should be handled in
                    # an appropriate class for configuring of locked subflows.
                    prev_node_flow = prev_node.flow
                    if prev_node_flow:
                        with prev_node_flow.propagate_ctx(prev_node):
            ...
            if not (inodes and nodes):
                self.message_output.emit(
                    -1, message.DataReadyMessage(node.full_uuid))

    def _got_status_request(self, ident, msg):
        uuid = msg.data
        node = self._appcore.get_flode(uuid)
        error, nodes, inodes = self._dependent_node_status(node)[:3]
        if error:
            self.message_output.emit(
                ident, message.DataBlockedMessage(node.full_uuid))
        elif not (inodes and nodes):
            self.message_output.emit(
                -1, message.DataReadyMessage(node.full_uuid))
