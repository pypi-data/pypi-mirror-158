...
def _migrate_node_name(node, new_name: str) -> str:
    library_name = node.library_node.name
    if node.name == library_name:
        return new_name
    if node.name.startswith(library_name):
        extra = node.name[len(library_name):]
        return new_name + extra
    if node.name.endswith(library_name):
        extra = node.name[:-len(library_name)]
