...
def config_aggregation_dialog(
        configure, socket_bundle, flow_info_with_instances,
        type_aliases):
    """
    ...
    """
    flow_info = flow_info_with_instances
    cdm = CentralDataModel(
        flow_info, type_aliases, configure)

    internal_deps = flow_info.pop('internal_deps', {})
    input_nodes = flow_info.pop('input_nodes', [])
    locked_nodes = flow_info.pop('locked_nodes', [])

    wfdthread = WaitForData(
        socket_bundle, internal_deps, input_nodes, locked_nodes, cdm)

    app = QtWidgets.QApplication.instance()
    if (configure in _configure_modes and
            cdm._builder == _wizard_builder):
        dialog = WizardParameterView(cdm, wfdthread)
    elif configure == _settings:
        dialog = SettingsDialog(cdm, wfdthread)
    else:
        dialog = TabbedParameterView(cdm, wfdthread)

    title = 'Parameter View' if configure else 'Aggregation Settings'
    ...
    flow_info['json_aggregation_settings'] = json.dumps(
        cdm.to_dict())
    flow_info['configure'] = configure in _configure_modes
    return result
...
class WidgetManager(object):
    ...
    def _init_uuid_to_widget(self, info_dict, top_level=True,
        ...
        top_level : bool
            True if this is the first recursion level, False otherwise.
    ...
    def _build_node_context(self, node_info_dict, node_dict):
        """Build context for node."""
        try:
            objects = {}
            if self._cdm._lock_wizard:
                objects = dict(self._cdm.objects)

            library_node_instance = node_info_dict['library_node_instance']
            node_context = library_node_instance.build_context(
                node_dict, self._type_aliases, exclude_output=True,
                objects=objects, port_dummies=True)
            return node_context
    ...
    def node_context(self, uuid):
        ...
        widget_dict = self._uuid_to_widget.get(uuid, None)
        return widget_dict['node_context']()
    ...
    def widget(self, uuid):
        ...
        def node_widget(item_dict):
            ...
            node_context = item_dict['node_context']()
...
class CentralDataModel(QtCore.QObject):
    ...
    def execute_node(self, uuid):
        node_info_dict = self.uuid_to_item[uuid]
        node_dict = json.loads(node_info_dict['json_node_dict'])
        node = node_info_dict['library_node_instance']
        context = node.build_execute_context(
            node.expanded_parameters(node_dict),
            self._type_aliases,
            objects=self.objects,
            entry_context=False)
        node._execute_with_context(context)

        for k, v in context._objects.items():
            if k not in self.objects or v in context._output:
                self.objects[k] = v
    ...
    def builder(self):
        return {_tab_builder: TabBuilder, _wizard_builder: WizardBuilder}[
            self._builder]
                ...
                if (uuid_selected(uuid, flow_info, selected_uuids_list) and
                    ...
                    if widget is not None:
                        ...
                        node_dict['parameters']['data'] = (
                            node_context.parameters.to_dict())
...
class WizardParameterView(ConfirmChangesDialogMixin, AggregatedDialogMixin,
    ...
    def page_factory(self, n, item, ntot):
        page = WizardReadyPage(n, ntot, item, self._wfdthread,
...
class SettingsView(QtWidgets.QWidget):
    ...
    def __init__(self, central_data_model, parent=None):
        ...
        self.selected_model.rowsInserted[QtCore.QModelIndex, int, int].connect(
            self.available_tree.widget_update)
        self.selected_model.layoutChanged.connect(
            self.available_tree.widget_update)
...
class AvailableTreeView(QtWidgets.QTreeView):
    ...
    def widget_update(self):
        # QAbstractItemView overrides QWidget.update.
        super(QtWidgets.QAbstractScrollArea, self).update()
...
class WaitForData(QtCore.QObject):
    ...
    def handle_input(self, msgs):
        for msg in msgs:
            if msg.type == message.DataReadyMessage:
                self._uuids_ready.add(msg.data)
    ...
    def request_execute(self, node):
        self._uuids_done.add(node)
        self._cdm.execute_node(node)

    def execute_order(self, node):
        """
        Return tuple of input nodes and internal nodes. Data need to be
        requested for input nodes, to execute any prior nodes. Internal nodes
        and should be executed in listed order.
        """
        visited = set()
        visited.update(self._uuids_done)
        internal = []
        inputs = []
        ...
        def visit(node):
            if node not in visited:
                visited.add(node)
                # In wizard configuration selecting lambda, node
                # may not be among internal_deps: get avoids KeyError.
                for dep_node in self._internal_deps.get(node, []):
                    if dep_node in self._internal_deps:
                        visit(dep_node)
                    else:
                        if dep_node not in inputs:
                            # Effective input node for locked subflow.
                            inputs.append(node)
        ...
        visit(node)
        return nodup_list(inputs), nodup_list(internal)


def nodup_list(items):
    seen = set()
    res = []
    for item in items:
        if item not in seen:
...
class WizardReadyPage(QtWidgets.QWizardPage):
    def __init__(self, n, ntot, item, wfdthread, cdm, parent=None):
        super().__init__(parent)
        self._item = item
        self._cdm = cdm
        self._wfdthread = wfdthread
    ...
    def validatePage(self):
        ...
        if is_complete:
            ...
            self._wfdthread.request_data(message.AggConfigUpdateMessage(
                clean_flow_info(self._cdm.flow_info)))
        return is_complete

    def isComplete(self):
        if self._dirty:
            return (self._widget and self._widget.valid) or False
        return True
...
class NullParametersAggregationMixin(NullParametersWidgetMixin):
    def cleanup(self):
        pass
...
class LabelParameterWidget(NullParametersAggregationMixin, QtWidgets.QLabel):
    pass
...
class FailedParameterWidget(NullParametersAggregationMixin, QtWidgets.QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
...
class WizardProgressPage(QtWidgets.QWizardPage):
    def __init__(self, n, ntot, item, wfdthread, parent=None):
        super().__init__(parent)
        self._wfdthread = wfdthread
        ...
        self._waiting = []
        self._internal = []
        self._wfdthread.data_ready.connect(self._handle_data_ready)
        self._wfdthread.data_blocked.connect(self._handle_data_blocked)

    def initializePage(self):
        self.completeChanged.connect(self.commit_page)
        if self._item.get('locked'):
            waiting, internal = self._wfdthread.execute_order(self._uuid)
            self._waiting.extend(waiting)
            self._internal = internal
            if self._uuid in internal:
                internal.remove(self._uuid)
        else:
            self._waiting.append(self._uuid)
        for uuid in self._waiting:
            self._wfdthread.request_data(message.DataRequestMessage(uuid))
    ...
    def _handle_data_blocked(self, uuid):
        self.cleanup()
        self._label.setText(
        ...
        if not self._waiting:
            ...
            for uuid in self._waiting:
                self._wfdthread.request_data(
                    message.StatusDataRequestMessage(uuid))

    def _handle_data_ready(self, uuid):
        if uuid in self._waiting:
            self._waiting.remove(uuid)
            self._execute_complete()
        # self.completeChanged.emit()

    def _execute_complete(self):
        if not self._waiting and not self._is_complete:
            self._is_complete = True
            for uuid in self._internal:
                self._wfdthread.request_execute(
                    uuid)
            self.completeChanged.emit()
