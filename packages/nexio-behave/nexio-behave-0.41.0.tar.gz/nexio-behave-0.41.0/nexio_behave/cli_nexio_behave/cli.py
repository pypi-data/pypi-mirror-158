...
from nexio_behave.cli_nexio_behave.config import NexioBehaveConfig
...
from nexio_behave.cli_nexio_behave.runner import NexioBehaveRunner  # isort: skip
...
@click.option(
    ...
    default="integration",
    help="The test suite to initialize.",
)
@cli.command()
def init(test: str) -> None:
    """Initializes the Nexio Behave framework"""
    logger.info("Initializing the Nexio Behaves' default directory. Writing new files.")
    ...
    try:
        ...
        os.mkdir(f"{TEST_DIR}/tests")
        open(f"{TEST_DIR}/tests/__init__.py", "w")
...
def integration(
    ...
    """Run the Nexio Behave integration tests"""
    logger.info("Starting Behave Integration tests.")
    behave_config = NexioBehaveConfig(
        "behave_integration",
        environment,
        common_app,
        stack_name,
    ...
    NexioBehaveRunner.run_nexio_behave(INTEGRATION_TESTS, behave_config.to_args())
