...
FILES_INS = ["ig_insights_wyj.csv", "ig_insights_wyj_sd.csv"]
FILES_POST = ["instagram_lib_posts_WYJ.csv"]

SOCIAL_N = "ig"

BRAND = random.sample(range(len(FILES_POST)), 1)

COLUMNS_POST = ["owner_username", "shortcode", "date_utc", "url"]
...
DF_INS_CAR = pd.read_csv(FOLDER + DFI_C)
...
class Test(unittest.TestCase):
    ...
    def test_data_normal(self):
        ...
        df_reach_c = ReachFaceInst(
            df_post=df_post, df_ins=df_ins_car, social_n=social_n
        ).reach()

        self.assertGreater(len(df_reach), 0)
        self.assertGreater(len(df_reach_c), 0)
