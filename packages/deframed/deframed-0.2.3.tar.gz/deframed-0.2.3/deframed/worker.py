...
async def _spawn(task, args=(), kwargs={}, task_status=trio.TASK_STATUS_IGNORED):
    processing.set(None)
...
class BaseWorker:
    ...
    async def talk(self):
        ...
        This task will get cancelled when the websocket terminates. It can
        be used to forward external events to the web page.
        """
        pass
    ...
    async def spawn(self, task, *args, **kwargs):
        """
        Start a new task. Returns a cancel scope which you can use to stop
        the task.
        """
        return await self._nursery.start(_spawn,task,args,kwargs)
...
class Worker(BaseWorker):
    ...
    async def get_attr(self, **a: Dict[str,List[str]]) -> Dict[str,Optional[List[str]]]:
        ...
        does not exist.
        """
        return await self.request("get_attr", a)
