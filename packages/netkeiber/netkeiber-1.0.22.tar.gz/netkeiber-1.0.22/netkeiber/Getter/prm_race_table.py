    ...
    class chrome(browserContainer.chrome):
        ...
        def getPrmRaceResult(self, *args, **kwargs):
            ...
            if raceListItem01:
                ...
                # レース情報class取得
                #raceTable01 = self.wDriver.find_elements_by_class_name('RaceTable01')
                ...
                if raceTables:
                    ...
                    for horseListRow in horseList:
                        try:
                            ...
                            # 競走馬ID
                            horseIdhref = drow[self.raceResultCol.horseId.value].find_element_by_tag_name('a')
                            horseId = ''
                            if horseIdhref:
                            ...
                            # 騎手ID
                            jockeyIdhref = drow[self.raceResultCol.jockey_id.value].find_element_by_tag_name('a')
                            jockey_id = ''
                            if jockeyIdhref:
                                jockey_id = jockeyIdhref.get_attribute('href').split('/')[6]
                            # 調教師ID
                            trainerIdhref = drow[self.raceResultCol.trainer_id.value].find_element_by_tag_name('a')
                            trainer_id = ''
                            if trainerIdhref:
                                trainer_id = trainerIdhref.get_attribute('href').split('/')[6]
                            # 単勝オッズ
                            win_odds = bc.getWinOdds(drow[self.raceResultCol.win_odds.value].text)
                            # 人気
                            popular = bc.getPopular(drow[self.raceResultCol.popular.value].text)
                            # 馬体重
                            horse_weight = bc.getHorseWeight(drow[self.raceResultCol.horse_weight.value].text)
                            weight_diff = bc.getWeightDiff(drow[self.raceResultCol.horse_weight.value].text)
                            # Modelに追加
                            raceResult.newRow()
