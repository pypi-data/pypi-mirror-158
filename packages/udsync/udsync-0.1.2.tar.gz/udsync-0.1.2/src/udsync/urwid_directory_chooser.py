...
class DirectoryChooser(urwid.WidgetWrap, urwid_multi_key_support.MultiKeySupport, View):
	...
	def __init__(self, app: App, select: typing.Callable[[str], None], cancel: typing.Callable[[], None], start_path: str = '', show_hidden: bool = False) -> None:
		self.app = app
		...
		self.text_path.keypress = self.keypress_path  # type: ignore [assignment]  # This is a bug in mypy https://github.com/python/mypy/issues/2427

		self.help_bar = urwid_multi_key_support.HelpBar(self.help_bar_content, MenuWidget._command_map, self.app, edit_context=False)
		self.help_bar_menu = urwid_multi_key_support.HelpBar(self.help_bar_content_menu, MenuWidget._command_map, self.app, edit_context=False)
	...
	def get_content_widgets(self, path: str) -> typing.Sequence[urwid.Widget]:
		...
		if os.path.isdir(path):
			...
			directory_names = []
			self.app.show_error('Directory does not exist: %r' % path)
...
class PathEdit(urwid.WidgetWrap):
	...
	def __init__(self, app: App, label: str, start_path: str) -> None:
		self.app = app
	...
	def on_browse(self) -> None:
		...
		self.app.open_view(DirectoryChooser(self.app, self.on_select, self.on_cancel, start_path=self.get_path()))
	...
	def validate(self) -> bool:
		if mounter.is_valid_path(self.get_path(), self.app.password_screen()):
			return True
