...
class DecoderSelfAttention(AttentionBase):
    ...
    def forward(
        ...
        keys = attention_state[:batch_count].to(torch.float32)
        values = attention_state[batch_count:].to(torch.float32)
...
class DalleBartDecoder(nn.Module):
    ...
    def decode_step(
        ...
        logits = self.lm_head(decoder_state).to(torch.float32)
