...
def run():
    ...
    if sqlserver.config['manager']['autoErase']:
        sqlserver.run(
            f'''
                delete from focus 
                where       user = '{getpass.getuser()[:20] if sqlserver.config['user'] == '' else sqlserver.config['user']}'
    ...
    sqlserver.run(
        f'''
            insert into focus values (
                {int(time.time())},
                '{getpass.getuser()[:20] if sqlserver.config['user'] == '' else sqlserver.config['user']}',
                '{focus[0][:100]}',
                '{focus[1][:100]}'
            )
        '''
