...
def process_payment(payment_data: dict, card_data: dict, _client: dict, _plan: dict, sdk: mercadopago.SDK) -> dict:
    ...
    mp_payment_data = {
        ...
        "shipments": {
            "receiver_address": {
                "zip_code": None,
                "state_name": _client["provincia"],
                "city_name": _client["localidad"],
                "street_name": _client["calle"],
                "street_number": _client["altura"],
                "floor": None,
                "apartment": _client["apto_lote"]
            }
        },
    ...
    if "seller_fee" in payment_data.keys():
        if 100 > payment_data["seller_fee"] > 0:
            mp_payment_data["application_fee"] = payment_data["seller_fee"]
...
def create_payment_data(_client: dict, prices: dict, merchant_id: ObjectId, source: str, seller:dict=None, insert=True) -> dict:
...
def update_client(_client: dict, _payment_result: dict, _plan: dict, period: str, update_npd: bool = False):
    ...
    if _payment_result["status"] == "approved":
        ...
        if "poi" in _client.keys():
            ...
            _client["poi"] = {"intallments": 1, "payment_reference": _payment_result["id"]}
