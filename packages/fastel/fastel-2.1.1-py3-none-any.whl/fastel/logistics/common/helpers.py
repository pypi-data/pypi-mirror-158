...
from fastel.logistics.common.models import LogisticsDataModel, LogisticsStatus
...
class EcpayGetShippingNote:
    ...
    def __call__(self, data: Dict[str, Any]) -> Tuple[bytes, str]:
        validated = self.validate(data)
        parsed = self.parse_payload(validated)
        html = self.gateway.get_shipping_note(parsed)
    ...
    def parse_payload(self, result: LogisticsDataModel) -> EcpayPrintOrderInfo:
        if result.logsitics_subtype == "UNIMARTC2C":
            payload = {
        ...
        elif result.logsitics_subtype in ["FAMIC2C", "OKMARTC2C", "HILIFEC2C"]:
            payload = {
                "LogisticsSubType": result.logsitics_subtype,
                "AllPayLogisticsID": result.logistics_id,
                "CVSPaymentNo": result.logistics_payment_no,
            }
        elif result.logsitics_subtype in [
            "FAMI",
            "UNIMART",
            "UNIMARTFREEZE",
            "HILIFE",
...
class SFLogisticsStatus:
    ...
    def __call__(self, logistics_id: str) -> LogisticsDataModel:
...
def get_callback_method() -> EcpayLogisticsCallback:
    default_payment_provider = get_site_config("default_payment_provider", "neweb")
    if default_payment_provider == "ecpay":
...
async def process_callback(request: Request) -> Response:
    ...
    stepfn_name = f"{callback_result.order_id}_{callback_result.logistics_status}_{tm}"
