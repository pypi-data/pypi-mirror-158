...
class masana(object):
    ...
    def tasks_by_date(self, date:datetime.datetime, log=False):
        ...
        try:
            ...
            if False: #SEARCH IS PREMIUM ONLY
                ...
                """
                Mass getting and manuall filtering
                """
                temp_output = list(self.client.tasks.get_tasks({
                    'assignee': self.user,
                    'workspace':self.workspace,
                    'opt_fields':[
                        'due_at','due_on'
    ...
    def tasks_by_tonight(self, log=False):
        return self.tasks_by_date(datetime.datetime.now().replace(hour=23,minute=59),log)
