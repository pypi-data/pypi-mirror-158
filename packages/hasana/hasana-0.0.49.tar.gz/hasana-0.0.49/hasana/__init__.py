            ...
            if False: #SEARCH IS PREMIUM ONLY
                ...
                for task in self.full_tasks(fields=['due_at','due_on','completed'],log=log):
                    ...
                    if task['due_on'] is not None:
                        print('looking at :> ' + task['due_on'])
                        #due_on = sub.strptime(task['due_on'], '%Y-%m-%d')
                        due_on = parse(task['due_on']) #.strptime(task['due_on'], '%Y-%m-%d')
                    else:
                        due_on = None
                    if task['due_at'] is not None:
                        print('Checking on :> ' + task['due_at'])
                        due_at = parse(task['due_at'])#, 'Y-%m-%dT%H:%M:%S.%fZ')##'%Y-%m-%dT%H:%M:%S.000')
                    ...
                    """
                    if (
                            (due_at is not None and date.replace(hour=0,minute=0) < due_at < date.isoformat())
                            or 
                            (due_on is not None and date.replace(hour=0,minute=0,second=0) <= due_on <= date.isoformat())
                        ):
