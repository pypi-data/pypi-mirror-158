...
class FeatureDefinition(Fco):
    ...
    def _get_historical_features(
        ...
        if has_point_in_time_join_params:
            ...
            if conf.get_bool("ALPHA_QUERYTREE_ENABLED") and not self._proto.HasField("on_demand_feature_view"):
                fdw = FeatureDefinitionWrapper(self._proto, self._fco_container)
                df = DataFrame._create(builder.build_spine_join_querytree(fdw, spine, timestamp_key, from_source))
            else:
