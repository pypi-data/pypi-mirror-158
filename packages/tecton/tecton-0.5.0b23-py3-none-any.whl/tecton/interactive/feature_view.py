...
class StreamFeatureView(FeatureView):
    ...
    def run(
        ...
        **mock_sources: Union[pandas.DataFrame, DataFrame],
        ...
        :param start_time: (Optional) The start time of the time window to materialize. If not set, defaults to `end_time` minus `batch_schedule`. If `end_time` is also not set, defaults to the start of the last complete materialization period.

        :param end_time: (Optional) The end time of the time window to materialize. If not set, defaults to `start_time` plus `batch_schedule`. If `start_time` is also not set, defaults to the end of the last complete materialization period.

        :param aggregation_level: (Optional) For feature views with aggregations, `aggregation_level` configures which stage of the aggregation to run up to.
        ...
        :param \*\*mock_sources: (Optional) kwargs for mock sources that should be used instead of fetching directly from raw data soruces. The keys should match the feature view's function parameters. For feature views with multiple sources, mocking some data sources and using raw data for others is supported.
