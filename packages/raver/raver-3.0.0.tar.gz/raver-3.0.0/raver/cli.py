...
@click.option(
    ...
    "-m",
    default=None,
    help="Path to Python module to check.",
    type=click.Path(
...
class RaverContext:
    ...
    def get(self, key: str, fallback: Any) -> Any:
        """Try to get a key or return None."""
    ...
    def parse_ref(self):
        ...
        fetch_git_ref(self.ref)  # Fetch if it's a remote branch.
        self.current_ref = get_current_ref()
        self.ref_changes = has_ref_changes(self.current_ref, self.ref)
...
def has_ref_changes(cur: Optional[str], ref: str) -> bool:
    debug("Checking for changes...")

    try:
        changes = capture(f"git diff {cur} {ref}")
    except Exception as e:
