...
def resolve_payload(node):
    ...
    # scout_spec = node.parm("scout_frames").eval()
    # # print("scout_spec", scout_spec)
    # match = AUTO_RX.match(scout_spec)
    # if match:
    #     samples = int(match.group(1))
    #     scout_seq = main_seq.subsample(samples)
    # else:
    #     try:
    #         scout_seq = Sequence.create(scout_spec)
    #     except:
    #         return {}

    # return {"scout_frames": ",".join([str(s) for s in scout_seq or []])}