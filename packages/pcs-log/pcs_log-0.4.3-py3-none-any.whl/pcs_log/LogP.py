...
class _LogP():
    ...
    class __PortLogServer():
        ...
        def __ReaderProc(self) -> None:
            ...
            logging.log(_STATUS,f'Log-server - Starting')
            ...
            for i in range(10):
                ...
                try:
                    ...
                    logging.warning(f'Log-server - Can not connect')
                    time.sleep(1)
            if not IsOk:
                self.__RunFlag = False
                return
            ...
            server.close()
            self.__RunFlag = False
            logging.log(_STATUS,f'Log-server - Terminating')
    ...
    # Add the Filter to implement stack-traces
        ...
        if not isinstance(LogLongLevel, str):
            raise ValueError(_Translation['LogLongLevelNoStr'].format(**{'ClassName': ClassName, 'LogLongLevel': LogLongLevel})) 
        ...
        if LogPath == '' and not StdErr:
            IsSyslog = True
        logging.getLogger().addFilter(self._MyContextFilter(trim_amount = LogStackDepth,below_level = Sd, is_syslog = IsSyslog))
        ...
        if LogPath != '':
            ...
            if AddPar == '' and ShowLevel == '':
                Format = f"%(asctime)s %(message)s%(stack)s"
                DebugFormat = f"%(asctime)s %(message)s%(stack)s"
            else:
                Format = f"%(asctime)s %(message)s%(stack)s"
