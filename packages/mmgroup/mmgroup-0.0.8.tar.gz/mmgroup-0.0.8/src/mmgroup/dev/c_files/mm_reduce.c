...
/** @file mm_reduce.c
 ...
 is the 198884-dimensional faithful rperesentation of the monster
 ...
 \code{.c}
     // The follwing buffers contain the element ``g`` to be reduced
     ...
     mm_reduce_load_axis(v, 0);       // Load 2A axis v_plus
     mm_op15_word(v, a, n, 1, work);  // multiply v_plus with g
     mm_reduce_vector_vp(v, mode, r, work); // reduce using axis v_plus  

     mm_reduce_load_axis(v, 1);       // Load 2A axis v_minus
     mm_op15_word(v, a, n, 1, work);  // multiply v_minus with g
     mm_reduce_vector_vm(v, r, work); // reduce using axis v_minus   
...
/* Vector \fv^+\f$ in Leech lattice encoding */
...
/**
  ...
  The function loads the 2A axis \f$v^+\f$ (in case ``sign == 0``)
...
// %%EXPORT px
MM_REDUCE_API
void  mm_reduce_load_axis(uint_mmv_t *v, uint32_t sign)
{
...
{
    uint32_t len_r = 0, i, status, len_r1, vt, v4, ax_type;
    ...
    for (i = 0; i < 5; ++i) {
        ...
        if (ax_type == 0x21) {
          ...
          #ifdef CHECK_AXIS
            mm_op15_word(v, r + len_r, len_r1, 1, p_work);
            vt = mm_aux_get_mmv1(15, v, (24+3)*32 + 2);
          #else
            mm_op15_word_ABC(v, r + len_r, len_r1, p_work);
            vt = mm_aux_get_mmv1(15, p_work, (24+3)*32 + 2);
          #endif
            len_r += len_r1;
          ...
          #ifdef CHECK_AXIS
            mm_reduce_load_axis(p_work, 0);
            if (mm_op15_compare(v, p_work)) return -10003;
          #endif
            r[len_r++] = 0x84000200;
            return len_r;
        }
...
/**
  ...
  the 2A axis \f$v^+ \in \rho_{15}\f$ under the operation of an 
...
// %%EXPORT px
MM_REDUCE_API
int32_t mm_reduce_vector_vp(uint_mmv_t *v, uint32_t mode, uint32_t *r, uint_mmv_t *work)
{
    int32_t res;
    r[0] = 0;
    res =  reduce_v_axis(v, r + 1, mode & 1, work);
    if (res > 0 && res <= 40) {
        ++res;
...
{
    uint32_t len_r = 0, i, status, len_r1, val2, vt, v4, ax_type, e;
    uint32_t target_axes[2];
    ...
    for (i = 0; i < 5; ++i) {
        ...
        if (ax_type == 0x21) {
            vt = p_ax->v_leech2[0];
            val2 = mm_op15_eval_A(v, axis);
            if (val2 == 0)  {
                vt ^= axis;
                len_r1 = gen_leech2_reduce_type4(vt, r + len_r); 
                if (len_r1 < 0) return -20011;
              #ifdef CHECK_AXIS
                mm_op15_word(v, r + len_r, len_r1, 1, p_work);
              ...
              #else
                mm_op15_word_ABC(v, r + len_r, len_r1, p_work);
                vt = mm_reduce_2A_axis_type(p_work) & 0xffffff;
                vt = v_leech2_adjust_sign(p_work, vt);
              #endif
                if (op_axis(&axis, r + len_r, len_r1)) return -20011;
                len_r += len_r1;
                r[len_r++] = 0x82000000 + (vt & 0xffffff);
                vt = gen_leech2_mul(vt, axis); 
                if ((vt & 0xffffff) != 0x800000) return -20012;
                e = 1 + ((vt >> 24) & 1);
                r[len_r] = 0xD0000003 - e;
              #ifdef CHECK_AXIS
                mm_op15_word(v, r + len_r, 1, 1, p_work);
                if (op_axis(&axis, r + len_r, 1)) return -20013;
              #endif
                len_r += 1;
            } else  {
                if (val2 != 4)  return -20014;
            }
            #ifdef CHECK_AXIS
                vt = mm_reduce_2A_axis_type(v) & 0xffffff;
                vt = v_leech2_adjust_sign(v, vt);
                if (vt < 0) return -22000 + vt;
                if ((vt ^ axis ^ 0x1000000) & 0x1ffffff) return -20015;
            #endif
...
/**
  ...
  Here input ``v`` of this function must refer to the image of
  the 2A axis \f$v^- \in \rho_{15}\f$ under the operation of the 
  same element \f$g\f$ of the monster group as described in the
  documentation of function ``mm_reduce_vector_vp``.
...
// %%EXPORT px
MM_REDUCE_API
int32_t mm_reduce_vector_vm(uint_mmv_t *v, uint32_t *r, uint_mmv_t *work)
{
    int32_t res;
    ...
    if (res >= 0)
         res = reduce_v_baby_axis(v, axis, r + len_start, work);
    if (res > 0 && res <= 40) {
         res += len_start;
...
int32_t mm_reduce_vector_v1(uint_mmv_t *v, uint32_t *r, uint_mmv_t *work)
{
    ...
    res =  mm_op15_word(v, r, len_start, 1, work);
    if (res >= 0)
         res = mm_order_check_in_Gx0(v, r + len_start, 3, work);
    if (res >= 0 && res <= 12) {
         res += len_start;
         r[0] = 0;
...
/////////////////////////////////////////////////////////////////////////
// Reducing an element of the moster group
/////////////////////////////////////////////////////////////////////////
...
int32_t mm_reduce_M(uint32_t *a,  uint32_t n, uint32_t mode, uint32_t *r)
{
    ...
    uint_mmv_t *v = calloc(2 * MM_OP15_LEN_V, sizeof(uint_mmv_t));
    uint_mmv_t *work;
    if (v == 0) return -1;
    work = v + MM_OP15_LEN_V;

    mm_reduce_load_axis(v, 0);
    if ((res = mm_op15_word(v, a, n, 1, work)) < 0) goto done;
    if ((res = mm_reduce_vector_vp(v, mode, r, work)) < 0) goto done;    

    mm_reduce_load_axis(v, 1);
    if ((res = mm_op15_word(v, a, n, 1, work)) < 0) goto done;
    if ((res = mm_reduce_vector_vm(v, r, work)) < 0) goto done;   
