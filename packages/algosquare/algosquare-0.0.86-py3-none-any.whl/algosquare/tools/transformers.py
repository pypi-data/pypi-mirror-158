...
def from_timestamp(df):
    def float_to_timestamp(x):
        if pd.isnull(x):
            return x
        return datetime.datetime.fromtimestamp(x)
    return df.astype(float).applymap(float_to_timestamp)
...
class BinaryTransformer(BaseEstimator, TransformerMixin):
    ...
    Args:
        nan_unknowns: replaces unknown category with nan.
    """
...
class UnknownToNanTransformer(BaseEstimator, TransformerMixin):
    """Sets unknown categories to nan."""
    def fit(self, X, y = None):
        """
        Fits transformer to X.
    ...
    def transform(self, X):
        """
        Replaces missing and unknowns with nan.
        ...
        idx_unknowns = []
        for col, s in X.items():
            idx_unknowns.append(~s.isin(self.categories[col]))

        return X.mask(pd.concat(idx_unknowns, axis=1))
