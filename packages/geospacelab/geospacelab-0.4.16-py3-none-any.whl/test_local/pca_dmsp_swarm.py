...
def visual_dmsp_swarm(dmsp_dn, dmsp_sat_id, dmsp_orbit_id, swarm_dn, swarm_sat_id, pole='N'):
    ...
    db_swarm.set_layout([[n_e], [T_e], [d_B_x, d_B_y, d_B_z], [pf_h, pf_v], [qflag, cflag]],
                       left=0.5, right=0.9, hspace=0.01 )
    db_swarm.draw()
    
    db_swarm.add_title(title='SWARM-' + swarm_sat_id.upper() + ', ' + swarm_dn.strftime('%Y-%m-%d %H:%M UT'))
    db_swarm.save_figure(file_name = 'compare_SWARM-' + swarm_sat_id + '_DMSP-' + dmsp_sat_id.upper(), 
