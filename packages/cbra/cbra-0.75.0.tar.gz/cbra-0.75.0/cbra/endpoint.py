...
class Endpoint(IEndpoint, metaclass=EndpointMetaclass):
    ...
    def responses(cls) -> dict[str | int, Any]:
        ...
        defaults = {
            ...
            'default': get_exception_response('default', headers, has_body),
        }
    ...
    def add_to_router(
        ...
        **kwargs: typing.Any
        ...
        description = kwargs.setdefault('description',
            cls.description or getattr(cls.handle, '__doc__', None))
        if hasattr(cls.handle, '__doc__'):
            # TODO: Strip preceding spaces from the docstring to prevent
            # rendering as a code block. This is quite ugly, since we have
            # to assume that the indentation is 4 spaces.
            description = kwargs['description'] = str.join('\n', [
                x[8:] if i>0 else x for i, x in enumerate(str.split(description, '\n'))
            ])
