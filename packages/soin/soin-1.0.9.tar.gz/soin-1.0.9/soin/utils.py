...
def download_file(url: str, dest: str = None) -> bytes:
    ...
    for i in range(5):
        ...
        logging.error(f"download file from {url} failed after {i + 1}, quit")
        fp.close()
        os.remove(fp.name)
        raise e
    if dest is None:
        ...
        def real_func(*args, **kwargs):
            for i in range(count):
                try:
                    ...
                    logging.exception(f"call func {func.__name__} failed for {i} time, exception {e}")
                    time.sleep(1 + i)
            else:
                logging.error(f"call func {func.__name__} failed after {count} time")
                return None
            return res
