...
Eigen::VectorXd propensity_function( const Eigen::MatrixXi& X_compressed,  const Eigen::MatrixXi& X_states, const Eigen::MatrixXi& X_spatial, const Eigen::MatrixXi& X_full,
                                      double* parameters, double* forward_rates,  double t, int n_total_reactions,
									  int n_ribosomes, int R, int max_length, double& probe_binding_rate, Eigen::MatrixXi& X_probes, std::uniform_real_distribution<double>, std::mt19937_64){
...
void generic_ssa_cpp(int* result, int* intensity, int* states, int* Stoich_states, int* Stoich_lattice, double* forward_rates, double* parameters, int* xi_lattice, int* xi_state, double* time_vector, 
	...
	while (t < tf){
        ...
        wn = propensity_function(X_comp,X_states, X_spatial, X_lat, parameters, forward_rates, t, n_total_rxns, N_particles, R, length, probe_binding_rate, X_probes, unif, rng);
		a0 = wn.sum();
