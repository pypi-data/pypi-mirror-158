...
class UseResourceGenerator:
    ...
    def write_analyzed(self):
        """
        Builds the entire UseResource archive using the generated Dolphin logs. This also populates the output archive.
        """
        all_archives = list()
        all_sounds = list()
        ...
        buffer = pyjkernel.write_archive_buffer(self._archive_)
        buffer = pyjkernel.compress_szs(buffer, 9)

        with open(arc_path, "wb") as f:
            f.write(buffer)
            f.flush()
        pyjkernel.write_archive_file(self._archive_, arc_path)
    ...
    def write_dummy(self):
        ...
        arc_path = os.path.join(self._game_files_, f"StageData/{self._galaxy_name_}/{self._galaxy_name_}UseResource.arc")
        buffer = pyjkernel.write_archive_buffer(arc)
        buffer = pyjkernel.compress_szs(buffer, 9)
