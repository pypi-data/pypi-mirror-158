...
class JOSEPayload:
    ...
    def compress_jwe(
        ...
        """Flatten JSON Web Encryption (JWE) `jwe` if there is a single recipient
        and `compact` is ``False`, otherwise use compact serialization or JSON
        serialization if there are multiple recipients.
        """
        is_single = len(jwe['recipients']) == 1
        result: str | dict[str, Any] = jwe
        if is_single:
            recipient = jwe['recipients'][0]
            result.update(recipient)
            del result['recipients']
            if compact:
                result = (
                    f"{result['protected']}."
                    f"{result['encrypted_key']}."
                    f"{result.get('iv') or ''}."
                    f"{result['ciphertext']}."
                    f"{result.get('tag') or ''}"
                )
