...
class NodeInfoConverter(Converter):
    def to_entity(self, do: NodeInfoDO):
        return NodeInfo(
            ...
            cpu_usage = None if do.cpu_usage is None else Usage(do.cpu_usage),
            memory_size = None if do.memory_size is None else Size(do.memory_size),
            actual_memory_size = None if do.actual_memory_size is None else Size(do.actual_memory_size),
            memory_usage = None if do.memory_usage is None else Size(do.memory_usage),
            flow_in = None if do.flow_in is None else Size(do.flow_in),
            flow_out = None if do.flow_out is None else Size(do.flow_out),
            disk_size = None if do.disk_size is None else Size(do.disk_size),
            disk_usage = None if do.disk_usage is None else Usage(do.disk_usage),
            io_in = None if do.io_in is None else Number(do.io_in),
            io_out = None if do.io_out is None else Number(do.io_out),
