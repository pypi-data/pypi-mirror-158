...
class FvExtractor(Extractor, Mesh):
    ...
    def write_time_series_file(self, out_file, locations, variables=None):
        ...
        for name in locations:
            nc.createGroup(name)
            nc[name].createDimension('N1', 1)

            nc[name].createVariable('X', 'f8', dimensions=('N1',))
            nc[name].createVariable('Y', 'f8', dimensions=('N1',))

            nc[name].variables['X'][:] = locations[name][0]
            nc[name].variables['Y'][:] = locations[name][1]

            nc[name].createDimension('NumLayers', self.nz[idx[name]])
            nc[name].createDimension('NumLayerFaces', self.nz[idx[name]]+1)
