...
def _to_table(result: LatestAnalysisResultResponse) -> LatestAnalysisResultTable:
...
def to_table(result: LatestAnalysisResultResponse, force_factor_value_to_str: bool = True) -> LatestAnalysisResultTable:
    table = _to_table(result)
    if force_factor_value_to_str:
        for i, row in enumerate(table.rows):
            row: Row = row
            table.rows[i].factor_0_value = str(row.factor_0_value)
            table.rows[i].factor_1_value = str(row.factor_1_value)
            table.rows[i].factor_2_value = str(row.factor_2_value)
    return table
