...
def CIFARunsup(root, download=True,
               transform=transforms.ToTensor(), unsup_fraction=0.5):
    
    filename = "ti_500K_pseudo_labeled.pickle"
    
    if download:
        download_file_from_google_drive("1LTw3Sb5QoiCCN-6Y5PEKkq9C9W60w-Hi", 
                                        root, filename)
    
    trainset = SemiSupervisedDataset(base_dataset='CIFAR10'.lower(),
                                     add_svhn_extra=False,
                                     root=root, train=True,
                                     download=True, transform=transform,
                                     aux_data_filename=filename,
                                     add_aux_labels=True,
