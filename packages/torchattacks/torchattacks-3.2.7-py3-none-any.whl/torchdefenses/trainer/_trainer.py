...
class Trainer():
    ...
    def fit(self, train_loader, optimizer, max_epoch,
        ...
        self.__check_valid_options(save_type)
        self.__check_valid_options(record_type)

        # Init Optimizer and Schduler
        self.__init_optimizer(optimizer)
        self.__init_schdeuler(scheduler, scheduler_type)
        self.__init_minimizer(minimizer)
        ...
        if save_path is not None:
            ...
            # Save Initial Model
            self.__check_path(save_path, overwrite=save_overwrite)
            self.__check_path(save_path+"/last.pth", overwrite=save_overwrite, file=True)
            self.__check_path(save_path+"/best.pth", overwrite=save_overwrite, file=True)
            self.__check_path(save_path+"/record.csv", overwrite=save_overwrite, file=True)
            self.__check_path(save_path+"/summary.txt", overwrite=save_overwrite, file=True)

            if save_type in ["Epoch", "Iter"]:
                self.__check_path(save_path+"/epoch_iter/", overwrite=save_overwrite)
                self.__save_dict_to_file(save_path, 0)
        else:
        ...
        for epoch in range(self.max_epoch):
            ...
            for i, train_data in enumerate(train_loader):
                ...
                self.__init_record_dict()
                self.__init_save_dict()
                ...
                if record_type is not None:
                    if self.rm is None:
                        ...
                        if save_best is not None:
                            self.__check_save_best(save_best, record_keys)
                    else:
                        self.rm.progress()

                    iter_record = self.__get_record_from_record_dict()
                    if record_type == "Epoch" and is_last_batch:
                        lr = self.optimizer.param_groups[0]['lr']
                        curr_record = self._update_record([self.epoch]+iter_record, lr=lr)
                        self.__update_record_and_best(record_keys, curr_record,
                                                      save_best, save_path)
                ...
                if save_type == "Epoch" and is_last_batch:
                    self.__save_dict_to_file(save_path, self.epoch)
                elif save_type == "Iter":
                    self.__save_dict_to_file(save_path, self.epoch, self.iter)
                else:
                    pass
        ...
        if record_type is not None:
            ...
            self.rm.to_csv(save_path+"/record.csv", verbose=False)
            self.__save_dict_to_file(save_path, is_last=True)
    ...
    def __get_record_from_record_dict(self):
        iter_record = []
        for key in self.record_dict.keys():
    ...
    def __init_save_dict(self):
        del self.save_dict
        self.save_dict = OrderedDict()

    # Init Optimizer
    def __init_optimizer(self, optimizer):
    ...
    # Init Scheduler
    def __init_schdeuler(self, scheduler, scheduler_type):
    ...
    # Init Minimizer
    def __init_minimizer(self, minimizer):
    ...
    def __check_save_best(self, save_best, record_keys):
    ...
    def __check_record_best(self, save_best):
    ...
    def __update_record_and_best(self, record_keys, curr_record,
                                 save_best, save_path):
        self.curr_record_dict = OrderedDict(zip(record_keys, curr_record))
        self.rm.add(curr_record)

        if save_best is not None:
            is_best = self.__check_record_best(save_best)
            if is_best:
                self.__save_dict_to_file(save_path, is_best=True)

    # Check and Create Path
    def __check_path(self, path, overwrite=False, file=False):
