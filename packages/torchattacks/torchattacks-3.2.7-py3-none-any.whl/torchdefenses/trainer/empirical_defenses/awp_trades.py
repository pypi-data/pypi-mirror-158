...
class AWP_TRADES(AdvTrainer):
    ...
    """
    def __init__(self, rmodel, eps, alpha, steps, beta, awp_gamma=0.01, proxy_lr=0.01):
        super().__init__("AWP_TRADES", rmodel)
        ...
        self.awp_adversary = TradesAWP(model=self.rmodel.model, proxy=self.proxy, proxy_optim=self.proxy_opt, gamma=awp_gamma)
    ...
    def _do_iter(self, train_data):
        ...
        awp = self.awp_adversary.calc_awp(inputs_adv=adv_images, inputs_clean=images,
                                          targets=labels, beta=self.beta)
