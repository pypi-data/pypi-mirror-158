...
class InfoDialog(QtWidgets.QDialog, Ui_InfoDialog):
    ...
    def create_device_info_string(self):
        ...
        commit_id = d['commit_id'] if d.get('commit_id') else "N/A"
        s += "Commit ID: {}\n".format(commit_id)
        repo_branch = d['repo_branch'] if d.get('repo_branch') else "N/A"
        s += "Repo Branch: {}\n".format(repo_branch)
        repo_name = d['repo_name'] if d.get('repo_name') else "N/A"
        s += "Repo Name: {}\n".format(repo_name)
        ...
        if d.get('nvm_modified') is True:
            nvm_modified = "Yes"
        elif d.get('nvm_modified') is False:
            nvm_modified = "No"
        else:
            nvm_modified = "Unknown"
        s += "NVM Modified: {}\n".format(nvm_modified)

        nvm_hash = d['nvm_hash'] if d.get('nvm_hash') else "N/A"
        s += "NVM Hash: {}\n".format(nvm_hash)
        setting_hash = d['setting_hash'] if d.get('setting_hash') else "N/A"
        s += "Setting Hash: {}\n".format(setting_hash)
        ...
        for i, (name, supply) in enumerate(d['supplies']):
            ...
            if 'supply_results' in d:
                ...
                if supply_results is not None:
                    ...
                    scaled_nominal = (supply.nominal * supply.scale +
                                      supply.offset)
