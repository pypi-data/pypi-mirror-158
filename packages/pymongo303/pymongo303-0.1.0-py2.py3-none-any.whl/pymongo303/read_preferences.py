...
from pymongo303.server_selectors import (member_with_tags_server_selector,
    ...
    for tags in tag_sets:
        if not isinstance(tags, Mapping):
            raise TypeError(
                ...
                "bson303.son.SON or other type that inherits from "
...
class ReadPreference(object):
    ...
    :class:`~pymongo303.mongo_client.MongoClient` connected to a single mongod:
    ...
    :class:`~pymongo303.mongo_client.MongoClient` initialized with the
    ``replicaSet`` option:

    - ``PRIMARY``: Read from the primary. This is the default, and provides the
      strongest consistency. If no primary is available, raise
      :class:`~pymongo303.errors.AutoReconnect`.

    - ``PRIMARY_PREFERRED``: Read from the primary if available, or if there is
      none, read from a secondary.

    - ``SECONDARY``: Read from a secondary. If no secondary is available,
      raise :class:`~pymongo303.errors.AutoReconnect`.
    ...
    :class:`~pymongo303.mongo_client.MongoClient` connected to a mongos, with a
    sharded cluster of replica sets:

    - ``PRIMARY``: Read from the primary of the shard, or raise
      :class:`~pymongo303.errors.OperationFailure` if there is none.
      This is the default.
