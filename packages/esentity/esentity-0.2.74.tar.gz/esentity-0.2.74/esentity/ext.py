...
class AuthorExtension(Extension):
    ...
    def _render(self, args, caller=None):
        logger.info('author_badge IN Args: {0}'.format(args))

        default = ['default', False]
        _ca = len(default)
        args = args[:_ca] + default[len(args):] + args[_ca:]

        logger.info('author_badge Args OUT: {0}'.format(args))

        author = args[0]
        prefix = bool(args[1])

        t = current_app.jinja_env.get_template('_ext-author.html')
        s = t.render(author=author, prefix=prefix)
        return s
...
class PagesListExtension(Extension):
    ...
    def _render(self, args, caller=None):
        ...
        default = [20, 4, 'pages_list', [], {'tags': 'Promo'}]
        _ca = len(default)
        args = args[:_ca] + default[len(args):] + args[_ca:]
