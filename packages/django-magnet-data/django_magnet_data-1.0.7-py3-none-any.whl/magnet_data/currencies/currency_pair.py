...
class CurrencyPair:
    ...
    def is_convertion_possible(self, date):
        ...
        if date > today:
            ...
            if today.day < 9:
                last_known_clf_date = datetime.date(today.year, today.month, 9)
            elif today.month < 12:
                last_known_clf_date = datetime.date(today.year, today.month + 1, 9)
            else:
                last_known_clf_date = datetime.date(today.year + 1, 1, 9)

            return last_known_clf_date >= date
