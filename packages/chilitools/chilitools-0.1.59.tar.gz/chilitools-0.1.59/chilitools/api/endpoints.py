...
class System:
  ...
  def GenerateApiKey(self, username: str = None, password: str = None) -> tuple:
    ...
    # Request
    resp = self.connector.makeRequest(
      method='post',
    ...
    if resp.success:
      if 'apiKey' in resp.contentAsDict().keys():
        return (True, resp.contentAsDict()['apiKey'])
    return (False, ErrorHandler().getError('GENAPIKEY'))
