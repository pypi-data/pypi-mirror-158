...
class LibraryReader(object):
    ...
    def _send_analysis_results_files_loaded(self, reportable_dists):
        ...
        if self.settings.config.is_service_bypassed:
            ...
            msg = teamserver_messages.ObservedLibraryUsage(reportable_dists)

            logger.debug(
                "Sending list of modules loaded to Contrast", direct_to_teamserver=1
            )

            self.send_message_func(msg)
        else:
            ...
            if send_msg:
                ...
                logger.debug(
                    "Sending Activity message with %s files loaded",
                    total_loaded_files_cnt,
                )
