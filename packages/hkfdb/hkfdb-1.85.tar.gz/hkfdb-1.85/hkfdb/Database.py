...
class Database:
    ...
    def get_hk_fut_ohlc(self, index, start_date, end_date, freq, rolling_day, rolling_time = 0, rth_only=False):
        ...
        if False not in list(check_bool_dict.values()):
            ...
            year_month_involved_list_str = year_month_involved_list_str.replace('[', '', regex=False)
            year_month_involved_list_str = year_month_involved_list_str.replace(']', '', regex=False)
            year_month_involved_list_str = year_month_involved_list_str.replace('\'', '', regex=False)
            year_month_involved_list_str = year_month_involved_list_str.replace(',', '', regex=False)
            year_month_involved_list_str = year_month_involved_list_str.replace(' ', '_', regex=False)
            year_month_involved_list_str = year_month_involved_list_str.replace('\"', '', regex=False)
    ...
    def get_hk_shortsell_by_code(self, code):
        ...
        if response_ok == True:
            ...
            df['ss_shares'] = df['ss_shares'].astype('int64')
            df['ss_dollars'] = df['ss_dollars'].astype('int64')
