...
class Domain:
    ...
    # add some other attr. by kleex.
    registrant_phone = None  # registrant_phone
    emails = None  # registrant_email
    registrant_organization = None  # registrant_organization
    registrant_state = None  # registrant_state_province
    registrant_city = None  # registrant_city
    address = None  # registrant_address
    registrant_email = None  # registrant_email
    ...
    def __init__(
        ...
        if "registrant" in data:
            self.registrant_organization = self.registrant = data["registrant"][0].strip()

        if "admin" in data:
            self.admin = data["admin"][0].strip()

        if "emails" in data:
            self.emails = data["emails"][0].strip()
        else:
            self.emails = ""

        if "registrant_email" in data:
            self.registrant_email = data["registrant_email"][0].strip()
        else:
            self.registrant_email = ""

        if "registrant_address" in data:
            self.registrant_address = data["registrant_address"][0].strip()
        else:
            self.registrant_address = ""

        if "registrant_phone" in data:
            self.registrant_phone = data["registrant_phone"][0].strip()
        else:
            self.registrant_phone = ""

        if "registrant_state" in data:
            self.registrant_state = data["registrant_state"][0].strip()
        else:
            self.registrant_state = ""

        if "registrant_city" in data:
            self.registrant_city = data["registrant_city"][0].strip()
        else:
