...
class Report(ReportPyx):
    def __init__(self, creturn, position, fee_ratio, tax_ratio, trade_at, next_trading_date, market_info):
        ...
        Args:
          ...
          tax_ratio (float): 交易稅率，預設為台灣普通股一般交易交易稅率 0.003。若交易策略的標的皆為ETF，記得設成 0.001。
        ...
        Attributes:
          ...
          asset_type (str): 資產類別，`'tw_stock' or 'crypto'`
    ...
    def get_stats(self, resample='1d', riskfree_rate=0.02, ndays_return_range=None):
        """取得策略統計數據
        取得數據如夏普率、索提諾比率、最大回檔、近期報酬率統計...
        Args:
           resample (str): 報酬率檢測週期，其他常用數值為 W、 M 、 Q（每週、每月、每季換股） 。
           riskfree_rate (float): 無風險利率。
           ndays_return_range (list): 回測近n日報酬率的統計集合，預設回測`[1, 5, 10, 20, 60]`。
        Returns:
          (dict): 策略指標數據。
        """
        return super().get_stats(resample, riskfree_rate, ndays_return_range)
    ...
    def display_mae_mfe_analysis(self, atr_freq=10, violinmode='group'):
        """ 顯示波動分析圖組
        [分析使用說明](https://www.finlab.tw/display_mae_mfe_analysis/)。
        Args:
          atr_freq (int): edge_ratio使用的 ATR 時段週期參數。預設為10。
          violinmode (str): violin型態統計圖樣式，模式分為'group'與'overlay'。
                            預設為'group'，'group'模式為將交易勝敗手分群統計呈現'，'overlay'採取全數統計。
        Returns:
          (plotly.graph_objects.Figure): 波動分析圖組
          ![波動分析圖組](img/analysis/display_mae_mfe.png)
        """
        return super().display_mae_mfe_analysis(atr_freq, violinmode)
