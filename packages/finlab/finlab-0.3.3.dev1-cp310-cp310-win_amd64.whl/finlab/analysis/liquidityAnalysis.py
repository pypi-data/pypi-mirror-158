...
class LiquidityAnalysis(Analysis):
  ...
  def __init__(self, required_volume=200000, required_turnover=1000000):
    ...
    Args:
        required_volume (int): mininum volume
        required_turnover (int): mininum turnover
  ...
  def calculate_trade_info(self, report):
    ...
    ret = [
      ...
      ["處置股", ~data.get('etl:disposal_stock_filter'), signal_dates],
      ["警示股", ~data.get('etl:disposal_stock_filter'), signal_dates],
      ["全額交割股", ~data.get('etl:full_cash_delivery_stock_filter'), signal_dates]
    ]
  ...
  def analyze(self, report):
    ...
    entry_buy_at_top = long_position & (trades['交易當下漲跌@entry_date'] > entry_pct_range * 0.99)
    entry_sell_at_bottom = (~long_position) & (trades['交易當下漲跌@entry_date'] < -entry_pct_range * 0.99)

    exit_sell_at_bottom = long_position & (trades['交易當下漲跌@exit_date'] < -exit_pct_range * 0.99)
    exit_buy_at_top = (~long_position) & (trades['交易當下漲跌@exit_date'] > exit_pct_range * 0.99)
