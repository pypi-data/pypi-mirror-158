...
class FinlabDataFrame(pd.DataFrame):
    ...
    def index_str_to_date(self):
      """財務月季報索引格式轉換
        將以下資料的索引:月營收 (ex:2022M01)從文字格式轉為公告截止日的datetime格式，
        財務季報 (ex:2022Q1) 從文字格式轉為資料上傳日的datetime格式。
    ...
    def hold_until(self, exit, nstocks_limit=None, stoploss=-np.inf, takeprofit=np.inf, trade_at='close', rank=None):
        ...
        此函式有很多細部設定，可以讓你最多選擇 N 檔股票做輪動。另外，當超過 N 檔進場訊號發生，也可以按照客制化的排序，選擇優先選入的股票。最後，可以持外設定停損停利，來增加出場的時機點。
        Args:
          exit (pd.Dataframe): 出場訊號。
          nstocks_limit (int)`: 輪動檔數上限，預設為None。
          stoploss (float): 停損基準，預設為None，不執行停損。範例：0.1，代表成本價下跌 10% 時出場。
          takeprofit (float): 停利基準，預設為None，不執行停利。範例：0.1，代表成本價上漲 10% 時出場。
