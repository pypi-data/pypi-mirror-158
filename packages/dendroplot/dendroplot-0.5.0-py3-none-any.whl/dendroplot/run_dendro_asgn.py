...
def run_dendro(label='mycloud', cubefile=None, mom0file=None, redo='n', rfreq=None,
    verbose=True, noshow=False, pltdir='plots', asgndir='asgn', **kwargs):
    ...
    for i, c in enumerate(srclist):
        struct_mask = d[c].get_mask()
        peakim = np.nanmax(hdu3.data*struct_mask, axis=0)
        peakim[peakim==0] = np.nan
        tmax[i]  = np.nanmax(peakim)
        tpkav[i] = np.nanmean(peakim)
        indices = ndimage.find_objects(struct_mask)[0]
        asgn = struct_mask[indices]
        new_wcs = w.slice(indices)
        asgnfile = join(asgndir, 'dendro_'+str(c)+'.fits.gz')
        fits.writeto(asgnfile, asgn.astype(np.uint8), new_wcs.to_header(), overwrite=True)
    if hd3['BUNIT'].upper()=='JY/BEAM':
