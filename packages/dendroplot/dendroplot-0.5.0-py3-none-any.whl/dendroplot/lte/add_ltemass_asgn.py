...
def add_ltemass(label = 'pcc_12', n13cub = None, i12cub = None, i13cub = None,
                efloor=0, n13cub_uc = None, distpc = 5e4, co13toh2 = 5.0e6, asgndir=None):
        ...
        if 'flux' in col or 'mlte' in col:
            for i, c in enumerate(srclist):
                if asgndir is not None:
                    adat, ahd = getdata(os.path.join(asgndir,'dendro_'+str(c)+'.fits.gz'), 
                                        header=True)
                    pad1i = int(hd['crpix1'] - ahd['crpix1'])
                    pad2i = int(hd['crpix2'] - ahd['crpix2'])
                    pad3i = int(hd['crpix3'] - ahd['crpix3'])
                    pad1f = int(hd['naxis1'] - ahd['naxis1']) - pad1i
                    pad2f = int(hd['naxis2'] - ahd['naxis2']) - pad2i
                    pad3f = int(hd['naxis3'] - ahd['naxis3']) - pad3i
                    asgn = np.pad(adat,((pad3i,pad3f),(pad2i,pad2f),(pad1i,pad1f)))
                    #mask = (asgn > 0)
                    if not col.startswith('e_'):
                        datcol[i] = np.nansum(data*asgn)
                        # nansum returns zero if all are NaN, want NaN
                        chknan = np.asarray(np.isnan(data*asgn))
                        if chknan.all():
                            datcol[i] = np.nan
                    else:
                        datcol[i] = np.sqrt(np.nansum((data*asgn)**2)) * osamp 
                else:
                    mask = d[c].get_mask()
                    if not col.startswith('e_'):
