...
def calc_phys_props(label='pcc_12', cubefile=None, boot_iter=100, efloor=0,
        ...
        ancfile=None, anclabel=None, refpos=None, asgndir=None):
    ...
    cube, hd3 = fits.getdata(cubefile, header=True)
    metadata = {}
    ...
    if copbcor is not None and conoise is not None:
        cube12, hd12 = fits.getdata(copbcor, header=True)
        ecube12, ehd12 = fits.getdata(conoise, header=True)
    ...
    if ancfile is not None:
        ancdata,anchd = fits.getdata(ancfile, header=True)
...
#           ealpha, tb12, ancmean, ancrms = [np.zeros(len(srclist)) for _ in range(14)]
    ...
    for j, clust in enumerate(srclist):
        ...
        if asgndir is not None:
            adat, ahd = fits.getdata(os.path.join(asgndir,'dendro_'+str(clust)+'.fits.gz'), 
                                     header=True)
            pad1i = int(hd3['crpix1'] - ahd['crpix1'])
            pad2i = int(hd3['crpix2'] - ahd['crpix2'])
            pad3i = int(hd3['crpix3'] - ahd['crpix3'])
            pad1f = int(hd3['naxis1'] - ahd['naxis1']) - pad1i
            pad2f = int(hd3['naxis2'] - ahd['naxis2']) - pad2i
            pad3f = int(hd3['naxis3'] - ahd['naxis3']) - pad3i
            asgn = np.pad(adat,((pad3i,pad3f),(pad2i,pad2f),(pad1i,pad1f)))

            #asgn, footprint = reproject_interp(hdu0, hd3, order=0)
        else:  
