    ...
    def acc(self):
        ...
        for planet in self.gravSys.planets:
            if planet is not self:
                ...
                if d<=0.62:
                    print('Dropped')
                self.gravSys.dt=0.00002*(d**1.2)*self.gravSys.dt_s
                try:
...
def main():
    ...
    try:scr.register_shape(IMAGE)
    except turtle.TurtleGraphicsError:pass

    # setup gravitational system
    gs = GravSys()
    sun = Sun(gs,SUN_MASS, Vec(0,0), Vec(0,0),
              IMAGE,1,"yellow",has_orbit=False,rotation = 360)
    #sun.color("yellow")
    sun.penup()
