...
class VeApiWrapper(VeApiBase):
    ...
    def __sync_mode_wrapper(self, req, sync=False, timeout=None, sid=None, rid=None, once=False):
        if sync:
            return self._get_result(rid, timeout, self._send_unitmsg, (req, sid), {}, once)
        self._send_unitmsg(req, sid)

    def _registerReq(self, req: trade_api_struct.RegisterReq, sync=False, timeout=None):
