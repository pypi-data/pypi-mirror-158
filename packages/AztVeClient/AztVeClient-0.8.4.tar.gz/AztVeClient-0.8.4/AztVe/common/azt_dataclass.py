...
def debug_calc_time(func):
    def wrapper(*args, **kwargs):
        ...
        t2 = time.time()
        azt_logger.debug(f"{obj.__class__}.py2proto耗时：{(t2 - t1) * 1000:.4f}ms" if argl == 2 else
...
def aztdataclass(protocls,
    ...
    def func_proto2py(cls, proto):
        ...
        for normal_attr_name in normal_attrs:
            proto_attr_name = proto_attrs[normal_attr_name]
            if hasattr(proto, proto_attr_name):
                normal_atrr_cls = cls_attrs[normal_attr_name]
                if hasattr(normal_atrr_cls, "__proto2py__"):
                    params[normal_attr_name] = normal_atrr_cls.__proto2py__(getattr(proto, proto_attr_name))
                else:
        ...
        # t2 = time.time()
        # azt_logger.debug(f"{cls}.proto2py耗时：{(t2 - t1) * 1000:.4f}ms")
        return cls(**params)
    ...
    def func_py2proto(self):
        ...
        # t2 = time.time()
        # azt_logger.debug(f"{self.__class__}.py2proto耗时：{(t2 - t1) * 1000:.4f}ms")
        return proto
