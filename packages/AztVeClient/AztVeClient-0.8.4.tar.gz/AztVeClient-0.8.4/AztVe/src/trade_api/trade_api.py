...
from .ve_api_wrapper import *


class TradeApi(VeApiWrapper):
    __account = None

    # 初始化
    def Start(self, server_addr: str, spi=None, hb_times: int = 3, hb_interval: int = 5, poller_timeout: int = None):
        return self._start(server_addr, spi, hb_times, hb_interval, poller_timeout)

    # 注册账户
    def RegisterAccount(self, strategy_id: str, strategy_check_code: str, sync: bool = False, timeout: int = None):
        register_req = trade_api_struct.RegisterReq(
            strategy_id=strategy_id,
            strategy_check_code=strategy_check_code,
        )
        return self._registerReq(register_req, sync, timeout)
    ...
    def Logout(self):
        ...
        self._userLogoutReq(logout_req)
