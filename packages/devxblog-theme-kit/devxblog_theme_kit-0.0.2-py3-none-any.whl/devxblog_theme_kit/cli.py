...
def start_theme_dev():
    ...
    subprocess.call(['python', 'manage.py', 'runserver', '8000'])
...
class Api:
    ...
    def upload_theme_file(self, theme_id, fname, fpath, content, update_id=None, kind='template'):
        if update_id:
            ...
            status, body = self.make_request(
                ...
                {'theme': theme_id, 'filename': fpath, 'content': content, 'kind': kind}
            )
        print(f'status={status}')
        return body['id']
