...
class httptools():
    ...
    def downloadM3u8(url, dstfile, headers=None, data=None, proxies=None, maxRetryCount=5, nowRetryCount=0, minSize=1024, theadnum=32, maxFailRate=0.015):
        ...
        if len(tsFilePaths) == 0:
            raise Exception("需要下载的ts文件数量为" + str(len(m3u8TmpFileDir)))
        elif len(os.listdir(m3u8TmpFileDir)) == 0:
            raise Exception("下载成功的ts文件数量为" + str(len(m3u8TmpFileDir)))
        elif len(os.listdir(m3u8TmpFileDir)) / len(tsFilePaths) < 1 - maxFailRate:
            raise Exception("下载成功的ts文件数量为" + str(len(m3u8TmpFileDir)))
