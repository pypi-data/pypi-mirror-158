...
def main():
    testSql = 'insert into t1 select ? where not exists(select 1 from t1 where c1 = ?)'
    testDB = DBConn('sqlite', db='test.db')
    tp = ThreadPool(64)
    for idx in range(0, 9999):
        tp.run(testDB.run, (testSql, (idx, idx)))
    tp.wait()
    testDB.close()
...
class DBConn:
    ...
    def run(self, sql, param=None, wait=None):
        """ 执行点什么
        sql中的占位符可以统一使用“?”
        wait为True则会等待当前正在执行的sql，有bug，暂不处理，自用规避"""
        if wait is None and self.dbtype == 'sqlite' and not sql.lower().strip().startswith("select"):
            wait = True
