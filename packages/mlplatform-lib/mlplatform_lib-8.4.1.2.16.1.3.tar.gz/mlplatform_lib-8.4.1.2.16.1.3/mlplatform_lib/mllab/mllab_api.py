    ...
    def get_do(self, do_id: str, char_encoding="euc-kr"):
        if self.api_client.run_mode == RunMode.KUBERNETES:
            ...
            if do_info.scope_regex == None or do_info.scope_regex == "":
                ...
                unstructured_file_name = self.mllab_virtualization_api.download_unstructured(do_id=do_id)
                print(unstructured_file_name)
                file_name = do_id + os.path.splitext(unstructured_file_name)[1]
                if(file_name[-3:].lower()=="jpg" or file_name[-3:]=="PNG" or file_name[-3:].lower()=="gif" or file_name[-3:].lower()=="bmp":
                    file_name = file_name[:-3]+"png"
                else if(file_name[-4:].lower()=="jpeg"):
                    file_name = file_name[:-4]+"png"
                else if(file_name[-3:]!="png"):
                    file_name = file_name+".png"
                full_path = dir_name + "/" + file_name
                os.rename(dir_name + "/" + unstructured_file_name, full_path)
