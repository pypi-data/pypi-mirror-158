...
class LxTokenizer(ToolWrapper):
    ...
    def tokenize_raw_sentence(self, sentence):
        ...
        if not tokenizer_output.startswith("<s>"):
            ok = False
            LOG.debug("tokenizer removed <s> from begining of line")
        if not tokenizer_output.endswith("</s>"):
            ok = False
            LOG.debug("tokenizer removed </s> from end of line")
        if not ok:
            LOG.debug(f"tokenizer input: {tokenizer_input!r}")
            LOG.debug(f"tokenizer output: {tokenizer_output!r}")
