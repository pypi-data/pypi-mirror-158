...
class OIDCAuthBackend(BaseAuthBackend):
    ...
    def get_or_create_user(self, access_token, id_token, payload):
        ...
        try:
            ...
            raise SuspiciousOperation("Error: Email missing from keycloak roles")
        ...
        try:
            ...
            raise SuspiciousOperation("Error: sub missing from keycloak roles")
            #
            # messages.error(self.request,
        ...
        try:
            ...
            raise SuspiciousOperation("We cannot create your user account as a user account in this system already exists with the same email address.")
            # messages.error(
            #     self.request,
            #     _(
        ...
        prev_roles = set(Organizer.objects.filter(
            id__in=u.teams.filter(name="Administrators").values_list('organizer', flat=True)
        ...
        if r.status_code != 200:
            raise SuspiciousOperation("Error Gestor Eventos API")
            # messages.error(self.request,
            #                _("Error Gestor Eventos API"), )
            # return redirect(reverse("control:auth.login"))
        ...
        #         msg = f"Event {slug} from Gestor de Eventos UP does not exist on Pretix"
        #         raise SuspiciousOperation(msg)
        #
        #     try:
