...
from .utils import absolutify, add_state_and_nonce_to_session, import_from_settings
...
class OIDCAuthenticationCallbackView(View):
    ...
    def login_success(self):
        ...
        process_login(self.request, self.user, keep_logged_in=False)
    ...
    def get(self, request):
        ...
        if request.GET.get("error"):
            ...
            # TODO kecloak_auth should not be hardcoded!! - jpolonia
            backend = get_auth_backends()["keycloak_auth"]
            # u = backend.request_authenticate(request)
            self.user = backend.authenticate(**kwargs)
...
class OIDCAuthenticationRequestView(View):
    ...
    def get(self, request):
        ...
        reverse_url = self.get_settings(
            ...
            "plugins:pretix_oidc:oidc_authentication_callback",
        )
        ...
        params = {
            ...
            # "redirect_uri": build_absolute_uri(reverse_url),
            "state": state,
        }
