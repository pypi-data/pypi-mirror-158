...
class Binomial(Distribution):
    ...
    def plot_bar(self):
        ...
        plt.hist(self.data)
        plt.title('My Histogram of Data')
        plt.xlabel('data')
        plt.ylabel('count')      

        plt.bar(x = ['0', '1'], height = [(1 - self.p) * self.n, self.p * self.n])
        plt.title('Udacitys Bar Chart of Data')
        plt.xlabel('outcome')
        plt.ylabel('count')
    ...
    def pdf(self, k):
        ...
        return (np.math.factorial(self.n) * self.p**k * (1-self.p)**(self.n-k) / (np.math.factorial(self.n-k) * np.math.factorial(k)))      
