...
class Line:
    ...
    def __init__(self, srv: bytes, devices: list[str], max_ttd: int):
        ...
        self._keepalives_sent: dict[bytes, tuple[int, float]] = {}
        ...
        self._stat_keepalives_sent: int = 0
    ...
    def _update_lut(self, srv: bytes, device: serial.Serial, ttd: int):
        ...
        # If no entry found, add it to LUT
        new_entry = LUTEntry(
            srv,
            device,
            ttd,
    ...
    def _keepalive_periodic(self):
        ...
        for entry in self._lut:
            if entry.srv in self._keepalives_sent.keys():
                last_keepalive_sent_to = self._keepalives_sent[entry.srv][1]
            else:
        ...
        if dst_srv in self._keepalives_sent:
            new_counter = self._keepalives_sent[dst_srv][0] + 1
        else:
            new_counter = time.time()
        self._keepalives_sent[dst_srv] = (new_counter, self._last_keepalive)

    def _buffer_rot(self):
        for dev in self._devices:
            if len(self._buffers[dev]) > 0:  # this ensures that last_bytes is sent as well
                if (time.time() - self._buffers_last_new_bytes[dev]) > 2.0:  # clean after 2 sec of not receiving anything
    ...
    def _pop_one_frame_from_buffer(self, dev: serial.Serial) -> Optional[bytes]:
        ...
        for i in range(len(buffer) - frame_header):  # <- leave room for forward seeking
            if buffer[i] != 0x69:
                continue
