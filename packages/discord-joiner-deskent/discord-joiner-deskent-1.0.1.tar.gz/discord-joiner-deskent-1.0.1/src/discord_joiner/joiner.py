...
class DiscordJoiner:
    """
    Класс принимает данные пользователя и добавляет его через
    прокси (если указана) на сервер дискорда по ссылке.
    ...
    def __init__(
        ...
        self.__logger = get_logger(log_level)

    async def _update_proxy(self):
        """Добавляет прокси в параметры сессии"""
    ...
    async def __get_finger_print(self) -> bool:
        """Отправляет запрос и добавляет полученные заголовки к сессии"""
    ...
    async def __authorization(self) -> bool:
        """Авторизиуется в дискорде, получает и добавляет куки к заголовкам."""
    ...
    async def __update_invite_id(self) -> bool:
        """Сохраняет id приглашения в атрибут класса из ссылки приглашения"""
    ...
    async def __join(self) -> bool:
        """Отправляет запрос на добавления пользователя на сервер дискорда"""
    ...
    async def __get_xsuperproperties(self) -> None:
        """Возвращает заголовок xsuperproperties"""
    ...
    async def join(self) -> dict:
        """Получает все необходимые данные для добавления пользователя на сервер дискорда"""
