...
class DeferredTaskSubmitter(object):
    ...
    def _set_task_meta(self, context, task, request, deferral=None):
        ...
        # set parent for inline pipelines to first non-inline pipeline in the ancestor chain
        if context.pipeline_state.pipeline.inline:
            request.meta['inline_parent_pipeline_id'] = context.get_inline_parent_pipeline_id() or context.get_pipeline_id()
            request.meta['inline_parent_pipeline_address'] = context.get_inline_parent_pipeline_address() or context.get_pipeline_id()
