...
def rw_redis(key, redis_name=None, redis_host="127.0.0.1",  redis_port=6379, redis_db=9, json_type=False, outer_time=REDIS_TIME_OUTER):
    ...
    def outer(f):
        def inner(*args, **kwargs):
            nonlocal key
            if not key:
                key = kwargs['redis_key']
            redis_data = str_read(key=key, redis_name=redis_name, redis_host=redis_host, redis_port=redis_port, redis_db=redis_db)
            if redis_data:
                if json_type:
                    try:
            ...
            if data:
                ...
                redis_flg = str_save(data=data, key=key, redis_name=redis_name, redis_host=redis_host,
