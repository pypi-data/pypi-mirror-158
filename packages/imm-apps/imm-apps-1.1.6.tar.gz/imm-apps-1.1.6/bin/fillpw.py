...
def prepare_document(form_type: str, name: str, wait_time: int):
    ...
    src = Path(DATADIR + f"/pdf/{form_type}.pdf")
    dst = Path(".").cwd() / f"{name}.pdf"
    shutil.copy2(src, dst)

    # open file with acrobat reader
    print(str(config_win.ACROBAT_READER_PATH) + " " + str(dst))
...
def main():
    ...
    in_file = str(Path(args.json_name + ".json"))
    verbose = args.verbose
    out_file = str(Path(".").cwd() / f"{args.output}.pdf")

    check_env()
    prepare_document(form_name, out_file, args.wait)

    # skip 3 times for first field
    skip = Skip(times=3)
    skip.fill()
