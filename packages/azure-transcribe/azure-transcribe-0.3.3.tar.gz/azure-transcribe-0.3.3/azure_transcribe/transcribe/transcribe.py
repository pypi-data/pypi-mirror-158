...
class AzureTranscribe:
    ...
    def create_dialog_transcription(self, sas_url: str, language: str = 'en-US') -> str:
        url = urljoin(self.base_url, 'transcriptions')
        payload = {
            "contentUrls": [
                sas_url
    ...
    def prepare_dialog(self, data: List[Dict[str, List[Dict[str, Union[str, List]]]]]) -> List[Dict[str, str]]:
    ...
    def get_dialog_result(self, files_url: str) -> Dict[str, Union[str, List[Dict[str, str]]]]:
        response = requests.get(files_url, headers=self.headers)
        file_url = self.get_transcription_url(response.json())
        if file_url:
            response = requests.get(file_url)
            ...
            if bool(full_transcript):
                full_transcript = full_transcript[0]['display']
                if bool(dialog_transcript):
                    dialog_transcript = self.prepare_dialog(dialog_transcript)
                else:
                    dialog_transcript = []
                return {
                    'full_transcript': full_transcript[0]['display'],
