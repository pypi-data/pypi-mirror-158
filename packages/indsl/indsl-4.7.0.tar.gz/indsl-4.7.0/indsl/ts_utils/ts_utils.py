...
def fill_gaps(
    ...
    Raises:
        ...
        UserValueError: Unable to convert granularity to time resolution
        UserValueError: granularity can not be 0
        UserValueError: Unable to convert ffill_resolution to time resolution
        UserValueError: ffill_resolution can not be 0
        UserValueError: Unable to convert interpolate_resolution to time resolution
    ...
    if method not in ("backfill", "bfill", "pad", "ffill"):
        warnings.warn("Method should be in ('backfill', 'bfill', 'pad', 'ffill'). Set to ffill.")
        method = "ffill"
    ...
    if granularity is not None:
        try:
            ...
            raise UserValueError(f"Unable to convert {granularity} to time resolution")
    else:
        # If granularity is not given as input. Granularity will be inferred from the data
        granularity_timedelta = pd.Timedelta(data.index.inferred_freq)
    if granularity_timedelta == pd.Timedelta("0T"):
        raise UserValueError("granularity can not be 0")

    if ffill_resolution is not None:
        try:
            ffill_resolution_timedelta = pd.Timedelta(ffill_resolution)
        except ValueError:
            raise UserValueError(f"Unable to convert {ffill_resolution} to time resolution")
        if ffill_resolution_timedelta == pd.Timedelta("0T"):
            raise UserValueError("ffill_resolution can not be 0")

    if interpolate_resolution is not None:
        try:
            interpolate_resolution_timedelta = pd.Timedelta(interpolate_resolution)
        except ValueError:
            raise UserValueError(f"Unable to convert {interpolate_resolution} to time resolution")
        if interpolate_resolution_timedelta == pd.Timedelta("0T"):
    ...
    if ffill_resolution:
        small_gap = granularity_timedelta / ffill_resolution_timedelta
    if interpolate_resolution is not None:
        mask_gap = granularity_timedelta / interpolate_resolution_timedelta
