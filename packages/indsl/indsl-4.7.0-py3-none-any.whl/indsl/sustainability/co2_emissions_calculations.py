...
def emissions_factor_combustor(
    emissions_factor: float = None, heating_value: float = None, carbon_content: float = None
):
...
# Rate of emissions
def rate_of_emissions(data: pd.Series, emissions_factor: float):
    r"""Rate of emissions
...
def cumulative_co2_production(rate_of_emissions: pd.Series, start_date: pd.Timestamp = None):
    ...
    data = resample_to_granularity(rate_of_emissions.loc[start_date:], granularity="1h")

    return trapezoidal_integration(data, granularity="1h")


def cumulative_co2_cost(
    data: pd.Series, co2_cost_factor: float, emissions_factor: float, start_date: pd.Timestamp = None
):
