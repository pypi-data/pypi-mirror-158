...
class TorchHttpClient(TorchClientInterFace, ABC):
    ...
    def get_root_span(self, pipeline_run_id: int):
        """
            Description:
        ...
        """
        url = f'{self._pipeline_api_base}/pipelines/runs/{pipeline_run_id}/spans/root-span'
        response = self._get(url)
        response['span']['client'] = self
        return Span(**response['span'])
    ...
    def _post(self, url, payload=None, as_json=True):
        ...
        while retry:
            ...
            if 500 <= response.status_code <= 511 and retry_count < max_retry:
                retry = True
                self.logger.info(
                    f" Retrying POST call because of error code: {response.status_code}"
                )
            else:
                retry = False
    ...
    def _put(self, url, payload=None, as_json=True):
        ...
        while retry:
            ...
            if 500 <= response.status_code <= 511 and retry_count < max_retry:
                retry = True
                self.logger.info(
                    f" Retrying PUT call because of error code: {response.status_code}"
                )
            else:
                retry = False
    ...
    def _get(self, url, params=None, as_json=True):
        ...
        while retry:
            ...
            if 500 <= response.status_code <= 511 and retry_count < max_retry:
                retry = True
                self.logger.info(
                    f" Retrying GET call because of error code: {response.status_code}"
                )
            else:
                retry = False
    ...
    def _put_form_data(self, url, payload=None, as_json=True):
        ...
        while retry:
            ...
            if 500 <= response.status_code <= 511 and retry_count < max_retry:
                retry = True
                self.logger.info(
                    f" Retrying POSPUTT call because of error code: {response.status_code}"
                )
            else:
                retry = False
    ...
    def _delete(self, url, params=None, as_json=True):
        ...
        while retry:
            ...
            if 500 <= response.status_code <= 511 and retry_count < max_retry:
                retry = True
                self.logger.info(
                    f" Retrying DELETE call because of error code: {response.status_code}"
                )
            else:
                retry = False
        return response
