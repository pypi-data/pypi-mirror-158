...
class WorkflowLauncher(object):
    ...
    def _get_schema_demo_data(self, schema: Dict):
        """Extract demo_data values from parameters in a schema."""
        demo_data = (
            self._get_schema_demo_data_for_fragment(
    ...
    def _get_schema_demo_data_for_fragment(
        self, fragment: Dict
    ) -> Dict[str, Union[str, int, float, bool]]:
        """Extract demo_data from parameters in a section of schema."""
        demo_data = {}
        for key, param in fragment.items():
            demo_value = param.get("demo_data")
            if demo_value is None:
                continue
            demo_data[key] = demo_value
        return demo_data
    ...
    def get_path(self, path: str) -> PathResponse:
        ...
        if path.lower() == 'root':
            if self.platform in ["win32"]:
                return self._get_windows_root_path_response()
            path = '/'
    ...
    def get_directory(
        ...
        if path.lower() == 'root':
            if self.platform in ["win32"]:
                resp = self._get_windows_root_path_response()
                items = self._get_windows_root_path_contents()
    ...
    def _get_windows_root_path_response(self):
        """Handle the special case of 'root' path on Windows."""
        resp = PathResponse(
            name='Root', path='root', updated=None, exists=True,
            error=None, isdir=True)
        return resp

    def _get_windows_root_path_contents(self):
    ...
    def _get_wslpath_params(
        ...
        for name, value in params.items():
            param_schema = self._get_schema_for_param(schema, name)
            fmt = param_schema.get('format', None)
            if fmt in ['path', 'file-path', 'directory-path']:
