...
class KorFinancials(Financials, Dart):
    ...
    def _get_latest_report_info(self, year: int) -> datetime:
        ...
        report_list = pd.DataFrame(data['list'])  # what's this for?
    ...
    def _get_raw_financials(self) -> tuple[pd.DataFrame, str]:
        ...
        today = datetime.now()
        year_now = today.year
        month_now = today.month  # what's this for?
    ...
    def open_report(self) -> None:
        ...
        raw_financial, period = self._get_raw_financials()  # what's period for?
