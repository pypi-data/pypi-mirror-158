...
def load_zipped(filezip):
    ...
    for elm in zfile.namelist():
        ...
        if elm.endswith(('.txt', '.htm', '.html', '.xml', '.xhtml', '.srt', '.ass')):  # noqa
            break
    ...
    if not text:
        LOGGER.warning("The first txt/html/htm/xml/xhtml/srt/ass/ file in the zip file does not contain any text. Return empty ('')")
    return text
