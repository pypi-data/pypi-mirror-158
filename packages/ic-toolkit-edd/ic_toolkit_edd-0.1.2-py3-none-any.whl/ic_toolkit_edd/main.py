        ...
        for i in progress:
            ...
            try:
                ...
                if e.response['Error']['Code'] == "404":
                    ...
                    typer.echo("\n\n  " + typer.style(e, fg=typer.colors.WHITE, bg=typer.colors.RED))
                    return
        typer.echo(typer.style(" SUCESSO", fg=typer.colors.GREEN, bold=True))
...
def upload():
    ...
    list_success = []
    filename_list = [f for f in os.listdir('./') if os.path.isfile("./" + f)]
    with typer.progressbar(filename_list) as progress:
    ...
    if len(list_error) > 0:
        typer.echo(f"\n  " + typer.style(f"ERRO", fg=typer.colors.WHITE, bg=typer.colors.RED) + f" ARQUIVOS COM EXTENSÃOS INVALIDAS: {list_error}\n")
    if len(list_success)> 0:
        typer.echo("  " + typer.style(f"SUCESSO", fg=typer.colors.WHITE, bg=typer.colors.GREEN) + f" FOI FEITO O UPLOAD DE {len(list_success)} ARQUIVOS: {list_success}")