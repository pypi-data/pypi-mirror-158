...
Usage:
  py -m rob.email <target> <content>...
  py -m rob.email add <name> <email>
  py -m rob.email list
...
_ARGS = docopt(_DOC, argv=sys.argv[1:])

_CONTACT_FILE = Path(appdirs.user_data_dir()) / "rob" / "email" / "contacts.db"
