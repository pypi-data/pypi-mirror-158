...
def has_suffix(payload):
    ...
    if len(payload) < 4:
        return False
    log.debug("buffer ", payload[-4:], " = ", SUFFIX_BIN)
...
class XenonDevice(object):
    ...
    def _send_receive(self, payload, minresponse=28, getresponse=True):
            ...
            try:
                ...
                if getresponse is True:
                    ...
                    if self.retry and len(data) <= minresponse:
                        log.debug("received null payload (",data,"), fetch new one")
                        time.sleep(0.1)
                        data = self.socket.recv(1024)  # try to fetch new payload
                    success = True
                    log.debug("received data= HEX:", binascii.hexlify(data))
        ...
        try:
            ...
            log.debug("raw unpacked message = ", msg)
    ...
    def _decode_payload(self, payload):
        log.debug("decode payload=", payload)
        ...
        if payload.startswith(PROTOCOL_VERSION_BYTES_31):
            ...
            if self.dev_type != "default" or payload.startswith(
                ...
                log.debug("removing 3.3=", payload)
            try:
                log.debug("decrypting=", payload)
                payload = cipher.decrypt(payload, False)
            except:
                log.debug("incomplete payload=", payload)
                return None

            log.debug("decrypted 3.3 payload=", payload)
            ...
            if not self.disabledetect and "data unvalid" in payload:
                ...
                log.debug(
                    "'data unvalid' error detected: switching to dev_type ",
                    self.dev_type
                )
                return None
        elif not payload.startswith(b"{"):
            log.debug("Unexpected payload=", payload)
            return error_json(ERR_PAYLOAD, payload)

        if not isinstance(payload, str):
            payload = payload.decode()
        log.debug("decoded results=", payload)
        try:
    ...
    def generate_payload(self, command, data=None, gwId=None, devId=None, uid=None):
        ...
        log.debug("building payload=", payload)
        ...
        log.debug("payload generated= HEX:", binascii.hexlify(buffer))
        return buffer
...
class Device(XenonDevice):
    ...
    def status(self):
        ...
        log.debug("status() received data=", data)
        # Error handling
    ...
    def set_status(self, on, switch=1):
        ...
        data = self._send_receive(payload)
        log.debug("set_status received data=", data)
    ...
    def product(self):
        ...
        log.debug("product received data=", data)
        return data
    ...
    def heartbeat(self):
        ...
        data = self._send_receive(payload, 0)
        log.debug("heartbeat received data=", data)
        return data
    ...
    def updatedps(self, index=[1]):
        ...
        data = self._send_receive(payload, 0)
        log.debug("updatedps received data=", data)
        return data
    ...
    def set_timer(self, num_secs, dps_id=0):
        ...
        data = self._send_receive(payload)
        log.debug("set_timer received data=", data)
        return data
...
def deviceScan(verbose=False, maxretry=None, color=True, poll=True):
    ...
    while (count + counts) <= maxretry:
        ...
        try:
            ...
            result = json.loads(result)
            log.debug("Received valid UDP packet: ", result)
            ...
            result = {"ip": ip}
            note = "Unknown"
            log.debug("Invalid UDP Packet: ", result)
...
class Cloud(object):
    ...
    def _getuid(self, deviceid=None):
        ...
        if not response_dict['success']:
            log.debug("Error from Tuya Cloud: ", response_dict['msg'])
            return None
