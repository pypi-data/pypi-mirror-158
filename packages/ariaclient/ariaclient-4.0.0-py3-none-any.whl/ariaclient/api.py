...
class AriaService:
    ...
    def _process_aria_message(self, res, attribute=None):
        try:
            ...
            if not data.get(attribute, None) and not data.get('detail', None):
                raise AriaError('Unrecognised server response', res)
            elif data['detail'].get(attribute, None):
                return data['detail'][attribute]
            else:
                raise AriaError('Unrecognised server response', res)
    ...
    def _authenticate(self, keyType, key):
        ...
        return self._process_aria_message(res, 'token')
    ...
    def send_message(self, channel, raw_message, **kwargs):
        ...
        if 'on_behalf' in kwargs and kwargs['on_behalf'] is not None:
            message['on_behalf'] = kwargs['on_behalf']
    ...
    def set_text(message, text, data=None):
        if isinstance(text, SymphonyMessageML):
            message['payload']['message'] = text.__str__()
        else:
