...
class SOPA(torch.optim.Optimizer):
    ...
    def update_lr(self, decay_factor=None, coef_decay_factor=None):
        ...
        if coef_decay_factor != None:
            self.loss_fn.update_coef(coef_decay_factor) # for moving average
            print ('Reducing loss coefficient (moving average) to %.5f @ T=%s!!' %(self.loss_fn.get_coef, self.steps))
