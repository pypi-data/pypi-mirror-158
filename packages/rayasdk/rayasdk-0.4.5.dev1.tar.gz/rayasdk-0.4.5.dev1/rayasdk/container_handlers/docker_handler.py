...
def download_simulator():
    ...
    try:
        ...
        pass
    print(f'Downloading version {GARYSIM_VERSION}...')
    SIMS_HOME.mkdir(parents=True, exist_ok=True)
    urllib.request.urlretrieve(GARYSIM_URL, str(SIM_TARPATH), MyProgressBar())
    log_info(f'Checking downloaded file...')
    try:
        sha256 = hashlib.sha256(open(SIM_TARPATH,'rb').read()).hexdigest()
    except FileNotFoundError:
...
def run_simulator_bridge():
    ...
    launch_command(
            ...
            suffix='simbridge',
            working_dir='/root', 
            ports=['10000:10000', '8789:8789', '8000:8000']
        )
    p.kill()
    p.wait()
...
def check_container():
    ...
    log_info(f'Downloading image (only once)...')
    urllib.request.urlretrieve(RAYAENV_DOCKER_URL, str(RAYAENV_DOCKER_IMGPATH),
                               MyProgressBar())
    log_info(f'Checking downloaded file...')
    try:
        sha256 = hashlib.sha256(open(RAYAENV_DOCKER_IMGPATH,'rb').read()).hexdigest()
    except FileNotFoundError:
        log_error('Download error, try again.')
        return False
    ...
    if ret != 0:
        ...
        return False
    log_info('Image create correctly, you can now execute \'rayasdk\'')
