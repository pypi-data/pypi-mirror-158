...
def getProjectVariables(access_token, project_id, project_name):
    ...
    VARIABLES_DICT = {}
    variables_json = requests.get("{}/api/v4/projects/{}/variables".format(GITLAB_URL, project_id), headers={"Authorization": "Bearer {}".format(access_token)}).json()
    ...
    for req_var in REQUIRED_VARIABLES:
        for gl_var in variables_json:
            if gl_var['key'] == req_var:
                variable_template = {
                    gl_var['key']: gl_var['value']
                  }
...
def cliConfigCreate(access_token):
    print("[GRDPCLI]: Getting and generation projects configuration file, please wait ...")
    PROJECTS_LIST = getAllProjects(access_token)
    config_template = {}
    id = 0
    for project in PROJECTS_LIST:
        project_data = getProjectVariables(access_token, project['id'], project['name'])
...
class callbackServer(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if os.path.exists(os.path.join(GRDPCLI_HOME_DIR,GRDPCLI_PROJECT_JSON)) and os.path.exists(os.path.join(GRDPCLI_HOME_DIR,GRDPCLI_CONFIG_JSON)):
            print("[GRDPCLI]: Projects configuration file generated successfully")
            raise SystemExit
        else:
