...
def exec_js(js_handle, method, *args):
    ...
    if any([type(js_handle) == execjs._external_runtime.ExternalRuntime.Context, type(js_handle) != str]):
...
if __name__ == '__main__':
    res = exec_js("add.js", "add", 1, 2)
    print("test:", res)

    with open('add.js', 'r', encoding='utf-8') as f:
        js_content = f.read()
    ctx = execjs.compile(js_content)
    res = exec_js(ctx, "add", 1, 2)
    print("test:", res)
