...
class Dashboard(jenkins_jobs.modules.base.Base):
    ...
    def root_xml(self, data):
        root = XML.Element('hudson.plugins.view.dashboard.Dashboard')
        root.set('plugin', 'dashboard-view')
        ...
        mapping = [
            ...
            ('include-std-job-list', 'includeStdJobList', True),
            ('hide-jenkins-panels', 'hideJenkinsPanels', True),
            ('left-portlet-width', 'leftPortletWidth', '50%'),
            ('right-portlet-width', 'rightPortletWidth', '50%'),
            ('right-portlet-width', 'rightPortletWidth', '50%'),
        ]

        xl_portlets = XML.SubElement(root, 'leftPortlets')
        sxl_portlets = XML.SubElement(xl_portlets, 'hudson.plugins.view.dashboard.test.TestStatisticsChart')
        XML.SubElement(sxl_portlets, 'name').text = 'Test Trend Chart'
        XML.SubElement(sxl_portlets, 'id').text = 'dashboard_statistics_chart_portlet_{}'.format(randint(12345, 999999))


        xr_portlets = XML.SubElement(root, 'rightPortlets')
        sxr_portlets = XML.SubElement(xr_portlets, 'hudson.plugins.view.dashboard.test.TestTrendChart')
        XML.SubElement(sxr_portlets, 'name').text = 'Test Trend Chart'
        XML.SubElement(sxr_portlets, 'id').text = 'dashboard_trend_chart_portlet_{}'.format(randint(12345, 999999))
        XML.SubElement(sxr_portlets, 'graphWidth').text = '300'
        XML.SubElement(sxr_portlets, 'graphHeight').text = '220'
        XML.SubElement(sxr_portlets, 'displayStatus').text = 'ALL'

        xt_portlets = XML.SubElement(root, 'topPortlets')
        sxt_portlets = XML.SubElement(xt_portlets, 'hudson.plugins.view.dashboard.test.TestStatisticsPortlet')
        XML.SubElement(sxt_portlets, 'name').text = 'Test Statistics Grid'
        XML.SubElement(sxt_portlets, 'id').text = 'dashboard_grid_portlet_{}'.format(randint(12345, 999999))
        XML.SubElement(sxt_portlets, 'skippedColor').text = 'FDB813'
        XML.SubElement(sxt_portlets, 'successColor').text = '71E66D'
        XML.SubElement(sxt_portlets, 'failureColor').text = 'E86850'
        XML.SubElement(root, 'bottomPortlets')
