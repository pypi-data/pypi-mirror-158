...
class CORN(Algo):
    ...
    def step_slow(self, x: pd.Series, last_b: pd.Series, history: pd.DataFrame) -> np.ndarray:
    ...
    def step_fast(self, x: pd.Series, last_b: pd.Series, history: pd.DataFrame) -> np.ndarray:
        # iterate time
        self.t += 1
