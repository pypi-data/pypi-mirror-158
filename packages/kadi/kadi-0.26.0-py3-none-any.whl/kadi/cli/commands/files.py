...
@files.command()
def check():
    """Check the database and storage for inconsistencies.

    Each file, upload and temporary file from the database as well as the configured
    local storage path (STORAGE_PATH) will be checked for inconsistencies.
    ...
    # Check the configured local storage path.
    storage_path = current_app.config["STORAGE_PATH"]
    ...
    if inconsistencies == 0:
        ...
        if click.confirm(
            ...
            " inconsistent data and/or database objects?"
        ):
