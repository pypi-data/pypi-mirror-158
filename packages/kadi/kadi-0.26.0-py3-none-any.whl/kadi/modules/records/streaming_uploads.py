...
def validate_storage_type(streaming_context):
    """Checks if the used storage type is valid.

    :param streaming_context: The :class:`.StreamingContext`.
    """

    def _validate_storage_type(target):
        storage_type = target.to_string()
        storage_type = StorageSchema(only=["storage_type"]).load_or_400(
...
def validate_filename(streaming_context):
    ...
    def _validate_filename(target):
        ...
        if response is not None:
            ...
            # Store the replaced file so it can be referenced again later, if necessary.
...
def create_streaming_parser(headers, streaming_context):
    ...
    _register_target(
        ...
        KadiValueTarget(
            on_finish=check_field(
                validate_storage_type(streaming_context), streaming_context
            )
        ),
        required=False,
    )
    ...
    _register_target(
        ...
        KadiValueTarget(
            on_finish=check_field(
                validate_mimetype(streaming_context), streaming_context
            )
        ),
        required=False,
    )
    _register_target(
        ...
        KadiValueTarget(
            on_finish=check_field(
                validate_checksum(streaming_context), streaming_context
            )
        ),
        required=False,
    )
