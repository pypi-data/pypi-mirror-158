...
class Upload(object):
    ...
    def __init__(self, client_id, client_secret, proxies={}, verify=True):
        ...
        self._headers = {
            ...
            "User-Agent": "Python API 1.92 / {local_version}".format(local_version=platform.python_version())
        }
    ...
    def upload_csv(self, filepath):
        response = self._requests(
            method='post',
            url='{base_url}/connections/user/datasets/upload'.format(
        ...
        response = self._requests(
            method='get',
            url='{base_url}/connections/user/dataset/{upload_id}/setup?'.format(
                base_url=self._BASE_URL,
                upload_id=upload_id
            ),
            headers=self._headers
