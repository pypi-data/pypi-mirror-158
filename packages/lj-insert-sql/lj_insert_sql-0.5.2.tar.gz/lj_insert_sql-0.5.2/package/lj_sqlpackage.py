...
class PackageDeps(Base):
    ...
    UNIQUE_FIELDS = ('dependency_package_id', 'package_type', 'package_version')
...
class SqlTool(object):
    def __init__(self, host, port, user, pwd, db, charset, package_type):
    ...
    def package_deps(self, deps: PackageDeps):
        try:
            for result in deps.requirement:
                ...
                deps_info = PackageInfo(result['dependency_package_name'], self.package_type)
