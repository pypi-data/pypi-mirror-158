...
class CloudPodsVersionManager(CloudPodsManager):
	...
	def _get_max_version_for_pod_from_platform(B,pod_name,auth_headers):
		...
		if not B._check_response(A,message=D,raise_error=_C):return
	...
	def _add_state_to_cloud_pods_store(A,extract_assets=_A,services=_B):
		...
		B=B or A.pods_api.config_context.get_services_from_config();F=get_state_zip_from_instance(get_content=_C,services=B);D=ServiceStateMarshaller.unmarshall(F,raw_bytes=_C)
		for (C,G) in D.state.items():
			for (H,I) in G.backends.items():A.pods_api.create_state_file_from_fs(file_name=H,service=C.service,region=C.region,root=_G,account_id=C.account_id,serialization=Serialization.MAIN,object=I)
		if extract_assets:
	...
	def _pull_versions(A,auth_headers,required_versions):
		...
		if not A._check_response(D,message=F,raise_error=_C):return
	...
	def push(A,comment=_B,services=_B):
		D=comment;A.pods_api.set_pod_context(A.pod_name);A._add_state_to_cloud_pods_store(extract_assets=_C,services=services);B:0;C=A.pods_api.get_head().version_number
	...
	def inject(A,version,reset_state=_A,merge=_A):
		...
		if not A.pods_api.config_context.pod_exists_locally(A.pod_name):LOG.debug(f"Pod {A.pod_name} does not exist locally. Requesting state from remote..");C=A._download_version_product(version=B).get(_N)
			...
			if B==-1:B=A.pods_api.get_max_version_no(require_state_archive=_C)
	...
	def register_remote(A,pod_name,ci_pod=_A):
		A.pods_api.set_pod_context(A.pod_name);B=A.pods_api.get_max_version_no(require_state_archive=_C)
		...
		D=json.loads(E.content);I={_E:D.get(_E),_K:D.get(_D)};J=D.get(_F);A.pods_api.upload_version_and_product_space(presigned_urls=J);remote_utils.create_remote_info_file(remote_info=I,config_context=A.pods_api.config_context);return _C
	...
	def list_pods(A,fetch_remote):
		...
		if fetch_remote:
			E=get_auth_headers();F=A.create_platform_url('');D=safe_requests.get(F,headers=E);A._check_response(D,message='Error fetching list of pods from API (status <status_code>)',raise_error=_C);G=json.loads(D.content)
...
def push_state(pod_name,pre_config=_B,comment=_B,register=_A,services=_B,**F):
	C=services;B=pod_name
	if C is _B:C=[]
	A=get_pods_manager(pods_name=B)
	if not A.pods_api.config_context.is_initialized():A.init()
	A.pods_api.set_pod_context(pod_name=B);A.push(comment=comment,services=C);D=_C
