...
class PodsApi:
	...
	def init(A,pod_name=_A):
		...
		mkdir(A.config_context.get_pod_root_dir());mkdir(A.config_context.get_versions_path());save_file(A.config_context.get_known_ver_path(),'');A.config_context.set_pod_context(C);F=random_hash();G=random_hash();E=Revision(hash_ref=F,parent_ptr=NIL_PTR,creator=A.config_context.get_context_user(),rid=short_uid(),revision_number=0,state_files=set());B=Version(hash_ref=G,parent_ptr=NIL_PTR,creator=A.config_context.get_context_user(),comment='Initial version',outgoing_revision_ptrs={F},incoming_revision_ptr=_A,state_files=set(),version_number=Version.DEFAULT_INITIAL_VERSION_NUMBER);B.revisions.append(E);A.object_storage.upsert_objects(B)
	...
	def upload_version_and_product_space(A,presigned_urls):
		...
		def B(pre_signed_url,zip_data_content):
			...
			return A
		G=C.get('version_space_url');D=A.create_version_space_archive()
		with open(D,_D)as H:B(G,H.read())
		I=C.get('meta_state_urls');rm_rf(D)
		for (E,F) in I.items():
