...
def multi_accounts_enabled():return api_key_configured()and localstack_config.MULTI_ACCOUNTS
...
@hooks.on_infra_start(should_load=multi_accounts_enabled,priority=1000)
def patch_moto_backend_dict():from localstack_ext.aws.accounts import _patch_moto_backend_dict as A;A()
@hooks.on_infra_start(should_load=multi_accounts_enabled,priority=1000)
def patch_account_id_resolver():from localstack_ext.aws.accounts import _patch_account_id_resolver as A;A()
@hooks.on_infra_start(should_load=multi_accounts_enabled,priority=900)
...
def configure_pro_container(container):
	...
	if localstack_config.is_env_true('AZURE'):A.ports.add(config_ext.PORT_AZURE)
