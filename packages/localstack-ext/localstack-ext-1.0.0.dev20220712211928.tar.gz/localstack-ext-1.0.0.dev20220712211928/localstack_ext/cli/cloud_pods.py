...
@click.option(_B,_C,help=_F,cls=required_if_not_cached(_E))
@click.option('-l',_I,help='Delete only the local Cloud Pod, leaving the remote copy intact',is_flag=_D,default=_A)
@publish_invocation
...
@click.option(_I,'-l',default=_A,is_flag=_D,help='Create the Cloud Pod version only locally, without pushing to remote')
...
@click.option(_L,is_flag=_D,default=_A,help=_M)
@click.option('-v',_K,default='-1',type=int,help='Version to inject (most recent one by default).')
@click.option(_B,_C,help='Name of the cloud pod.',cls=required_if_not_cached(_E))
@publish_invocation
...
@click.option('--fetch',default=_A,is_flag=_D,help='Only fetch the Cloud Pod from the remote platform.')
@click.option(_L,is_flag=_D,default=_A,help=_M)
@click.option(_B,_C,help='Name of the cloud pod',cls=required_if_not_cached(_E))
@pod.command(name='pull',help='Incorporate the state of a Cloud Pod into the application runtime.')
@publish_invocation
def cmd_pod_pull(name,fetch,merge):from localstack_ext.bootstrap import pods_client;pods_client.pull_state(pod_name=name,inject=not fetch,merge=merge)
@pod.command(name='list',help='List all available Cloud Pods.')
@click.option(_I,'-l',help='List also locally available Cloud Pods.',is_flag=_D,default=_A)
@publish_invocation
