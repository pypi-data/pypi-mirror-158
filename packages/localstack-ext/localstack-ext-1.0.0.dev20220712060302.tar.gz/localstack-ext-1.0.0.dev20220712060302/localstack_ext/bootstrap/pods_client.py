...
class CloudPodsManager(metaclass=ABCMeta):
	...
	@abstractmethod
	def pull(self,inject=_C,merge=_C):...
	@abstractmethod
...
class CloudPodsVersionManager(CloudPodsManager):
	...
	def push_overwrite(A,version,comment=_A,services=_A):
		...
		return _B
	def pull(A,inject=_C,merge=_C):
		E=get_auth_headers();B=0
		if A.pod_name in A.pods_api.list_locally_available_pods(only_local=_B):A.pods_api.set_pod_context(A.pod_name);B=A.pods_api.get_max_version_no()
	...
	def _inject_from_remote(B,version,retain=_C):
		...
		H=D.get(_L);E=new_tmp_file();download(H,E);F(meta_archives={C:E},state_archives={C:A},config_context=B.pods_api.config_context);return _B
	@staticmethod
	...
	def inject(A,version=-1,merge=_C):
		...
		B=A.pods_api.config_context.get_version_state_archive(C)
		...
		else:reset_local_state(reset_persistence=_B)
		A.deploy_pod_into_instance(B);return _B
	...
	def register_remote(A,ci_pod=_A):
		...
		D=json.loads(E.content);I={_E:D.get(_E),_J:D.get(_D)};J=D.get(_G);A.pods_api.upload_version_and_product_space(presigned_urls=J);remote_utils.create_remote_info_file(remote_info=I,config_context=A.pods_api.config_context);return _B
	...
	def list_pods(A,only_local):
		if only_local:return A.pods_api.list_locally_available_pods(only_local=_B)
		C=A.pods_api.list_locally_available_pods();E=get_auth_headers();F=A.create_platform_url('');D=safe_requests.get(F,headers=E);A._check_response(D,message='Error fetching list of pods from API (status <status_code>)',raise_error=_B);G=json.loads(D.content)
...
def push_state(pod_name,comment=_A,register=_C,services=_A,**E):
	C=pod_name;B=services
	if B is _A:B=[]
	A=get_pods_manager(pods_name=C)
	if not A.pods_api.config_context.is_initialized():A.init()
	A.pods_api.set_pod_context(pod_name=C);A.push(comment=comment,services=B);D=_B
	if register:D&=A.register_remote()
	return D
def get_pods_endpoint():A=config.get_edge_url();return f"{A}{API_PATH_PODS}"
def pull_state(pod_name,inject=_C,merge=_C,**C):
	A=pod_name
