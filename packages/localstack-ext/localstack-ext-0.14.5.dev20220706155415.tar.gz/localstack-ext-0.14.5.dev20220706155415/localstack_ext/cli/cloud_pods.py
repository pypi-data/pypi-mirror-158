...
@click.option(_B,_C,help=_D,cls=required_if_not_cached(_E))
@click.option('-s',_I,help='List of services to push in the pods. It pushes all, if not specified',multiple=_A,default=[])
...
@click.option(_L,is_flag=_A,default=_F,help='For each service in the application state, its backend is merged with the backend specified by the given pod and version, or added if missing.')
...
@click.option('--inject/--no-inject',default=_A,help='Whether the latest version of the pulled pod should be injected')
@click.option(_L,is_flag=_A,default=_F,help='When injecting a pod version, it merges its state with the one of the locally running application')
@click.option(_B,_C,help=_D,cls=required_if_not_cached(_E))
