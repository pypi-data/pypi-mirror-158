...
from localstack_ext.bootstrap.pods.constants import ASSETS_ROOT_DIR,CLOUD_PODS_DIR,COMMIT_FILE,COMPRESSION_FORMAT,DEFAULT_POD_DIR,HEAD_FILE,KNOWN_VER_FILE,MAX_VER_FILE,META_ZIP,OBJ_STORE_DIR,PODS_CONFIG_FILE,REMOTE_FILE,STATE_ZIP,VER_LOG_FILE,VER_LOG_STRUCTURE,VERSION_SPACE_DIRS,VERSION_SPACE_FILES,VERSIONS_DIR
...
class PodsConfigContext:
	...
	def get_pods_root_dir(B):
		A=os.environ.get('POD_DIR')
		if not A:A=os.path.join(localstack_config.dirs.tmp,DEFAULT_POD_DIR)
		A=os.path.join(A,CLOUD_PODS_DIR);return A
