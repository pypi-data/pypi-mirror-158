...
from localstack_ext.bootstrap.pods.servicestate.service_state_types import Blob
...
class PodsConfigContext:
	...
	def get_version_meta_archive_path(B,version,with_format=_B):
	...
	def get_version_state_archive_path(B,version,with_format=_B):
...
def marshall_backend(backend):
	A=backend;from localstack_ext.bootstrap.state_merge import traverse_object as B
	def C(_type):
		B=_type
		if hasattr(B,'_reduce_patched'):return
		def __reduce__(self):
			A=super(B,self).__reduce__();A=list(A)
			if len(A)>2 and A[2]is _A:A[2]=self.__dict__
			return tuple(A)
		B.__reduce__=__reduce__;B._reduce_patched=_B
	def D(_obj):
		A=type(_obj)
		if isinstance(_obj,defaultdict)and A!=defaultdict:C(A)
	B(A,handler=D,safe=_B);E=marshall_object(A);return E
def unmarshall_backend(backend):
	from localstack_ext.bootstrap.state_merge import traverse_object as B;from localstack_ext.bootstrap.state_utils import get_object_dict as D;A=unmarshall_object(backend)
	def C(_obj):
		A=D(_obj)
		if not A:return
		for (C,B) in A.items():
			if isinstance(B,PriorityQueue):A[C]=E=PriorityQueue();E.queue=B.queue
			if isinstance(B,RLOCK_TYPE):A[C]=threading.RLock()
	B(A,handler=C,safe=_B);return A