...
def is_backend_empty(backend,service):
	...
	match service:
		case'lambda':
			...
			return True
		case _:return not getattr(B,'_ACCOUNT_BACKENDS',{})
def _service_state_from_region_backend(service_backend,api):
	...
	B=ServiceState()
	for (D,E) in A.regions().items():F=ServiceKey(account_id=get_aws_account_id(),region=D,service=api);G=BackendState(F,{C:E});B.put_backend(G)
	return B
def _service_state_from_backend_state(service_backend,api):
	A=service_backend;from localstack_ext.constants import MOTO_BACKEND_STATE_FILE as C;from localstack_ext.services.s3.s3_extended import REGION_PLACEHOLDER as D;B=ServiceState()
	if not isinstance(A,dict):
		if isinstance(A,moto.s3.models.S3Backend):
			if not A.buckets:return B
		A={D:A}
	for (E,F) in A.items():G=ServiceKey(account_id=get_aws_account_id(),region=E,service=api);H=BackendState(G,{C:F});B.put_backend(H)
	return B
	...
	def reset_state(B):
		...
		for A in C:
			...
			if isinstance(A,dict):
				for G in A.keys():
					try:reset_moto_backend_state(A,G)
					except Exception as H:LOG.error('Failed to reset the state for %s: %s',B.service,H)
