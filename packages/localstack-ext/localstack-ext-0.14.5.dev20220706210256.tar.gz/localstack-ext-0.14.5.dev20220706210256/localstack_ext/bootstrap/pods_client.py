...
def get_state_zip_from_instance(get_content=_A,services=_C):
	B=services;C=f"{get_pods_endpoint()}/state";E=','.join(B)if B else'';A=requests.get(C,params={_I:E})
...
class CloudPodsManager(metaclass=ABCMeta):
	...
	def restart_container():
		...
		def C():LOG.info('Waiting for LocalStack instance to be fully initialized ...');B=requests.get(A);C=json.loads(to_str(B.content));D=[A for(B,A)in C[_I].items()];assert set(D)=={'running'}
...
class CloudPodsVersionManager(CloudPodsManager):
	...
	def _prepare_archives_from_pre_signed_urls(content):
		A=new_tmp_file();B=content.get(_F);I=B.get('version_space_url');download(url=I,path=A);C={};D={};J=B.get('meta_state_urls')
	...
	def _add_state_to_cloud_pods_store(A,extract_assets=_A,services=_C):
		...
		for (C,H) in D.state.items():
			for (I,J) in H.backends.items():A.pods_api.create_state_file_from_fs(file_name=I,service=C.service,region=C.region,root=_G,account_id=C.account_id,serialization=Serialization.MAIN,object=F(J))
		if extract_assets:
	...
	def _pull_versions(A,auth_headers,required_versions):
		...
		B=json.loads(D.content);G={_E:B.get(_E),_K:B.get(_D)};remote_utils.create_remote_info_file(remote_info=G,config_context=A.pods_api.config_context);C=CloudPodsVersionManager._prepare_archives_from_pre_signed_urls(B);H=C[0];I=C[1];J=C[2];A.pods_api.merge_from_remote(version_space_archive=H,meta_archives=I,state_archives=J)
	...
	def _inject_from_remote(B,version,retain=_A):
		...
		H=D.get(_M);E=new_tmp_file();download(H,E);F(meta_archives={C:E},state_archives={C:A},config_context=B.pods_api.config_context);return _B
	@staticmethod
	...
	def register_remote(A,pod_name,ci_pod=_A):
		A.pods_api.set_pod_context(A.pod_name);B=A.pods_api.get_max_version_no(require_state_archive=_B)
		if B==0:A.pods_api.push('Initial Version');B=1
		F=get_auth_headers();G=A.create_platform_url(A.pod_name);C={_D:A.pod_name,_J:B,_H:ci_pod};C=json.dumps(C);E=safe_requests.put(G,C,headers=F);H=f"Failed to register pod {A.pod_name}: <content>"
		if not A._check_response(E,message=H):return _A
		D=json.loads(E.content);I={_E:D.get(_E),_K:D.get(_D)};J=D.get(_F);A.pods_api.upload_version_and_product_space(presigned_urls=J);remote_utils.create_remote_info_file(remote_info=I,config_context=A.pods_api.config_context);return _B
...
def push_state(pod_name,pre_config=_C,comment=_C,register=_A,services=_C,**F):
	...
	if C is _C:C=[]
	A=get_pods_manager(pods_name=B)
	if not A.pods_api.config_context.is_initialized():A.init()
	A.pods_api.set_pod_context(pod_name=B);A.push(comment=comment,services=C);D=_B
	if register:E=(pre_config or{}).get(_H,_A);D&=A.register_remote(pod_name=B,ci_pod=E)
	return D
