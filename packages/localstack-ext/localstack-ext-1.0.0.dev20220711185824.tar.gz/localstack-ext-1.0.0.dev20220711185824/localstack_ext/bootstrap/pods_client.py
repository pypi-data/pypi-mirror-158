...
def get_state_zip_from_instance(get_content=_B,services=_A):
...
class CloudPodsManager(metaclass=ABCMeta):
	...
	@abstractmethod
	def pull(self,inject=_B,merge=_B):...
	@abstractmethod
...
class CloudPodsVersionManager(CloudPodsManager):
	...
	def push(A,comment=_A,services=_A):
		...
		if B:A.inject(version=-1,reset_state=_C,merge=_B)
		return PodInfo()
	...
	def inject(A,version,reset_state=_B,merge=_B):
		B=version
	...
	def register_remote(A,ci_pod=_A):
		...
		D=json.loads(E.content);I={_E:D.get(_E),_J:D.get(_D)};J=D.get(_G);A.pods_api.upload_version_and_product_space(presigned_urls=J);remote_utils.create_remote_info_file(remote_info=I,config_context=A.pods_api.config_context);return _C
...
def push_state(pod_name,comment=_A,register=_B,services=_A,**E):
	C=pod_name;B=services
	if B is _A:B=[]
	A=get_pods_manager(pods_name=C)
	if not A.pods_api.config_context.is_initialized():A.init()
	A.pods_api.set_pod_context(pod_name=C);A.push(comment=comment,services=B);D=_C
	if register:D&=A.register_remote()
	return D
def get_pods_endpoint():A=config.get_edge_url();return f"{A}{API_PATH_PODS}"
def pull_state(pod_name,inject=_B,merge=_B,**C):
	A=pod_name
...
def reset_local_state(reset_persistence=_B,services=_A):
