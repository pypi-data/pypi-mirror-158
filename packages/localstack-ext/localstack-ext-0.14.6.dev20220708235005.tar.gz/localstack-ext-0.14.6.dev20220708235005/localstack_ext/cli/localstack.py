...
from ..bootstrap.licensing import is_logged_in
from .cloud_pods import pod
class ProCliPlugin(LocalstackCliPlugin):
	name='pro'
	def should_load(A):return is_logged_in()
	def attach(B,cli):A=cli.group;A.add_command(cmd_logout);A.add_command(daemons);A.add_command(dns);A.add_command(pod)
class LoginCliPlugin(LocalstackCliPlugin):
	name='login'
	def attach(B,cli):A=cli.group;A.add_command(cmd_login)
