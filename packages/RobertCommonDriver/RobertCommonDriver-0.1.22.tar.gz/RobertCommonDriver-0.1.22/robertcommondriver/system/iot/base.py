...
class IOTBaseCommon:
    ...
    class DataTransform:
        ...
        def convert_bytes_to_values(bytes: bytearray, type: TypeFormat, index: int, length: int = 1, encoding: str = '') -> list:
            ...
            if type in [IOTBaseCommon.DataTransform.TypeFormat.INT16, IOTBaseCommon.DataTransform.TypeFormat.INT16_ARRAY]:
                type_size = 2
                type_fmt = '<h'
        ...
        # 将WORD
        @staticmethod
        def convert_values_to_value(values: List[int], type: TypeFormat, format: Optional[DataFormat] = DataFormat.ABCD):
            if len(values) <= 0:
                return None
            bytes = IOTBaseCommon.DataTransform.convert_values_to_bytes(values, IOTBaseCommon.DataTransform.TypeFormat.UINT16)  # CDAB
            bytes = IOTBaseCommon.DataTransform.reverse_bytes(bytes, len(bytes), 0, format)  # 转换为指定顺序
            return IOTBaseCommon.DataTransform.convert_bytes_to_values(bytes, type, 0)[0]

        @staticmethod
        def convert_value_to_values(value, type: TypeFormat, format: Optional[DataFormat] = DataFormat.ABCD):
            bytes = IOTBaseCommon.DataTransform.convert_values_to_bytes(value, type)  # CDAB
            bytes = IOTBaseCommon.DataTransform.reverse_bytes(bytes, len(bytes), 0, format)  # 转换为指定顺序
            return IOTBaseCommon.DataTransform.convert_bytes_to_values(bytes, IOTBaseCommon.DataTransform.TypeFormat.UINT16, 0, int(len(bytes)/2))
...
class IOTDriver:
    ...
    def clean(self):
        try:
            for [index, driver] in self.drivers:
                if hasattr(driver, 'exit'):
                    try:
                        driver.exit()
                    except:
                        pass
