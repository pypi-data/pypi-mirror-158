...
def add_missing(data, cause_dict, m_min=0.1, m_max=0.6):
            ...
            for cause in cause_dict[var]:
                if data[cause].dtype == 'category':
                    ...
                    while True:
                        ...
                        if data[cause].value_counts()[selected_state].sum() > m * data.shape[0]:
                            m_h = np.random.uniform(m, m * data.shape[0] / data[cause].isin(selected_state).sum())
                            m_l = (data.shape[0] * m - data[cause].isin(selected_state).sum() * m_h) / data[cause].isin(
                                states).sum()
                            break
                    data_missing[var][
    ...
    if method == 'pw':
        ...
        if W not in cache_data:
            ...
            if len(W):
                ...
                cache_data[W] = data
            cache_weight[W] = np.ones(len(cache_data[W])) * data.shape[0] / len(cache_data[W])
    elif method == 'ipw':
        W = list(vars)
        while True:
        ...
        if W not in cache_data:
            ...
            if len(W) > 0:
                if 'int' in data.dtype.name:
                    ...
                    cache_data[W] = data[~np.isnan(data[:, W_ids]).any(axis=1)]
                cache_weight[W] = compute_weights(data, varnames, W_ids, cause_list) / cache_data[W].shape[0] * \
                                  data.shape[0]
            else:
                cache_data[W] = data
        ...
        if W not in cache_data:
            if len(W) > 0:
                ...
                if pa_d:
                    cache_weight[W] = np.ones(cache_data[W].shape[0]) / cache_data[W].shape[0] * data.shape[0]
                else:
                    cache_weight[W] = compute_weights(data, varnames, W_ids, cause_list) / cache_data[W].shape[0] * \
                                      data.shape[0]
            else:
...
def edges(graph):
    ...
    for var in graph:
        ...
        for nei in graph[var]['nei']:
            if len(edge[edge['from'] == nei and edge['to'] == var]) == 0:
