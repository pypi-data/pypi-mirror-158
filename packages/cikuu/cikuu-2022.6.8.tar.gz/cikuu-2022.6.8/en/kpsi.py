...
import json, traceback,sys, time, fire,os,traceback
...
def walk(doc): 
	add(f"#SNT") 
	[add( f"{t.lemma_}:POS:{t.pos_}", f"{t.lemma_}:LEX:{t.text.lower()}", f"LEM:{t.lemma_}", f"LEX:{t.text.lower()}", f"{t.pos_}:{t.lemma_.lower()}"
		, f"{t.tag_}:{t.lemma_.lower()}",f"{t.lemma_.lower()}:{t.pos_}:{t.tag_}:{t.text.lower()}",f"{t.lemma_.lower()}:{t.pos_}:{t.tag_}", f"{t.lemma_.lower()}:{t.pos_}") for t in doc if not t.pos_ in ('PROPN','X', 'PUNCT') and t.is_alpha ]
	for t in doc:
		add( "#LEX", f"#{t.pos_}", f"#{t.tag_}",f"#{t.dep_}",) # book:VERB:VBG
	...
	for lem, pos, type, chunk in en.kp_matcher(doc): 
		add(f"{lem}:{pos}:{type}:{chunk}", f"{lem}:{pos}:{type}", f"#{type}")

	for trpx, row in en.dep_matcher(doc): #[('svx', [1, 0, 2])]
		verbi = row[0]
		add(f"{doc[verbi].lemma_}:{doc[verbi].pos_}:{trpx}", f"#{trpx}") #consider:VERB:svx
...
def batch(names="qdu,gzjc,gaokao,clec,dic"):
	''' batch model, --names qdu,gzjc,gaokao,clec,dic '''
...
if __name__	== '__main__':
	fire.Fire({"mysql":mysql, "tsv":tsv, "batch":batch, 'trp':trp })
