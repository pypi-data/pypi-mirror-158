...
def walk(doc): 
	...
	#for type, chunk, start, end in en.vp_matcher(doc): #[('vend', 'consider going', 1, 3) #consider:VERB:vtov:consider to go
	#	add(f"{doc[start].lemma_}:{doc[start].pos_}:{type}:{chunk}", f"{doc[start].lemma_}:{doc[start].pos_}:{type}") if type.startswith("v") else add(f"{doc[end-1].lemma_}:{doc[end-1].pos_}:{type}:{chunk}", f"{doc[end-1].lemma_}:{doc[end-1].pos_}:{type}" ) 
...
def mysql(dbfile, host='127.0.0.1', port=3307):  
...
def tsv(dbfile):  
	...
	with open (f"{name}.tsv.snt", "w", encoding='UTF-8') as fw:
		for rowid, snt, doc in tqdm(Spacybs(dbfile).docs()) : #for rowid, snt, bs in tqdm(Spacybs(dbfile).items()) :
