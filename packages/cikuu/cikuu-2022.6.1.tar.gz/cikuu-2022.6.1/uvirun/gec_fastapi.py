...
def gecsnts_get(snts:str="She has ready.|It are ok.", sepa:str="|", max_length:int=128,  do_sample:bool=False, batch_size:int=64, unchanged_ratio:float=0.45, len_ratio:float=0.5):
	''' return {'She has ready.': 'She is ready.'}, 'It are ok.': 'It is ok.'}, based on local model, no REDIS '''
	return gecsnts( snts.split(sepa), max_length,  do_sample, batch_size, unchanged_ratio, len_ratio)
