...
class GlobalConfigFactory():
    ...
    def load_global_config(lib_path, client_path):
        ...
        for keys in pathtocompute:
            lpath = result_config.get(keys)
            if not os.path.isabs(lpath):
                ...
                if not os.path.exists(lpath):
                    ...
                    if not os.path.exists(lpath):
                        raise Exception('GlobalConfigLoader path {} not exist'.format(keys))
                result_config.flat_config[keys] = os.path.abspath(lpath)
