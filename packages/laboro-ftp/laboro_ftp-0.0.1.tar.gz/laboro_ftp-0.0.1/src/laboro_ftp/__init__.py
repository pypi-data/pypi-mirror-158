...
class Ftp(Module):
  ...
  Arguments:
    ...
    args: A dictionary specifying the FTP connection parameters and client type.
      ...
      ``ssl_version``: String. Optional. A string specifying the TLS version to use (Usualy one of ``TLSv1_3`` or  ``TLSv1_2``). Default to `ssl.PROTOCOL_TLS_CLIENT` which will trigger negotiation between client and server for highest possible security.
  """
  ...
  def delete_remote_dir(self, remote_dir, recursive=False, exit_on_error=True):
    ...
    Arguments:
      ...
      ``recursive``: If set ti ``True``, delete directory and all its content.
      ``exit_on_error``: Boolean. Optional. If set to ``False``, any error encountered will only be logged as a warning. Default: ``True``.
    """
  ...
  def _create_local_dirs(self, remote_file, local_dir):
    ...
    Arguments:
      ...
      ``local_dire``: String. The local directory within duplicate the remote directory tree.
    """
