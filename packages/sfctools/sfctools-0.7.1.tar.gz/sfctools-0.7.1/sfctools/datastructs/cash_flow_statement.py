...
class CashFlowStatement:
    ...
    def total_cash_flow(self):
        ...
        """
        return self.tot_dict["Operating"]  + self.tot_dict["Financing"] + self.tot_dict["Investing"]
    ...
    def operating_cash_flow(self):
        ...
        """
        return self.tot_dict["Operating"]
    ...
    def financing_cash_flow(self):
        ...
        """
        return self.tot_dict["Financing"]
    ...
    def investing_cash_flow(self):
        ...
        """
        return self.tot_dict["Investing"]
    ...
    def reset(self):
        ...
        self.data = {"Operating": defaultdict(lambda: 0.0),
                     "Financing": defaultdict(lambda: 0.0),
                     "Investing": defaultdict(lambda: 0.0),
                     }
    ...
    def to_dataframe(self):
        ...
        df_operating = pd.DataFrame(self.data["Operating"], index=[idx]).T
        df_total_op = pd.DataFrame({"Tot. Operating": self.operating_cash_flow}, index=[idx]).T

        df_financing = pd.DataFrame(self.data["Financing"], index=[idx]).T
        df_total_fin = pd.DataFrame({"Tot. Financing": self.financing_cash_flow}, index=[idx]).T

        df_investing = pd.DataFrame(self.data["Investing"], index=[idx]).T
        ...
        df = pd.concat(frames,keys=["Operating","Tot. Operating", "Finacing","Tot. Financing","Investing","Tot. Investing"])
