...
def get_valid_start_end(
    ...
    * Lastly, if both are provided by the user, we use those without
    ...
    if now is None:
        now = dtlib.datetime.now(dtlib.timezone.utc)
    ...
    if not start and not end:
        return _get_current_start_end(
            timeframe, time_adjustment, include_latest, available_start,
        )
...
def _get_current_start_end(
    ...
    end = dtlib.datetime.utcnow().replace(tzinfo=dtlib.timezone.utc)
    start = end - time_adjustment
...
def get_datetime(value: str | int | None) -> dtlib.datetime | None:
    ...
    if value.isdigit() and len(value) >= 12:
        ...
        first_day_current_yr = dtlib.datetime(
            dtlib.datetime.now(dtlib.timezone.utc).year, 1, 1
        )
...
def get_item_start(item, timeframed_item) -> dtlib.datetime:
    if not item and not timeframed_item:
        return BTC_START

    if not item and timeframed_item:
        return timeframed_item.timestamp_oldest

    if item:
        return item.timestamp_newest
