...
class BaseProtocol(asyncio.Protocol):
    ...
    async def aconnect(self) -> None:
        ...
        async with self._connection_lock:
            ...
            try:
                ...
                # theres a possibility that connection_made can be called
                ...
                # same reasoning applies here as timeout above
                if self._protocol_state is ProtocolState.CONNECTED:
                    self.abort()
                self._protocol_state = ProtocolState.CLOSED
                raise
    ...
    async def wait_flushed(self) -> None:
        # in websocket connections, when failing the connection. we want to
        # send our end of the closing handshake and then close the connection.
        # this coroutine ensures the message is sent and the connection can then
        # close immediately without waiting for connection_lost to be called
    ...
    def connection_made(self, transport: asyncio.Transport) -> None:
        ...
        assert self._flush_waiter is None
    ...
    def resume_writing(self) -> None:
        assert self._drain_waiter is not None
    ...
    def cleanup_futures(
        self,
        exc: Union[ConnectionLost, ConnectionLostError]
    ) -> None:
        # pending reads/writes will raise an exception
        futures = ('_read_waiter', '_drain_waiter')
        for fut in futures:
            self.set_future(fut, exc)
...
class H11Mixin(ReconnectMixin):
    ...
    async def arequest(self, request: Request, auth: Auth = None) -> H11Response:
        ...
        # each time (i.e comlpleting the req/resp cycle). We dont want the
...
class H11Pool:
    ...
    async def arequest(self, request: Request, auth: Auth = None) -> HttpPoolResponse:
        """
    ...
    async def attempt_to_acquire_connection(self, status: RequestStatus) -> bool:
        ...
        for idx, connection in enumerate(self._available):
            ...
            # they can be closed
            assert connection.state is not HttpState.ACTIVE
    ...
    async def response_closed(self, status: RequestStatus) -> None:
        async with self._pool_lock:
            ...
            # all connections should be in the 'busy' container
            assert connection in self._busy
...
class W11Protocol(H11Mixin):
    ...
    async def asend(
        ...
        # passing an AsyncIterable that yields anything other than str or byytes
    ...
    def cleanup_tasks(self) -> None:
        tasks = ('_data_task', '_close_interrupt_task', '_keepalive_task')
        for task in tasks:
