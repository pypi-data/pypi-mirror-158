...
def encode_content(
    ...
    if content is None:
        ...
        return RequestStream(bytes(content))
    elif isinstance(content, str):
        return RequestStream(content.encode())
    elif isinstance(content, dict):
        return RequestStream(json.dumps(content).encode())
    else:
        return content
...
class Header:
    key: Union[str, bytes] = field(
        converter=encode_key, validator=instance_of((str, bytes))
    )
    val: Union[str, int, bytes] = field(
        converter=encode_val, validator=instance_of((str, int, bytes))
    )
...
class URL:
    ...
    def copy_with(
        ...
        host: str = UseExisting,
        port: int = UseExisting,
        path: str = UseExisting,
        query: str = UseExisting,
        fragment: str = UseExisting
    ) -> "URL":
        authority = {}
        for key, val in {'host': host, 'port': port}.items():
            if val is not UseExisting:
                authority[key] = val
        if not authority:
            authority = UseExisting
        kwargs = dict(
            scheme=scheme,
            authority=authority,
            path=path,
    ...
    def copy_with_origin(self, origin: Origin) -> "URL":
        url = self.url.copy_with(
            ...
            authority={'host': origin.host, 'port': origin.port}
        )
        return URL(url)
...
class Request:
    ...
    def _to_event_http(self) -> h11.Request:
        if isinstance(self.content, RequestStream):
...
class H11Response:
    ...
    @property
    def history(self) -> List["H11Response"]:
    ...
    @property
    def peercert(self) -> Union[str, None]:
