...
def cast_query_params(
    ...
    cast_keys = cast.keys()
    invalid_keys = set(cast_keys) - set(available_keys)
    if invalid_keys:
        raise ValueError(
            "The following fields cannot be casted. Fields are not method "
            f"arguments:\n{', '.join(invalid_keys)}"
        )
    casted = []
...
class RestApi:
    ...
    def get(
        ...
        def wrapper(func: Callable[[Any], None]) -> Tuple[HttpMethod, URL]:
            return builder(
                func,
                model.path,
                model.cast,
                HttpMethod.get_method('GET'),
                default_query_type
            )
        return wrapper
    ...
    def websocket(
        ...
        model.merge(self._base_endpoint)
        def wrapper(func: Callable[[Any], None]) -> Tuple[HttpMethod, URL]:
            return builder(func, model.path, model.cast, HttpMethod.get_method('GET'))
        return wrapper