...
from laboro_ssh.utils import log_line
...
class Ssh(Module):
  ...
  Arguments:
    ...
    ``missing_host_key_policy``: String. Optional. Define the behavior when a remote key is missing and remote host is unknown. This should be one of ``auto``, ``warn``, ``reject``. Default to ``warning`` which automatically add remote host key to the known keys and log a warning.
  ...
  def sftp(self):
    ...
    raise LaboroError("[SshError] No connection to remote host.")
  ...
  def _connect(self):
    logging.info(f"Connecting to {self.args.get('host')}:{self.args.get('port')}")
    self.client = paramiko.SSHClient()
    self.client.set_log_channel("laboro.ssh")
  ...
  def _close(self):
    logging.info(f"Closing connection to {self.args.get('host')}:{self.args.get('port')}")
    if self.client is not None and self.connected:
      self.client.close()
      self.connected = False
    ...
    try:
      ...
      while True:
        while channel.recv_ready():
          log_line(stdout.readline().strip(),
                   stdout_content,
                   logging.INFO)
        while channel.recv_stderr_ready():
          log_line(stderr.readline().strip(),
                   stderr_content,
                   logging.ERROR)
  ...
  def get_file(self, remote_file, local_dir,
    ...
    logging.info(f"Getting remote file: {remote_file}")
    try:
  ...
  def put_file(self, local_file, remote_dir,
    ...
    logging.info(f"Sending file to: {remote_fname}")
    if create_dirs:
...
# -----------------------------------------------------------------------------
  ...
  def list_local_dir(self, local_dir, file_type="f", pattern=".*"):
    ...
    logging.info(f"Exploring {local_dir}")
    try:
  ...
  def delete_local_dir(self, local_dir, recursive=False, exit_on_error=True):
    ...
    Arguments:
      ...
      ``exit_on_error``: Boolean. Optional. If set to ``False``, any error encountered will only be logged as a warning. Default: ``True``.
    """
    logging.info(f"Removing local dir: {local_dir}")
  ...
  def _create_local_dirs(self, remote_file, local_dir):
    ...
    logging.info(f"Creating local dir: {hierarchy}")
    try:
...
# -----------------------------------------------------------------------------
  ...
  def list_remote_dir(self, remote_dir, file_type="f", pattern=".*"):
    ...
    if len(remote_dir) > 1:
      remote_dir = remote_dir.rstrip("/")
    reg = re.compile(pattern)
    logging.info(f"Exploring {remote_dir}")
    try:
  ...
  def delete_remote_file(self, remote_file, exit_on_error=True):
    ...
    Arguments:
      ...
      ``exit_on_error``: Boolean. Optional. If set to ``False``, any error encountered will only be logged as a warning. Default: ``True``.
    """
    logging.info(f"Deleting remote file: {remote_file}")
    try:
  ...
  def delete_remote_dir(self, remote_dir, recursive=False, exit_on_error=True):
    ...
    Arguments:
      ...
      ``exit_on_error``: Boolean. Optional. If set to ``False``, any error encountered will only be logged as a warning. Default: ``True``.
    """
    logging.info(f"Deleting remote directory: {remote_dir}")
    try:
      for r_file in self.list_remote_dir(remote_dir=remote_dir):
  ...
  def _create_remote_dirs(self, remote_dir, exit_on_error=True):
    ...
    logging.info(f"Creating remote dir: {remote_dir}")
    r_dir = "/"
