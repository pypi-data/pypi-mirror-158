...
def new_method_colors(
        top,
        fragments=None,
        consensus_maps=None,
        colors=None
):
    r""" Try to construct a comprehensive color list per residue, for all cases"""
    ...
    def aligntop(self, top,
        ...
        for ii, idf in enumerate(df):
            ...
            if isinstance(top, str) or str(_frag_str).lower() in ["none", "false"] or len(df) == 1:
                ...
                for frag_idx, (fraglab, fragidxs) in enumerate(consfrags.items()):
                    ...
                    if not spread_frg:
                        unbroken = False
                        if debug or True:
                            print("Optimal alignment nr. %u (score %u): the fragments derived from '%s' break the consensus fragment %s:"%(ii, idf.alignment_score, _frag_str, fraglab))
                            _mdcfrg.print_frag(frag_idx, top, fragidxs, resSeq_jumps=True, idx2label=conlab, fragment_desc="%s"%fraglab)
                        break
                if unbroken:
                    break
