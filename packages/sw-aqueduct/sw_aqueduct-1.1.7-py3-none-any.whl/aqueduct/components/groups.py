...
class Groups(ComponentBase):
    ...
    def _get_destination_group(self, group: Group):
        """Attempts to retrieve a destination instance group.

        Args:
            group (Group): The source Swimlane instance `Group` object.

        Returns:
            Group or bool: Returns a `Group` object if found if not then a boolean.
        """
        try:
    ...
    def _process_group(self, group: Group):
        """Processes a source Swimlane instance group objects related objects.

        This method processes any users or roles defined associated with the provided Group object.

        Args:
            group (Group): A source Swimlane instance `Group` object.

        Returns:
            Group: The source Swimlane instance `Group` object with possible updates.
        """
        if group.users:
