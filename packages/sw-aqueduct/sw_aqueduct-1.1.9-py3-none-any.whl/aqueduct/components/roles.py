...
class Roles(ComponentBase):
    ...
    def _process_role(self, role: Role):
        """Used to ensure any referenced user and groups are processed before continuing with adding / updating roles.

        Args:
            role (Role): A source Swimlane instance `Role` object.

        Returns:
            Role: A modified source Swimlane instance `Role` object.
        """
        if role.users:
    ...
    def sync_role(self, role: Role):
        """Syncs a single source Swimlane instance `Role` object.

        Args:
            role (Role): A source Swimlane instance `Role` object.

        Raises:
            AddComponentError: Raises when unable to add a `Role` to a destination Swimlane instance.

        Returns:
            Role: A destination Swimlane instance `Role` object.
        """
