...
class CloudRequests:
    ...
    def __init__(self, cloud_connection, *, ignore_exceptions=True, force_reconnect=True, _log_url="https://clouddata.scratch.mit.edu/logs", _packet_length=220):
        print("\033[1mIf you use CloudRequests in your Scratch project, please credit TimMcCool!\033[0m")
        if log_url != "https://clouddata.scratch.mit.edu/logs":
            ...
            if not self.last_data == data:
                for activity in data:
                    if activity['timestamp'] > self.last_timestamp and activity['name'] == "☁ TO_HOST":
                        ...
                        commands = list(filter(lambda k: k.__name__ == request, self.requests))7
                        if len(commands) == 0:
                            print(f"Warning: Client received an unknown request called '{request}'")
                            self._respond(request_id, Encoding.encode(f"Error: Unknown request"), packet_length)
                        else:
                            try:
                                output = commands[0](*arguments)
                            except TypeError as e:
                                self._respond(request_id, Encoding.encode("Error: Client received too many arguments, not enough arguments or invalid arguments"), packet_length)
                                print(f"Error in request '{request}': Client received too many arguments, not enough arguments or invalid arguments.\nOriginal error: {e}")
                            except Exception as e:
                                self._respond(request_id, Encoding.encode(f"Error: Check the Python console"), packet_length)
                        ...
                        if not isinstance(output, list):
                            ...
                            for i in input:
                                ...
                                output += "89"
                        self._respond(request_id, output, packet_length)
                        self.last_timestamp = activity['timestamp']
                self.last_data = data
