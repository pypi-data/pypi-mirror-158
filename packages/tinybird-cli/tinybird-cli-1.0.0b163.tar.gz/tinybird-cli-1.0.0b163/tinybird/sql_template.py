...
def _and(*args, **kwargs):
    ...
    def _op(k):
        ...
        if len(tk) == 1:
            ...
            raise SQLTemplateException(f"operand {tk[1]} not supported", documentation='/cli/advanced-templates.html#id18')
...
def columns(x, default=None, fn=None):
    if x is None or type(x) == Placeholder:
        if default is None:
            raise SQLTemplateException("Missing columns() default value, use `columns(column_names, 'default_column_name')`", documentation='/cli/advanced-templates.html#id13')
        x = default

    try:
        _columns = [c.strip() for c in x.split(',')]
    except AttributeError:
        raise SQLTemplateException("The 'columns' function expects a String not an Array", documentation='/cli/advanced-templates.html#id13')
...
def column(x, default=None):
    if x is None or type(x) == Placeholder:
        if default is None:
            raise SQLTemplateException("Missing column() default value, use `column(column_name, 'default_column_name')`", documentation='/cli/advanced-templates.html#id12')
        x = default
...
def day_diff(d0, d1, default=None):
    ...
    try:
        ...
        raise SQLTemplateException(
            'invalid date format, it must be ISO format date YYYY-MM-DD, e.g. 2018-09-26', documentation='/cli/advanced-templates.html#id14')
...
def render_sql_template(sql: str, variables: dict = None, test_mode: bool = False) -> Tuple[str, dict]:
    ...
    try:
        ...
        raise SQLTemplateException(e, documentation='/cli/advanced-templates.html#id11')
