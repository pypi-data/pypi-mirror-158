...
class ApolloClientBase(object):
    ...
    def get_json_from_net(self, namespace='application', timeout=3):
        url = '{}/configs/{}/{}/{}?releaseKey={}&ip={}'.format(self.config_url, self.app_id, self.cluster, namespace,
                                                               "", self.ip)
        try:
            code, body = self.http_request(url, timeout=timeout, headers=self._signHeaders(url))
            if code == 200:
        ...
        try:
            ...
            if http_code == 200:
                ...
                for entry in data:
                    ...
                    self._get_net_and_set_local(namespace, n_id, timeout=self._pull_timeout, call_change=True)
                    return
            else:
    ...
    def _get_net_and_set_local(self, namespace, n_id, timeout=60, call_change=False):
        namespace_data = self.get_json_from_net(namespace, timeout=timeout) or {}
    ...
    def _do_heartBeat(self, namespace):
        ...
        url = '{}/configs/{}/{}/{}?releaseKey={}&ip={}'.format(self.config_url, self.app_id, self.cluster, namespace,
                                                               release_key, self.ip)
        try:
            code, body = self.http_request(url, timeout=self._heart_timeout, headers=self._signHeaders(url))
            if code == 200:
                data = json.loads(body)
