...
refer to https://github.com/martijnvermaat/calmap
"""
...
def yearplot(data, year=None, how='sum', vmin=None, vmax=None, cmap='Reds',
    ...
    data = cmadaas_obs_by_time_range_and_id("[20200101000000,20201231000000]",
            data_code='SURF_CHN_MUL_DAY', elements="Station_Id_C,Datetime,TEM_Max")
    ...
    levels = [-20, -10, 0, 10, 25, 40]
    cmap = colors.ListedColormap(color_list)  
    norm = colors.BoundaryNorm(levels, cmap.N)

    # 绘图
    fig, ax = plt.subplots(figsize=(18, 9))
    yearplot(data,cmap=cmap, norm=norm, how=None, year=2020, ax=ax, )

    cbar_ax  = fig.add_axes([0.94, 0.4, 0.015, 0.2])
    cb  = mpl.colorbar.ColorbarBase( cbar_ax, cmap=cmap, ticks=levels, norm=norm,
            orientation='vertical', extend='neither', extendrect=True, extendfrac=0.15 )
    ...
    # 标题
    ax.set_title(f'Maximum Temperature', fontweight = 'bold', fontsize = 25)
    plt.show()
    """
    if year is None:
    ...
    by_day = pd.DataFrame({'data': by_day,
                           ...
                           'week': pd.Int64Index(data.index.isocalendar().week)})
    ...
    if 'norm' in kwargs:
        ax.pcolormesh(plot_data, cmap=cmap, **kwargs)
    else:
    ...
    ax.set_xlabel('')
    ax.set_xticks([by_day.loc[pd.Timestamp(year, i + 1, 15)].week
                   for i in monthticks])
