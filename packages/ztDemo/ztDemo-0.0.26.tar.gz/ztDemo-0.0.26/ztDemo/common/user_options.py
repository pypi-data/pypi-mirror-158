...
def parse_options(user_options=None):
    ...
    parser.add_argument('-env', action='store', default=None, dest='default_env', metavar='target environment',
                        help='Specify test environment')
    parser.add_argument('-env_f', action='store', default=None, dest='default_env_folder', metavar='target environment folder',
                        help='Specify test environment folder')
    parser.add_argument('-t', action='store', default=None, dest='test_targets',
    ...
    parser.add_argument('-r', action='store', dest="report_file", default=None,
                        metavar='store report base file',
                        help='Specify report base file')
    ...
    def split(option_value):
        return None if option_value is None else re.split(r'[;,\s]\s*',  option_value)
    ...
    if options.default_env:
        try:
            module_package = options.default_env_folder
            print(88)
            print(module_package)
            module_package = module_package.replace('/', os.sep)
