...
def test_job_submit_parse_logic(mock_auth_api_client) -> None:
    job_controller = JobController()
    job_controller.submit_with_entrypoint = Mock()  # type: ignore
    job_controller.submit_from_file = Mock()  # type: ignore
    ...
    job_controller.submit_from_file.assert_called_once_with(
        "file", name=None, description=None
    )
    job_controller.submit_from_file.reset_mock()

    job_controller.submit("file", entrypoint=["entrypoint"], is_entrypoint_cmd=True)
    job_controller.submit_with_entrypoint.assert_called_once_with(
