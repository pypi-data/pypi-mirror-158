...
class Redshift():
    def __init__(self, env, rs_iam_role):
        ...
        if env == 'local':
            try:
                ...
                logging.error('Issue retrieving redshift env variabls from local env')
        if env in ['dev', 'prd']:
    ...
    def _create_stage_like_target(self,rs_target, rs_stage):
        self.query(f"""DROP TABLE IF EXISTS {rs_stage}""")
        self.query(f"""CREATE TABLE {rs_stage} (LIKE {rs_target})""")
        self._enable_autocommit() 
        self.query(f"""ALTER TABLE {rs_stage} DROP COLUMN meta_loading_date_utc""")
    ...
    def _upsert(self, rs_target, rs_stage, upsert_keys):
        ...
        for col in upsert_keys:
            if counter == 0:
                line = f'{rs_target}.{col} = {rs_stage}.{col}'
            else:
                line = f'AND {rs_target}.{col} = {rs_stage}.{col}'
            where_condition =+ line
            counter =+ 1
        ...
        try:
            self.query(f"""
                    ...
                    WHERE {where_condition} AND
                        {rs_stage}.datecreated > {rs_target}.datemodified
                    ;
