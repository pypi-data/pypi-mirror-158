...
mapping_table_sqlserver_redshift = {
    'bigint': 'BIGINT',
...
mapping_table_pandas_redshift = {
    'int64': 'BIGINT',
    'float64': 'DOUBLE PRECISION',
    'object': 'VARCHAR(MAX)'
}

mapping_table_pandas_pyspark = {
    'int64': IntegerType(),
    'float64': DoubleType(),
    'object': StringType()
}
...
def csv_to_flyway_ddl(cols, dtypes, target_table, flyway_schema='ingest'):
    """
        ARGUMENTS
            cols: df.columns
            dtypes: df.dtypes
    ...
    for key, val in d.items():
        target_type = mapping_table_pandas_redshift[val.name]
...
def ddl_sql_to_redshift(sql_ddl, target_table, flyway_schema='ingest'):
    ...
    for col in columns:
        ...
        target_type = mapping_table_sqlserver_redshift[source_type]
...
def pyspark_to_redshift(pyspark_schema, target_table, flyway_schema='ingest', ):
    json_sch = pyspark_schema.jsonValue()
    target_cols = []
    for field in json_sch['fields']:
        target_name = field['name']
        target_type = mapping_pyspark_to_redshift[field['type']]
