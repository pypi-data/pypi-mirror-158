...
def deploy_xe_infra(device_list: DeviceDict, usecase: Automation) -> None:
    ...
    try:
        ...
        info("DDR XE Infra Deploy: Install DDR Python Packages and Dependencies")

        for result in pack_ins.result.values():
            ver_check_list = result["python3 --version"].data.split("\n")
#PVH export command options?
        if "Python 3.6.8" in ver_check_list:
            export_ins1 = flow.exec_wait(
                [
                    ...
                    "pip3 install ddr-python --no-cache-dir --user",
                    "exit",
                ],
                timeout=1200,
            )

# treat Python 2 as an error

        else:
            raise Exception(
                Color.BOLD
                + Color.RED
                + f"DDR XE Infra Deploy: Exception: Python 2 in the Guestshell is not supported"
                + " "
                + str(e)
                + Color.END
            )
    ...
    try:
        ...
        raise Exception(
            Color.BOLD
            + Color.RED
            + f"DDR Infra Deploy: An Exception occured in the /bootflash/guest-share/ddr directory"
            + " "
            + str(e)
            + Color.END
        )
    ...
    time.sleep(10)
#PVH
def deploy_infra(device_list: DeviceDict, usecase: Automation) -> None:
    """
    ...
    try:
        ...
        if xe_dict:
            deploy_xe_infra(xe_dict, usecase)
...
#                "ip ssh version 2",
    ...
    info(f"DDR XE Runtime is deployed")

def deploy_xe_usecase(device_list: DeviceDict, usecase: Automation) -> None:
    """
    ...
    try:
        ...
        for device in flow.active_devices.values():
            if "Cisco IOS XE Software" in ver_check.result[device.name].data:
                ...
                raise ValueError(
                f"DDR Use Case Deploy: Valid Cisco OS Version not detected in device - {device.name}, terminating the session\n"
            )

        xe_dict = flow.active_devices.subset(xedevices)

        if xe_dict:
            deploy_xe_usecase(xe_dict, usecase)

    except Exception as e:
        raise Exception(
            Color.BOLD
            + Color.RED
            + f"DDR Use Case Deploy: An Exception occured in the section verify cisco os version on the device"
            + " "
            + str(e)
            + Color.END
        )
...
#PVH update log processing for CLIPS
        if usecase.model is Model.CLIPS:
...
# Git the most recent use case log which will be the first in the directory
#
...
#
            ...
            all_files = flow.exec_wait(log_files)
            for result in all_files.result.values():
                all_files_list = result.data.split("\n")
            ...
            for ddr_log in all_files_list:
                ...
                if result:
                    ...
                    log_content = flow.exec_wait(log_path)
    ...
    info(f"DDR Collect XE Data: DDT Use Case Execution Logs saved in /bootflash/guest-share/ddr/{usecase.name}/")
