...
class PluginParallelsProjectBackend(AbstractBackend):
    ...
    def run_eks_job(
        ...
        if input_data_spec:
            try:
                core_api_instance.delete_namespaced_secret(namespace=job_namespace, name='input-data-spec')
            except:
                pass
            sec2 = kubernetes.client.V1Secret()
            sec2.metadata = kubernetes.client.V1ObjectMeta(name='input-data-spec', namespace=job_namespace)
            sec2.type = 'Opaque'
            sec2.data = {'input-data-spec': input_data_spec}
        ...
        if input_data_spec:
            volume_mounts.append(kubernetes.client.V1VolumeMount(mount_path='/root/.mlflow-parallels', name='input-data-spec'))
        ...
        if input_data_spec:
            job_template["spec"]["template"]["spec"]["volumes"].append(
                    kubernetes.client.V1Volume(name="input-data-spec", secret=kubernetes.client.V1SecretVolumeSource(secret_name='input-data-spec')))
