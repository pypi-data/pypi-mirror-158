...
def get_input_spec_json(input_name=None):
    if not os.path.isfile(INPUT_SPEC_CONFIG):
        ...
        return None
    with open(INPUT_SPEC_CONFIG, 'r') as fp:
        inp_contents = fp.read()
    print('input_data_spec raw=' + inp_contents)
    specs_int = json.loads(inp_contents)
    print('input_data_spec decoded1=' + str(specs_int))
    specs = []
    for s in specs_int:
        specs.append(json.loads(s))
    print('input_data_spec decoded2=' + str(specs))
