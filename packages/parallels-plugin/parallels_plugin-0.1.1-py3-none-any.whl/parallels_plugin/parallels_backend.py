...
from parallels_plugin.login import get_conf, get_token, tokfile_name, get_env_var
import kubernetes
from kubernetes.config.config_exception import ConfigException
...
class PluginParallelsProjectBackend(AbstractBackend):
    ...
    def run_eks_on_local(self, backend_type, project_uri, entry_point, params,
        ...
        if (verbose):
            _logger.info("PluginParallelsProjectBackend: run_eks. kube-context=" + str(kube_context)\
        ...
        submitted_run = self.run_eks_job(
            ...
            backend_config.get('INPUT_DATA_SPEC'),
        )
    ...
    def run_eks_job(
        ...
        print('run_eks_job: input_data_spec=' + str(input_data_spec))
        ...
        tok = base64.b64encode(open(tokfile_name(), "r").read().encode('utf-8')).decode('utf-8')
        try:
            core_api_instance.delete_namespaced_secret(namespace=job_namespace, name='parallelstokenfile')
        except:
            pass
        sec = kubernetes.client.V1Secret()
        sec.metadata = kubernetes.client.V1ObjectMeta(name='parallelstokenfile', namespace=job_namespace)
        sec.type = 'Opaque'
        sec.data = {'token': tok}
        ...
        sec1.metadata = kubernetes.client.V1ObjectMeta(name='awscredsfile', namespace=job_namespace)
        sec1.type = 'Opaque'
        sec1.data = {'credentials': aws_creds}
        core_api_instance.create_namespaced_secret(namespace=job_namespace, body=sec1)
        if input_data_spec:
            try:
                core_api_instance.delete_namespaced_secret(namespace=job_namespace, name='input_data_spec')
            except:
                pass
            sec2 = kubernetes.client.V1Secret()
            sec2.metadata = kubernetes.client.V1ObjectMeta(name='input_data_spec', namespace=job_namespace)
            sec2.type = 'Opaque'
            sec2.data = {'input_data_spec': input_data_spec}
        ...
        if input_data_spec:
            volume_mounts.append(kubernetes.client.V1VolumeMount(mount_path='/root/.mlflow-parallels', name='input_data_spec')
        job_template["spec"]["template"]["spec"]["containers"][0]["volumeMounts"] = volume_mounts

        job_template["spec"]["template"]["spec"]["volumes"] = [
                    kubernetes.client.V1Volume(name="parallels-token-file", secret=kubernetes.client.V1SecretVolumeSource(secret_name='parallelstokenfile')),
        ...
        if input_data_spec:
            job_template["spec"]["template"]["spec"]["volumes"].append(
                    kubernetes.client.V1Volume(name="input_data_spec", secret=kubernetes.client.V1SecretVolumeSource(secret_name='input_data_spec')))
