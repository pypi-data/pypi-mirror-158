...
INPUT_SPEC_CONFIG = '/root/.mlflow-parallels-data/input-data-spec'
...
def get_input_spec_json(input_name=None):
    if not os.path.isfile(INPUT_SPEC_CONFIG):
        print(INPUT_SPEC_CONFIG + ' does not exist. Doing nothing')
        return None
    with open(INPUT_SPEC_CONFIG, 'r', encoding="utf-8") as fp:
        inp_contents = fp.read()
    print('input_data_spec raw=' + inp_contents)
    inp_contents = inp_contents.rstrip('"').lstrip('"')
    print('input_data_spec stripped=' + inp_contents)
    print('inp_contents type=' + str(type(inp_contents)), flush=True)
    for s in inp_contents:
        print(str(s) + " of type " + str(type(s)), flush=True)
    specs = json.loads(inp_contents)
    print('input_data_spec decoded1=' + str(specs))
    print('Yippp', flush=True)
...
#
#     if mountname:
#         spec = named_spec_map.get(mountname)
#     else:
#         spec = specs[0]
#
#     print("Mounting..." + mpath)
#     mount_spec_object = load_input_specs(spec, mpath)
#     updated_mount_point = mount_spec_object['mountpoint']
#
#     perform_mount(updated_mount_point, mount_spec_object)
