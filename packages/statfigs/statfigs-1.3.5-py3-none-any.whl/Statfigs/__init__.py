...
def bar_unttest(df,para,grp,ymax,flt=''):
    ...
    ax.set_xticks(x1)
    ax.set_xticklabels(u1), ax.set_ylabel(para.replace('_',' ')+'(g)', labelpad=5) #labels
    ax.set_ylim(top=ymax),
    ax.set_xlim(-0.5, 1.5)
    ax.yaxis.set_major_formatter(FormatStrFormatter('%.1f')) #sets decimal places
    ax.locator_params(axis='y', nbins=5) #set the maximum number of ticks on the y-axis
    ...
    props = {'connectionstyle':"bar,fraction=0.2",'arrowstyle':'-','shrinkA':10,'shrinkB':10,'linewidth':1} #fraction is the distance of the connecting line from point A and B 
    ax.annotate(text,xy=(x,y*1.11),ha='center',fontsize=15) #text annotation
...
def plot_2y(df,grp,x,y,order='',ymax=0,x_label=''):
    ...
    ax.errorbar(x1, y1, yerr=yerr1,capsize=(2),ecolor='k',color='#92414e',fmt='s',ms='4')
    ax.errorbar(x1, y2, yerr=yerr2,capsize=(2),ecolor='k',color='#200e11',fmt='s',ms='4')
    ...
    for p in pval:
        if p < 0.0001:
            text = '****'
            ax.text(s=text,fontsize=15,ha='center',va='center',x=pval.index(p),y=(np.max(ymerge,axis=1)[pval.index(p)])+(np.max(yerrmerge,axis=1)[pval.index(p)])*1.1)
        elif p < 0.001:
            text='***'
            ax.text(s=text,fontsize=15,ha='center',va='center',x=pval.index(p),y=(np.max(ymerge,axis=1)[pval.index(p)])+(np.max(yerrmerge,axis=1)[pval.index(p)])*1.1)
        elif p<0.01:
    ...
    #Figure size and export as .png
    fig.savefig(grp+'_'+y+'_JR.png',dpi=200,bbox_inches='tight')

#4: cluster bar graph but only an paired t-test for within groups within 1 paramater  
def cbar_pttest(df, para, grp, legend, ymax):
