...
def bar_unttest(df,para,grp,ymax,flt=''):
    ...
    #Graph: Create
    fig, ax = plt.subplots(1,1, figsize = (7.2,11))
    ax.bar(x1, y1, color="#00004d",width=0.5)
    ax.errorbar(x1, y1, yerr=yerr1,fmt=' ', capsize=(3),zorder=0, ecolor='k')
    ax.spines['right'].set_visible(False), ax.spines['top'].set_visible(False)
    ax.set_xticks(x1)
    ax.set_xticklabels(u1,size=20), ax.set_ylabel(para.replace('_',' ')+'(g)',size=20, labelpad=10) #labels
    ax.set_ylim(top=ymax), ax.tick_params(axis='both',labelsize=17)
    ax.set_xlim(-0.5, 1.5)
    ...
    ax.annotate(text,xy=(x,y*1.12),ha='center',fontsize=30) #text annotation
