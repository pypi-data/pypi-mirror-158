...
from fsai_file_split.split_libs.jsonl_split import split_by_number_of_buckets


def t_or_f(arg):
    ua = str(arg).capitalize()
...
def app():
    ...
    parser.add_argument(
        "-cs",
        "--chunk_size",
        help="The size of each split chunk.",
        default=10,
        required=True,
        type=int,
    )
    ...
    if output_file_path.endswith(".jsonl"):
        ...
        if args["split_by"] == "number_of_buckets":
            saved_files = split_by_number_of_buckets(
                args["chunk_size"], args["input_file_path"], output_file_path
            )
