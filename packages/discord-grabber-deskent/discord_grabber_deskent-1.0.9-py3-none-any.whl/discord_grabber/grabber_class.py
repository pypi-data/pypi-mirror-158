...
from .exceptions import CaptchaAPIkeyError, CaptchaTimeoutError, RequestError, FingerPrintError
...
class TokenGrabber:
    ...
    def __init__(
            self, email: str, password: str, anticaptcha_key: str, proxy: str = '',
            log_level: int = 20, user_agent: str = '', logger=None, max_tries: int = 12,
            timeout: int = 5, pause: int = 10
    ):
        ...
        self.user_agent: str = (
            user_agent
            if user_agent
            else UserAgent(path='./useragent.json', verify_ssl=False)['google chrome']
        )
        self.headers: dict = {}
        ...
        async with aiohttp.ClientSession(timeout=self.timeout) as session:
            ...
            try:
                return await self._get_token_data()
                    ...
                    CaptchaTimeoutError, CaptchaAPIkeyError, RequestError, FingerPrintError
            ) as err:
                error_text = f'{err}'
    ...
    async def _get_captcha_id(self, site_key: str) -> str:
        url = (
    ...
    async def __get_captcha_token(self, id_: str) -> str:
        url = (
    ...
    async def _get_captcha_token(self, captcha_sitekey: str) -> str:
        ...
        captcha_id: str = await self._get_captcha_id(captcha_sitekey)
        captcha_result: str = await self.__get_captcha_token(captcha_id)
