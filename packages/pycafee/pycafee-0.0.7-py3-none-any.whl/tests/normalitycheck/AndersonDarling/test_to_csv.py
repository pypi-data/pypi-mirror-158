...
Command to run at the prompt:
    python -m unittest -v tests/normalitycheck/AndersonDarling/test_to_csv.py

    or
    python -m unittest -b tests/normalitycheck/AndersonDarling/test_to_csv.py
...
class Test_to_csv(unittest.TestCase):
    ...
    def test_file_creating_1(self):
        ...
        test.to_csv()
        result = _check_file_exists("andersondarling.csv")
        self.assertTrue(result, "File was not created")
        rem_file = Path("andersondarling.csv")
        rem_file.unlink()
