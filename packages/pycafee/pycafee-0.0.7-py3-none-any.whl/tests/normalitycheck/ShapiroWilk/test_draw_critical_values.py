...
Command to run at the prompt:
    python -m unittest -v tests/normalitycheck/ShapiroWilk/test_draw_critical_values.py
    or
    python -m unittest -b tests/normalitycheck/ShapiroWilk/test_draw_critical_values.py
...
class Test_axes_test_draw_critical_values(unittest.TestCase):
    ...
    def test_basic_plot(self):
        test = ShapiroWilk()
        fig, ax = plt.subplots()
        axes = test.draw_critical_values(ax=ax)
    ...
    def test_is_exported_plot(self):
        test = ShapiroWilk()
        fig, ax = plt.subplots()
        axes = test.draw_critical_values(ax=ax, export=True)
        result = _check_file_exists("shapiro_wilk_critical_plot.png")
        plt.close()
        try:
            rem_file = Path("shapiro_wilk_critical_plot.png")
            rem_file.unlink()
        except:
            pass
        self.assertFalse(result, "File 'shapiro_wilk_critical_plot.png' was created even with ax=ax")
...
class Test_None_draw_test_draw_critical_values(unittest.TestCase):
    ...
    def test_basic_plot(self):
        with patch('matplotlib.pyplot.show') as p:
            test = ShapiroWilk()
            axes = test.draw_critical_values(ax=None)
            result = isinstance(axes, matplotlib.axes.SubplotBase)
            self.assertTrue(result, "Ax not returned")
            plt.close()
    ...
    def test_plot_export(self):
        with patch('matplotlib.pyplot.show') as p:
            test = ShapiroWilk()
            axes = test.draw_critical_values(ax=None, export=True)
            result = _check_file_exists("shapiro_wilk_critical_plot.png")
            rem_file = Path("shapiro_wilk_critical_plot.png")
            rem_file.unlink()
        ...
        with patch('matplotlib.pyplot.show') as p:
            test = ShapiroWilk()
            axes = test.draw_critical_values(ax=None, export=True, file_name="teste")
        ...
        with patch('matplotlib.pyplot.show') as p:
            test = ShapiroWilk()
            axes = test.draw_critical_values(ax=None, export=True, file_name="teste", extension='pdf')
        ...
        with patch('matplotlib.pyplot.show') as p:
            test = ShapiroWilk()
            axes = test.draw_critical_values(ax=None, export=False, file_name="teste", extension='pdf')
            result = _check_file_exists("teste.pdf")
            plt.close()
            try:
                rem_file = Path("teste.pdf")
    ...
    def test_decimal_separator(self):
        with patch('matplotlib.pyplot.show') as p:
            test = ShapiroWilk()
            axes = test.draw_critical_values(ax=None, decimal_separator=",")
        ...
        with patch('matplotlib.pyplot.show') as p:
            test = ShapiroWilk()
            axes = test.draw_critical_values(ax=None, decimal_separator=None)
            for text in axes.get_yticklabels():
                temp = text.get_text()
                result = True
                if ',' in temp:
