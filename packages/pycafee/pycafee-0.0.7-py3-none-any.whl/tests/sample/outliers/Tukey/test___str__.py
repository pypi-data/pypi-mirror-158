...
Command to run at the prompt:
    python -m unittest -v tests/sample/outliers/Tukey/test___str__.py
    or
    python -m unittest -b tests/sample/outliers/Tukey/test___str__.py
...
class Test_str(unittest.TestCase):
    ...
    def test_without_fit(self):
        teste = Tukey()
        self.assertEqual(teste.__str__(), "The Tukey test was not performed yet. Use the 'fit' method to perform the test.", msg="worng __str__")

        teste = Tukey(language="pt-br")
        self.assertEqual(teste.__str__(), "O teste de Tukey não foi realizado. Utilize o método 'fit' para realizar o teste.", msg="worng __str__")
