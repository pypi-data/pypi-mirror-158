"""Tests if the confidencial_interval function for Sample is working as expected

--------------------------------------------------------------------------------
Description:

---> Class Test_confidencial_interval. This function checks if a ValueError is raised when the fit was not performed, if the output is a dataframe, if the dataframe has the espected column names, if the dataframe has the right shape. Also, tests the print output, but just the last print statement, and test when show=False.



--------------------------------------------------------------------------------
Command to run at the prompt:
    python -m unittest -v tests/sample/test_confidencial_interval.py
    or
    python -m unittest -b tests/sample/test_confidencial_interval.py
...
class Test_confidencial_interval(unittest.TestCase):
    ...
    def test_no_fit(self):
        with self.assertRaises(ValueError, msg="Does not raised error when fit was not performed"):
            amostra = Sample()
            amostra.confidencial_interval()

        with self.assertRaises(ValueError, msg="Does not raised error when fit was not performed"):
            amostra = Sample()
            amostra.confidencial_interval(show=True)
    ...
    def test_output_is_data_frame(self):
        amostra = Sample(name="bwidaba", language="pt-br")
        amostra.fit(self.x)
        result = amostra.confidencial_interval(show=True)
        self.assertIsInstance(result, pd.DataFrame, msg="Output is not a DataFrame")

        amostra = Sample()
        amostra.fit(self.x)
        result = amostra.confidencial_interval(show=False)
        self.assertIsInstance(result, pd.DataFrame, msg="Output is not a DataFrame")

    def test_output_data_frame(self):
        amostra = Sample(name="bwidaba", language="pt-br")
        amostra.fit(self.x)
        df = amostra.confidencial_interval(show=True)
        result = df.columns
        espected = ['Média - IC', 'Média', 'Média + IC']
    ...
    def test_print(self, mock_print):
        ...
        amostra.fit(self.x)
        amostra.confidencial_interval(show=True)
        mock_print.assert_called_with('with a 95.0% of confidence level.')

        amostra = Sample(language='pt-br')
        amostra.fit(self.x)
        amostra.confidencial_interval(show=True)
        mock_print.assert_called_with('com 95.0% de confiança.')
    ...
    def test_show_false(self, mock_print):
        with self.assertRaises(AssertionError, msg="Does not raised AssertionError when show=False"):
            amostra = Sample()
            amostra.fit(self.x)
            amostra.confidencial_interval(show=False)
