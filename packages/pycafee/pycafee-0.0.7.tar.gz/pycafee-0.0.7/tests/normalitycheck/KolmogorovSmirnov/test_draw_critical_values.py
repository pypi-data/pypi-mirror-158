...
Command to run at the prompt:
    python -m unittest -v tests/normalitycheck/KolmogorovSmirnov/test_draw_critical_values.py
    or
    python -m unittest -b tests/normalitycheck/KolmogorovSmirnov/test_draw_critical_values.py
...
class Test_axes_draw_ks_tabulated_values(unittest.TestCase):

    def test_basic_plot(self):
        ks_test = KolmogorovSmirnov()
        fig, ax = plt.subplots()
        axes = ks_test.draw_critical_values(ax=ax)
    ...
    def test_is_exposted_plot(self):
        ks_test = KolmogorovSmirnov()
        fig, ax = plt.subplots()
        axes = ks_test.draw_critical_values(ax=ax, export=True)
        result = _check_file_exists("kolmogorov_smirnov_critical_plot.png")
        plt.close()
        try:
            rem_file = Path("kolmogorov_smirnov_critical_plot.png")
            rem_file.unlink()
        except:
            pass
        self.assertFalse(result, "File 'kolmogorov_smirnov_critical_plot.png' was created even with ax=ax")




class Test_None_draw_ks_tabulated_values(unittest.TestCase):

    def test_basic_plot(self):
        with patch('matplotlib.pyplot.show') as p:
            ks_test = KolmogorovSmirnov()
            axes = ks_test.draw_critical_values(ax=None)
            result = isinstance(axes, matplotlib.axes.SubplotBase)
            self.assertTrue(result, "Ax not returned")
            plt.close()
    ...
    def test_plot_export(self):
        with patch('matplotlib.pyplot.show') as p:
            ks_test = KolmogorovSmirnov()
            axes = ks_test.draw_critical_values(ax=None, export=True)
            result = _check_file_exists("kolmogorov_smirnov_critical_plot.png")
            rem_file = Path("kolmogorov_smirnov_critical_plot.png")
            rem_file.unlink()
        ...
        with patch('matplotlib.pyplot.show') as p:
            ks_test = KolmogorovSmirnov()
            axes = ks_test.draw_critical_values(ax=None, export=True, file_name="teste")
        ...
        with patch('matplotlib.pyplot.show') as p:
            ks_test = KolmogorovSmirnov()
            axes = ks_test.draw_critical_values(ax=None, export=True, file_name="teste", extension='pdf')
        ...
        with patch('matplotlib.pyplot.show') as p:
            ks_test = KolmogorovSmirnov()
            axes = ks_test.draw_critical_values(ax=None, export=False, file_name="teste", extension='pdf')
            result = _check_file_exists("teste.pdf")
            plt.close()
            try:
                rem_file = Path("teste.pdf")
    ...
    def test_decimal_separator(self):
        with patch('matplotlib.pyplot.show') as p:
            ks_test = KolmogorovSmirnov()
            axes = ks_test.draw_critical_values(ax=None, decimal_separator=",")
        ...
        with patch('matplotlib.pyplot.show') as p:
            ks_test = KolmogorovSmirnov()
            axes = ks_test.draw_critical_values(ax=None, decimal_separator=None)
            for text in axes.get_yticklabels():
                temp = text.get_text()
                result = True
                if ',' in temp:
