...
class DICOMSegmentationWriterOperator(Operator):
    ...
    def process_images(
        ...
        for study_selected_series in study_selected_series_list:
            if not isinstance(study_selected_series, StudySelectedSeries):
                raise ValueError("Element in input is not expected type, 'StudySelectedSeries'.")
            dicom_series = study_selected_series.selected_series[0].series
            break

        # The DICOM Seg instance file name is based on the input file name or series instance UID.
        prefix = input_path if input_path else dicom_series.SeriesInstanceUID
        output_filename = "{0}{1}{2}".format(
            os.path.splitext(os.path.basename(prefix))[0],
