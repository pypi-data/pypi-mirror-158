...
from paco.models import schemas, registry
...
do
    ...
    do
        ...
        ec2lm_async_run "" "docker pull ${!SOURCE_REPO_URI}${!SOURCE_REPO_NAME[$I]}:${!SOURCE_DEPLOY_TAG}"
    done
    ...
    do
        ...
        if [ "${!RELEASE_PHASE}" == 'true' ] ; then
            ENABLE_RELEASE_PHASE=true
            #run_command "docker tag ${!SOURCE_REPO_NAME}:${!SOURCE_DEPLOY_TAG} ${!DEST_REPO_NAME}:${DEPLOY_TAG}" "docker tag ${!SOURCE_REPO_URI}${!SOURCE_REPO_NAME}:${!SOURCE_DEPLOY_TAG} ${!DEST_REPO_URI}\
${!DEST_REPO_NAME}:${RELEASE_PHASE_TAG}"
            ...
            ec2lm_async_run "" "docker push ${!DEST_REPO_URI}${!DEST_REPO_NAME}:${RELEASE_PHASE_TAG}"
        fi
    done
...
function run_task() {
    ...
    #     aws logs get-log-events --log-group-name <LOG_GROUP_NAME> --log-stream-name <LOG_STREAM_NAME> --profile <PROFILE> |jq -r .events[].message
    #     break
    # fi
    ...
    do
        ...
        if [ $RET -ne 0 ] ; then
            ...
            echo "${ECHO_PREFIX}: run-task: status: $LAST_STATUS"
...
class DeploymentPipelineResourceEngine(ResourceEngine):
    ...
    def init_resource(self):
        ...
        if self.pipeline.configuration.account != None:
        ...
        if self.pipeline.configuration.artifacts_bucket != None:
...
"""
    ...
    def init_stage_action_codedeploy_deploy(self, action_config):
        ...
        if action_config.paco_service_hook != None and registry.CODEPIPELINE_CODEDEPLOY_INIT_STAGE_HOOK != None:
            # Paco Service CodeDeploy Overrides
            stack_extra_context = {
                'base_aws_name': self.base_aws_name,
                'app_name': self.app.name,
            ...
            registry.CODEPIPELINE_CODEDEPLOY_INIT_STAGE_HOOK(
                self.paco_ctx,
                action_config,
                self.resource,
                self.aws_region,
...
""")
    ...
    def add_notification_rules_stack_hooks(self, notification_rules_stack, rules_arn_ref_list):
        ...
        rules_config = {
            ...
            'environment_name': self.pipeline.paco_ref_parts.split('.')[2]
        }
...
"""
    ...
    def resolve_ref(self, ref):
        ...
        if schemas.IDeploymentPipelineDeployCodeDeploy.providedBy(ref.resource):
            ...
            # BitBucket Source
            if ref.resource_ref == 'codestar.connection.arn':
                return ref.resource._stack.template.get_github_codestar_connection_role_arn()
