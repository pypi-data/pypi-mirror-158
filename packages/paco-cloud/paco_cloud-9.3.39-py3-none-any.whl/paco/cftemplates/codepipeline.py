...
class CodePipeline(StackTemplate):
    def __init__(self, stack, paco_ctx, base_aws_name,
        ...
        self.codestar_connection_enabled = False
    ...
    def add_github_webhook(self, pipeline_res, stage_name, action, sourcebuilddeploy=False):
        ...
        if action.github_access_token == None:
            return
    ...
    def add_pipeline_service_role(self):
        ...
        if self.codestar_connection_enabled:
    ...
    def create_pipeline_from_sourcebuilddeploy(self, deploy_region):
        ...
        for action in self.pipeline.source.values():
            ...
            # GitHub Action
                ...
                if action.github_access_token == None:
                    self.create_codestar_connection_action('GitHub', action, source_stage_actions)
                else:
                    self.create_github_access_token_action(action, source_stage_actions)
            # ECR.Source Action
            ...
            # CodeCommit Action
                ...
                if action.owner == None:
                    action.owner = action.bitbucket_owner
                if action.repository == None:
                    action.repository = action.bitbucket_repository
                self.create_codestar_connection_action('Bitbucket', action, source_stage_actions)
    ...
    def init_deploy_stage(self, deploy_region):
        ...
        for action in self.pipeline.deploy.values():
            ...
            if action.type == 'CodeDeploy.Deploy':
                if action.paco_service_hook != None and registry.CODEPIPELINE_CODEDEPLOY_ACTION_HOOK != None:
                    codedeploy_deploy_action = registry.CODEPIPELINE_CODEDEPLOY_ACTION_HOOK(self, action)
                else:
    ...
    def get_github_codestar_connection_role_name(self):
        codestar_connection_role_name = f'{self.stack.get_name(self)}-GitHub-CodeStar-ConnectionRole'
        return self.create_resource_name(codestar_connection_role_name, filter_id='IAM.Role.RoleName', hash_long_names=True)
    ...
    def get_github_codestar_connection_role_arn(self):
        codestar_connection_role_name = self.get_github_codestar_connection_role_name()
        return f'arn:aws:iam::{self.account_ctx.get_id()}:role/{codestar_connection_role_name}'

    def get_codepipeline_name(self):
        return self.res_name_prefix

    # CodeStar Connection Action
    def create_codestar_connection_action(self, connection_type, action, source_stage_actions):
        if connection_type not in ['Bitbucket', 'GitHub']:
            raise
        self.codestar_connection_enabled = True
        print(f"{connection_type} CodeStar connection update URL: https://console.aws.amazon.com/codesuite/settings/connections")
        deploy_branch_name_param = self.create_cfn_parameter(
            param_type='String',
            name=f'{connection_type}DeploymentBranchName',
            description='The name of the branch where commits will trigger a build.',
            value=action.deployment_branch_name,
        )
        full_repository_id = f'{action.owner}/{action.repository}'
        if action.codestar_connection_arn == None:
            connection_arn_res = troposphere.codestarconnections.Connection(
                title=f"{connection_type}Connection",
                template=self.template,
            ...
            connection_arn_res = self.create_cfn_parameter(
                param_type='String',
                name=f'{connection_type}SourceCodeStarConnectionArn',
                description=f'The ARN of the CodeStart Connection to {connection_type}.',
                value=action.codestar_connection_arn,
            )

        if connection_type == 'Bitbucket':
            codestar_connection_role_name = self.get_bitbucket_codestar_connection_role_name()
        else:
            codestar_connection_role_name = self.get_github_codestar_connection_role_name()
                    ...
                    PolicyDocument=Policy(
                        ...
                        Statement=[
                            Statement(
                                ...
                                Resource=[troposphere.Ref(connection_arn_res)],
                            )
                        ]
                    )
                ),
        ...
        self.create_output(
            title=f'{connection_type}CodeStarConnectionRoleArn',
            description=f"{connection_type} CodeStar Connection Role ARN",
            value=troposphere.GetAtt(self.codestar_connection_role_res, 'Arn'),
            ref=action.paco_ref_parts + '.codestar.connection.arn'
        )

        source_action = troposphere.codepipeline.Actions(
            Name=f'{connection_type}',
            ActionTypeId = troposphere.codepipeline.ActionTypeId(
                Category = 'Source',
                Owner = 'AWS',
                Version = '1',
            ...
            Configuration = {
                'ConnectionArn': troposphere.Ref(connection_arn_res),
                'FullRepositoryId': full_repository_id,
                'BranchName': troposphere.Ref(deploy_branch_name_param),
                'DetectChanges': action.detect_changes
            },
            OutputArtifacts = [
                troposphere.codepipeline.OutputArtifacts(
                    Name = f'{connection_type}Artifact'
                )
            ],
            RunOrder = action.run_order,
            RoleArn = troposphere.GetAtt(self.codestar_connection_role_res, 'Arn')
        )
        source_stage_actions.append(source_action)
        self.build_input_artifacts.append(
            troposphere.codepipeline.InputArtifacts(
                Name=f'{connection_type}Artifact'
            )
    ...
    # GitHub Access Token Action
    def create_github_access_token_action(self, action, source_stage_actions):
