...
class ECRRepository(StackTemplate):
    def __init__(self, stack, paco_ctx):
        ...
        # Repository Name
        repository_name = self.resource.name
        if ecr_repository.repository_name != None:
            repository_name = ecr_repository.repository_name
        if ecr_repository.prefix_environment_name == True or ecr_repository.repository_name == None:
            reseng = self.resource._reseng
            repository_name = f'{reseng.env_ctx.netenv.name}-{reseng.env_ctx.env.name}-{reseng.app.name}-{self.resource_group_name}-{repository_name}'
        repo_dict = {
