...
class CWAlarms(CFBaseAlarm):
    ...
    def __init__(self, stack, paco_ctx):
        ...
        elif schemas.ILoadBalancer.providedBy(resource):
            for target_group in resource.target_groups.values():
                if target_group.monitoring == None or target_group.monitoring.is_enabled() == False:
                    continue
                if getattr(target_group, 'monitoring', None) == None or getattr(target_group.monitoring, 'alarm_sets', None) == None:
                    continue
                alarm_sets = target_group.monitoring.alarm_sets
                for alarm_set_name in alarm_sets.keys():
                    alarm_set = alarm_sets[alarm_set_name]
                    for alarm_name, alarm in alarm_set.items():
                        cfn_resource_name = 'AlarmTG{}{}{}'.format(
                            self.create_cfn_logical_id(target_group.name),
                            self.create_cfn_logical_id(alarm_set_name),
                            self.create_cfn_logical_id(alarm_name)
                        )
                        alarm_set[alarm_name].cfn_resource_name = cfn_resource_name
                        alarm._target_group = target_group
                        alarms.append(alarm_set[alarm_name])
        else:
            for alarm_set_name in alarm_sets.keys():
                alarm_set = alarm_sets[alarm_set_name]
    ...
    def add_alarms(self, alarms):
        ...
        # DBCluster with DBInstances
            ...
            dimension_parameters['LBFullName'] = self.create_cfn_parameter(
                name='DimensionResourceLoadBalancer',
                param_type='String',
                description='The resource id or name for the metric dimension.',
                value=f"{resource.paco_ref}.fullname"
            )
            if resource.target_groups != None:
                for target_group in resource.target_groups.values():
                    if target_group.monitoring == None or target_group.monitoring.is_enabled() == False:
                        continue

                    target_group_name = self.create_cfn_logical_id(f'MetricDimensionTGFullName'+target_group.name)
                    target_group_name_param = self.create_cfn_parameter(
                        name=target_group_name,
                        param_type='String',
                        description='The Target Group full name.',
                        value=target_group.paco_ref + '.fullname'
                    )
                    dimension_parameters['TGName'+target_group.name] = target_group_name_param
...
""".format(alarm.log_set_name, alarm.log_group_name, resource.name, resource.type, resource.paco_ref)
            ...
            if not schemas.ICloudWatchLogAlarm.providedBy(alarm):
                ...
                elif schemas.ILoadBalancer.providedBy(resource) and hasattr(alarm, '_target_group') == True:

                    dimensions.append(
                        {'Name': 'TargetGroup',
                        'Value': troposphere.Ref(dimension_parameters['TGName'+alarm._target_group.name])}
                    )
                    dimensions.append(
                        {'Name': 'LoadBalancer',
                        'Value': troposphere.Ref(dimension_parameters['LBFullName'])}
                    )
