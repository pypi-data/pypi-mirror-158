...
class Lambda(StackTemplate):
    def __init__(
        ...
        # SNS Topic Lambda permissions and subscription
        idx = 1
        # if awslambda.paco_ref_parts.startswith('service.antivirus.tools.us-west-2.antivirus.groups.lambda.resources.scan'):
        #    breakpoint()
        for s3_bucket in awslambda.s3_buckets:
            if is_ref(s3_bucket):
                s3_bucket_value = s3_bucket + '.arn'
            else:
                s3_bucket_value = s3_bucket
            # SNS Topic Arn parameters
            param_name = 'S3BucketArn%d' % idx
            self.create_cfn_parameter(
                name=param_name,
                param_type='String',
                description='A S3 Bucket ARN to grant permission to.',
                value=s3_bucket_value
            )
        ...
        for obj in get_all_nodes(app):
            ...
            if schemas.IEventsRule.providedBy(obj):
                ...
                for target in obj.targets:
                    if hasattr(target, 'target') == False:
                        continue
    ...
    def add_log_group(self, loggroup_name, logical_name=None):
        ...
        if self.awslambda.expire_events_after_days != 'Never' and self.awslambda.expire_events_after_days != '':
            expire_events_after_days = self.awslambda.expire_events_after_days
            # If nothing is set, default to 30 days
            if expire_events_after_days == None:
