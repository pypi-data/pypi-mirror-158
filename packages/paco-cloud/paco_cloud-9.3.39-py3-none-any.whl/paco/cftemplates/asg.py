...
class ASG(StackTemplate):
    def __init__(
        ...
        min_instances_param = self.create_cfn_parameter(
            param_type='String',
            name='MinInstances',
        ...
        asg_dict = {
            ...
            'MinSize': troposphere.Ref(min_instances_param),
        ...
        if asg_config.is_enabled() and asg_config.disable_target_groups == False:
            if asg_config.target_groups != None and len(asg_config.target_groups) > 0:
                ...
                for target_group_arn_ref in asg_config.target_groups:
                    lb_ref = '.'.join(target_group_arn_ref.split('.')[:-2])
                    lb_obj = get_model_obj_from_ref(lb_ref, self.paco_ctx.project)
                    target_group_obj = get_model_obj_from_ref(target_group_arn_ref, self.paco_ctx.project)
                    if lb_obj.is_enabled() == False or target_group_obj.is_enabled() == False:
                        continue
                    target_group_arn_param = self.create_cfn_parameter(
                        param_type='String',
                        name='TargetGroupARNs'+utils.md5sum(str_data=target_group_arn_ref),
                        description='A Target Group ARNs to attach to the ASG',
                        value=target_group_arn_ref+'.arn',
                    )
