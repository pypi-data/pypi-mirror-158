...
class StackTags():
    ...
    def has_kvp(self, key_in, value_in):
        for key, value in self.tags.items():
            if key_in == key and value_in == value:
                return True
        return False
    ...
    def stack_success(self):
        ...
        if self.action == "update":
            if self.paco_ctx.hooks_only == True:
                return
            if self.paco_ctx.alarms_only == True and self.tags and self.tags.has_kvp('Paco-Resource-Sub-Type', 'Alarms') == False:
                return
...
A Stack can cache it's templates to the filesystem or check them against AWS and get their status.
    ...
    def update_stack(self):
        ...
        run_hooks = True
        # Alarms Only
        if self.paco_ctx.alarms_only == True:
            run_hooks = False
            if self.tags == None or self.tags.has_kvp('Paco-Resource-Sub-Type', 'Alarms') == False:
                suppress = True
        ...
        if suppress:
            self.log_action("Update", "Suppressed")
            return
    ...
    def wait_for_complete(self):
        ...
        while True:
            ...
            if self.action == "create":
                ...
                if self.paco_ctx.alarms_only == False:
