...
#include "distribution_sampler_win.h"
...
DLLEXPORT void _sample_gamma(float* shape_host, float* scale_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
...
//    printf("matrix_scale:%zu, require:%zu  grid:%zu, block:%d\n\n", matrix_scale, nElems, size_t(ceil(float(sst_device ->nElems) / float(nThreads_gamma))), nThreads_gamma); //=========================
...
DLLEXPORT void _sample_standard_gamma(float* shape_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    float* shape_device;
    size_t nBytes = matrix_scale * sizeof(float);
    cudaMalloc((void**)&shape_device, nBytes);
    cudaMemcpy(shape_device, shape_host, nBytes, cudaMemcpyHostToDevice);
...
DLLEXPORT void _sample_beta(float* a_host, float* b_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
...
DLLEXPORT void _sample_standard_normal(void* output, size_t nElems, curandStateXORWOW_t* status) {
    float* output_device;
...
DLLEXPORT void _sample_normal(float* loc_host, float* scale_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
...
DLLEXPORT void _sample_standard_uniform(void* output, size_t nElems, curandStateXORWOW_t* status) {
    float* output_device;
    size_t nBytes = nElems * sizeof(float);
    cudaMalloc((void**)&output_device, nBytes);
...
DLLEXPORT void _sample_uniform(float* low_host, float* high_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
...
DLLEXPORT void _sample_negative_binomial(int* r_host, float* p_host, int* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    int* r_device;
...
DLLEXPORT void _sample_poisson(float* lam_host, int* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    float* lam_device;
    size_t nBytes = matrix_scale * sizeof(float);
...
DLLEXPORT void _sample_crt(float* point_host, float* p_host, int* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
...
DLLEXPORT void _sample_cauchy(float* loc_host, float* scale_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    float* loc_device, * scale_device;
    size_t nBytes = matrix_scale * sizeof(float);
    cudaMalloc((void**)&loc_device, nBytes);
    cudaMalloc((void**)&scale_device, nBytes);
...
DLLEXPORT void _sample_standard_cauchy(float* output, size_t nElems, curandStateXORWOW_t* status) {
    float* output_device;
    size_t nBytes = nElems * sizeof(float);
    cudaMalloc((void**)&output_device, nBytes);
...
DLLEXPORT void _sample_chisquare(int* degrees_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    int* degrees_device;
    size_t nBytes = matrix_scale * sizeof(int);
...
DLLEXPORT void _sample_noncentral_chisquare(int* df_host, float* nonc_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    int* df_device;
    size_t nBytes = matrix_scale * sizeof(int);
...
DLLEXPORT void _sample_exponential(float* Lambda_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    float* Lambda_device;
    size_t nBytes = matrix_scale * sizeof(float);
...
DLLEXPORT void _sample_f(int* n1_host, int* n2_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
...
DLLEXPORT void _sample_noncentral_f(int* dfnum_host, int* dfden_host, float* nonc_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
...
DLLEXPORT void _sample_geometric(float* p_host, int* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    float* p_device;
    size_t nBytes = matrix_scale * sizeof(float);
    cudaMalloc((void**)&p_device, nBytes);
    cudaMemcpy(p_device, p_host, nBytes, cudaMemcpyHostToDevice);
...
DLLEXPORT void _sample_gumbel(float* loc_host, float* scale_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    float* loc_device, * scale_device;
    size_t nBytes = matrix_scale * sizeof(float);
    cudaMalloc((void**)&loc_device, nBytes);
    cudaMalloc((void**)&scale_device, nBytes);
...
DLLEXPORT void _sample_hypergeometric(int* ngood_host, int* nbad_host, int* nsample_host, int* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
...
DLLEXPORT void _sample_laplace(float* loc_host, float* scale_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    float* loc_device, * scale_device;
    size_t nBytes = matrix_scale * sizeof(float);
    cudaMalloc((void**)&loc_device, nBytes);
    cudaMalloc((void**)&scale_device, nBytes);
...
DLLEXPORT void _sample_logistic(float* loc_host, float* scale_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    float* loc_device, * scale_device;
    size_t nBytes = matrix_scale * sizeof(float);
    cudaMalloc((void**)&loc_device, nBytes);
    cudaMalloc((void**)&scale_device, nBytes);
...
DLLEXPORT void _sample_power(float* a_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    float* a_device;
    size_t nBytes = matrix_scale * sizeof(float);
    cudaMalloc((void**)&a_device, nBytes);
    cudaMemcpy(a_device, a_host, nBytes, cudaMemcpyHostToDevice);
...
DLLEXPORT void _sample_zipf(float* a_host, int* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    float* a_device;
    size_t nBytes = matrix_scale * sizeof(float);
    cudaMalloc((void**)&a_device, nBytes);
    cudaMemcpy(a_device, a_host, nBytes, cudaMemcpyHostToDevice);
...
DLLEXPORT void _sample_pareto(float* k_host, float* xm_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
...
DLLEXPORT void _sample_rayleigh(float* scale_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    float* scale_device;
    size_t nBytes = matrix_scale * sizeof(float);
    cudaMalloc((void**)&scale_device, nBytes);
    cudaMemcpy(scale_device, scale_host, nBytes, cudaMemcpyHostToDevice);
...
DLLEXPORT void _sample_t(float* df_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    float* df_device;
    size_t nBytes = matrix_scale * sizeof(float);
    cudaMalloc((void**)&df_device, nBytes);
    cudaMemcpy(df_device, df_host, nBytes, cudaMemcpyHostToDevice);
...
DLLEXPORT void _sample_triangular(float* left_host, float* mode_host, float* right_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
...
DLLEXPORT void _sample_weibull(float* shape_host, float* scale_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    float* shape_device, * scale_device;
    size_t nBytes = matrix_scale * sizeof(float);
    cudaMalloc((void**)&shape_device, nBytes);
    cudaMalloc((void**)&scale_device, nBytes);
...
DLLEXPORT void _sample_multinomial(int* count_host, float* prob_host, int* output, size_t matrix_scale_1, size_t matrix_scale_2, size_t repeats, curandStateXORWOW_t* status) {
    int* count_device;
    float* prob_device;
