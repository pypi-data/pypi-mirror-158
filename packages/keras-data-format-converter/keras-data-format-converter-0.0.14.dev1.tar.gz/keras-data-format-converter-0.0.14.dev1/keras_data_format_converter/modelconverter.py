...
class ModelConverter:
    ...
    def _default_call_tf_op_lambda_layer(
        ...
        for i, arg in enumerate(call_args):
            if hasattr(arg, 'name'):
                try:
                    if arg.name == node_input_tensors[0].name:
                        new_tfoplambda_args[i] = node_input_tensors.pop(0)
                except IndexError:
                    pass
        ...
        for key, arg in call_kwargs.items():
            if hasattr(arg, 'name'):
                try:
                    if arg.name == node_input_tensors[0].name:
                        new_tfoplambda_kwargs[key] = node_input_tensors.pop(0)
                except IndexError:
                    pass
    ...
    def _call_tf_op_lambda_layer(
        ...
        last_input_axis_index = len(current_layer.input_shape) - 1
