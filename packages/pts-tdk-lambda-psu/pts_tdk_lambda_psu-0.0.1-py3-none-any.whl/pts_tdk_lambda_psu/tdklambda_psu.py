...
logging.basicConfig(filename='tdk_psu_log.txt',
                    filemode='a',
...
class TDKLambdaPSU:
    ...
    def open_connection(self):
        ...
        try:
            ...
            logging.info(f": ERROR {e}: Could not open Resource")
            exit()
    ...
    def self_test(self):
        ...
        if self_test is None:
            ...
            assert AssertionError(f'ERROR in Self-test: Contact Customer Service at TDK-Lambda')
            exit()
    ...
    def system_info(self):
        ...
        sys_info = self.psu.query('*IDN?')
        logging.info(f": System Info of PSU: {sys_info}")
        ...
        baud_rate = self.psu.query(f'SYST:COMM:BAUD?')
        logging.info(f": Baud rate of PSU: {baud_rate}")
        ...
        firmware_version = self.psu.query(f'SYST:FIRM?')
        logging.info(f": Firmware version of PSU: {firmware_version}")
    ...
    def switch_display_window(self, disp_state):
        """
    ...
    def check_max_min_programmable_volt(self):
        """
    ...
    def check_max_min_programmable_current(self):
        """
    ...
    def set_program_output_voltage(self, volt_in):
        """
        ...
        if self.psu.query(f'OUTP:STAT?') == 0:
            ...
            # Setting the voltage and current works only in the CC and CV modes
            source_mode = self.psu.query(f'SOUR:MOD?')
            if source_mode == 'CV':
                self.psu.query(f':VOLT {volt_in}')
                logging.info(f": Program Output Voltage Set to : {volt_in} Volts")
                return True
            else:
                raise OSError("FAIL: Please set the PSU to CV mode before setting Voltage.")

    def get_program_output_voltage(self):
        """
        ...
        """
        if self.psu.query(f'SYST:ERR?') is None:
    ...
    def set_program_output_current(self, curr_in):
        """
        ...
        if self.psu.query(f'OUTP:STAT?') == 0:
            ...
            if source_mode == 'CC':
                ...
                logging.info(f": Program Output Current set to: {curr_in} Amps")
                return True
            else:
                raise OSError(f"FAIL: Please set the PSU to CC mode before setting Current.")

    def get_program_output_current(self):
        """
