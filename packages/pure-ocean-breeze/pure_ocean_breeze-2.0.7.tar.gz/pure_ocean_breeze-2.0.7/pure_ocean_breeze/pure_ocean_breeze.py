...
def database_update_minute_files(startdate=None,enddate=None):
    ...
    if startdate:
        ...
        logger.info(f'上次更新到{datetime.datetime.strftime(pd.Timestamp(startdate)-pd.Timedelta(days=1))}')
    if enddate:
        ...
    else:
        enddate=datetime.datetime.now()+pd.Timedelta(days=1)
        enddate=datetime.datetime.strftime(enddate,'%Y%m%d')
        logger.info(f'本次将更新到{datetime.datetime.strftime(pd.Timestamp(enddate)-pd.Timedelta(days=1))}')
    ...
    config.dump()
    logger.success(f'分钟数据已更新，现在最新的是{datetime.datetime.strftime(pd.Timestamp(enddate)-pd.Timedelta(days=1))}')
...
def database_update_daily_files(startdate=None,enddate=None):
    ...
    logger.success(f'日频数据已更新，现在最新的是{datetime.datetime.strftime(pd.Timestamp(enddate)-pd.Timedelta(days=1))}')
