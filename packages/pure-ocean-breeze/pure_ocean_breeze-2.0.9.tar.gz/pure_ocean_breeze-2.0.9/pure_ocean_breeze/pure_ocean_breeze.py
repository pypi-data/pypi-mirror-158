...
class HomePlace(object):
    __slots__ = ['daily_data_file','minute_data_file','factor_data_file','barra_data_file','update_data_file',
                 'api_token','final_factor_file','daily_enddate','minute_enddate']
    def __init__(self):
...
class pure_moonson(object):
    ...
    def __init__(self,fac,group_num=5):
        homeplace=HomePlace()
        pindu=pd.read_feather(homeplace.daily_data_file+'各行业行情数据.feather').set_index('date').resample('M').last()
...
class to_one(object):
    ...
    def __init__(self,minute_data):
        ...
        self.df.columns=['index','high','open','close','volume','windcode','low','amount']
    ...
    def unify_dataframe(self,df):
        ...
        df=df[['index','windcode','open','high','low','close','volume','amount']]
...
def download_single_minute(stock,startdate,enddate):
    try:
        df=pro.stk_mins(code=stock,freq='1min',start_date=startdate,end_date=enddate,
...
def database_update_minute_files(startdate=None,enddate=None):
    '''
    ...
    if startdate:
        ...
        logger.info(f"上次更新到{datetime.datetime.strftime(pd.Timestamp(startdate)-pd.Timedelta(days=1),format='%Y-%m-%d')}")
    if enddate:
        ...
    else:
        enddate=datetime.datetime.now()+pd.Timedelta(days=1)
        enddate=datetime.datetime.strftime(enddate,'%Y%m%d')
        logger.info(f"本次将更新到{datetime.datetime.strftime(pd.Timestamp(enddate)-pd.Timedelta(days=1),format='%Y-%m-%d')}")
    ...
    config.dump()
    logger.success(f"分钟数据已更新，现在最新的是{datetime.datetime.strftime(pd.Timestamp(enddate)-pd.Timedelta(days=1),format='%Y-%m-%d')}")
...
def download_single_daily(day):
    ...
    try:
        #8个价格，交易状态，成交量，
        df1=pro.a_daily(trade_date=day,
                        fields=['code','trade_date','open','high','low','close','volume','adjopen','adjclose','adjhigh','adjlow','tradestatus'])
        #换手率，流通股本，换手率要除以100，流通股本要乘以10000
        df2=pro.daily_basic(trade_date=day,
...
def database_update_daily_files(startdate=None,enddate=None):
    '''
    ...
    def read_mat(path):
        ...
        index=list(scio.loadmat(suffix_path_save+'TradingDate_Daily.mat').values())[3]
        col=[i[0] for i in col[0]]
        if len(index)>1000:
    ...
    def to_mat(df,row,filename=None,ind='trade_date',col='code'):
        ...
        new=new[new.index<=int(enddate)]
        scio.savemat(suffix_path_save+filename,{'data':new.to_numpy()},do_compression=True)
    ...
    part2_new=part2_new[part2_new.index<=int(enddate)]
    # part2_new=part2_new.dropna(how='all',axis=1)
    part2_new=part2_new[closes.columns]
    scio.savemat(suffix_path_save+'AllStock_DailyTR.mat',{'data':part2_new.to_numpy()},do_compression=True)
    ...
    part3=part3.set_index(['CHANGE_DT','S_INFO_WINDCODE']).unstack()
    part3.columns=[i[1] for i in list(part3.columns)]
    part3=part3*10000
    part3_0=pd.DataFrame(0,columns=part2.columns,index=part2.index)
    part3_0=part3_0+part3
    part3_nan=part3_0.replace(0,np.nan)
    part3_old=read_mat('AllStock_DailyAShareNum.mat')
    part3_new=pd.concat([part3_old,part3_nan])
    part3_new=part3_new.fillna(method='ffill')
    part3_new=part3_new[part3_new.index.isin(list(part2_new.index))]
    part3_new=part3_new.T
    part3_new=part3_new[part3_new.index.isin(list(part2_new.columns))]
    part3_new=part3_new.T
    # part3_new=part3_new.dropna(how='all',axis=1)
    part3_new=part3_new[closes.columns]
    scio.savemat(suffix_path_save+'AllStock_DailyAShareNum.mat',{'data':part3_new.to_numpy()},do_compression=True)
    logger.success('流通股数更新完成')

    #st
    part4=df3[['s_info_windcode','entry_dt','remove_dt']]
    part4=part4.sort_values('s_info_windcode')
    part4.remove_dt=part4.remove_dt.fillna(int(enddate)).astype(int)
    part4=part4.set_index('s_info_windcode').stack()
    part4=part4.reset_index().assign(he=sorted(list(range(int(part4.shape[0]/2)))*2))
    ...
    part4=part4.drop_duplicates(subset=['date','code']).pivot(index='date',columns='code',values='st')
    ...
    scio.savemat(suffix_path_save+'AllStock_DailyST.mat',{'data':part4_0.to_numpy()},do_compression=True)
    logger.success('st更新完了')

    #上市天数
    part5_close=pd.read_feather(suffix_path+'BasicFactor_Close.txt').set_index('index')
    ...
    scio.savemat(suffix_path_save+'AllStock_DailyListedDate.mat',{'data':part5.to_numpy()},do_compression=True)
    logger.success('上市天数更新完了')

    #交易日历和股票代码
    scio.savemat(suffix_path_save+'TradingDate_Daily.mat',{'data':part2_new.index.to_numpy()},do_compression=True)
    scio.savemat(suffix_path_save+'AllStockCode.mat',{'data':part2_new.columns.to_numpy()},do_compression=True)
    enddate=pd.Timestamp(enddate)+1
    enddate=datetime.datetime.strftime(enddate,'%Y%m%d')
    config.set('daily_enddate',enddate)
    config.dump()
    logger.success('交易日历和股票代码更新完了')
    logger.success(f"日频数据已更新，现在最新的是{datetime.datetime.strftime(pd.Timestamp(enddate)-pd.Timedelta(days=1),format='%Y-%m-%d')}")
