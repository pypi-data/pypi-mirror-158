...
def initialize():
    ...
    while '/' not in update_data_file:
        print("请不要输入反斜杠'\'，请替换为'/'，并以'/'结尾")
        barra_data_file=input("请设置更新辅助数据存放路径(请最终以斜杠结尾，请不要输入反斜杠'\',请都替换为'/')：")
    #最终因子成果路径
    final_factor_file=input("请设置最终因子成果存放路径(请最终以斜杠结尾，请不要输入反斜杠'\',请都替换为'/')：")
    while '/' not in final_factor_file_data_file:
        print("请不要输入反斜杠'\'，请替换为'/'，并以'/'结尾")
        final_factor_file=input("请设置最终因子成果存放路径(请最终以斜杠结尾，请不要输入反斜杠'\',请都替换为'/')：")
    #数立方token
    ...
    save_dict={'daily_data_file':daily_data_file,'minute_data_file':minute_data_file,
               ...
               'update_data_file':update_data_file,'final_factor_file':final_factor_file,
               'api_token':api_token,'daily_enddate':daily_enddate,'minute_enddate':minute_enddate}
