...
def batch_process_videos_on_colab(bd, local_temp_dir, video_batch_size=2):
    ...
    while True:  # keep going until there are no more files to process
        ...
        process_these_videos = np.where(file_dict['is_processed'] == False)[0][:video_batch_size] #fix001 - make not equal to True so that skip string can be in there
...
def combine_final_h5s(h5_file_list_to_combine, delete_extra_files=False):
    ...
    overwrite_h5_key(fn, 'template_img', image_tools.get_h5_key_and_concatenate(h5_file_list_to_combine[0], 'template_img'))
    overwrite_h5_key(fn, 'multiplier', image_tools.get_h5_key_and_concatenate(h5_file_list_to_combine[0], 'multiplier'))
    overwrite_h5_key(fn, 'trial_nums_and_frame_nums', image_tools.get_h5_key_and_concatenate(h5_file_list_to_combine[0], 'trial_nums_and_frame_nums'))
