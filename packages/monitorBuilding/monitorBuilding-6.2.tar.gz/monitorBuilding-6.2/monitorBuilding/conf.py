...
def __vmuc_mbmap():
    vmuc_modebus_map = __parseXML_VMUC(FILE_VMUC_XML)
    vmuc_modebus_map = __makemodebus_mapUnique(vmuc_modebus_map)
    ############ keep only variables of interest
    var_vmuc = ['kW _sys','kWh','kW L1', 'kW L2', 'kW L3','kVA L1', 'kVA L2', 'kVA L3','kWh L1', 'kWh L2', 'kWh L3']
...
def quick_modbus_reading():
    ...
    _result = client.read_holding_registers(0, 2, unit=1)
    decoder = BinaryPayloadDecoder.fromRegisters(_result._registers, byteorder=Endian.Little, wordorder=Endian.Little)
...
def quick_modbus_okwind_reading():
    ...
    _result = client.read_holding_registers(1000, 2, unit=UNIT)
    decoder = BinaryPayloadDecoder.fromRegisters(_result._registers, byteorder=Endian.Big, wordorder=Endian.Big)
    plant_production = decoder.decode_32bit_uint() / 10 # divide by the gain
    plant_production = decoder.decode_32bit_uint() / 10 # divide by the gain

    _result = client.read_holding_registers(1021, 1, unit=UNIT)
    decoder = BinaryPayloadDecoder.fromRegisters(_result._registers, byteorder=Endian.Big, wordorder=Endian.Big)
...
def generate_conf_monitoring():
    print('generating configuration files and store in :'+_FILE_CONF_MONITORING_PKL)
    start=_time.time()
    plcs,modebus_maps={},{}
...
# del _os,_glob,_re,_pd,_sys,_pickle,_time,baseFolder,Meteo_Client,_monitoring_objs
del Meteo_Client,Utils
