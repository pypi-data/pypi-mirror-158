...
from pyrasgo.utils import naming, rendering
from pyrasgo.utils import exports
from pyrasgo.constants import DS_PUBLISHED_STATUS, SOURCE_TABLE_ARG_NAME, TABLE_ARG_TYPE, TABLE_LIST_ARG_TYPE
...
class Dataset(Connection):
    ...
    def schema(self) -> Optional[List[Tuple[str, str]]]:
        """
        Return Dataset Schema has list of LIst of Tuples like (<Col Name>, <Col Data Type>)
        """
        if self._api_dataset:
            data_warehouse = dw_connection(self._dw_type())
            return data_warehouse.list_table_columns(self._api_dataset.dw_table.fqtn)
    ...
    def to_df(
        ...
        if self._operations:
            sql = rendering.offline_operations_as_cte(
                operations=self._operations, transforms=self._available_transforms
            )
            if filters:
            ...
            if columns:
                final_op_name = naming.gen_operation_table_name(
    ...
    def run_stats(self, only_if_data_changed: Optional[bool] = True) -> None:
        ...
        Create()._dataset_correlation_stats(
            table_id=self._api_dataset.dw_table_id, only_if_data_changed=only_if_data_changed
        )
        print(
    ...
    def take_snapshot(self) -> List:
        """
        Take a snapshot of this dataset
        """
        from pyrasgo.api import Create

        Create().dataset_snapshot(self.id)
        self._refresh()
    ...
    def _get_cte_for_dataset(self) -> str:
        ...
        if self._api_operation_set and self._api_operation_set.operations:
            return rendering.operations_as_cte(self._api_operation_set.operations)
        # If this is a published Dataset without operations:
        if self._api_dataset and self._api_dataset.dw_table:
            return f'SELECT * FROM {self._api_dataset.dw_table.fqtn}'
        # If this is an Offline Dataset:
        # For now: return None | Future: build an offline CTE rendering function
        else:
            return rendering.offline_operations_as_cte(
                operations=self._operations, transforms=self._available_transforms
            )
        ...
        if self._api_dataset:
            if not self._api_dataset.dw_table_id or self.status != DS_PUBLISHED_STATUS:
                raise errors.RasgoRuleViolation(
                    f"Dataset({self._api_dataset.id}) has not been published, "
    ...
    def _assert_is_published(self) -> None:
        ...
        """
        if self.status != DS_PUBLISHED_STATUS or not self._api_dataset.dw_table_id:
            raise errors.RasgoRuleViolation(
