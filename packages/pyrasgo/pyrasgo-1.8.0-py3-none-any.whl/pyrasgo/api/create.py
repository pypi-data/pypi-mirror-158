...
class Create:
    ...
    def dataset_snapshot(self, id: int) -> str:
        """
        Takes a snapshot of a dataset

        Args:
            id: Id of dataset to snapshot
        """
        self.api._post(f"/datasets/snapshots", _json={"datasetId": id}, api_version=2)
        return (
            "Sent request to build dataset snapshot. "
            f"Run rasgo.get.dataset({id}).snapshots to see available snapshots."
        )
    ...
    def _dataset(
        ...
        *,
        ...
        if not publish_ds_table_type or publish_ds_table_type.upper() not in ("TABLE", "VIEW"):
            raise errors.ParameterValueError("table_type", ["TABLE", "VIEW"])
        ...
        for i in range(1, MAX_POLL_ATTEMPTS):
            ...
            if status_tracking.status == "failed":
                raise errors.APIError(f"Could not publish dataset: {status_tracking.message}")
