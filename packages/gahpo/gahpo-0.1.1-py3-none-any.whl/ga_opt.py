...
class GeneticCOFOptimization(object):
    ...
    def __init__(self, blackbox_func, pbounds, cspace=10):
...
#         print("done", self.best_chromosome)
    ...
    def max(self):
        return self.parse_params_array(self.best_chromosome), self.best_score
    ...
    def run_generation(self, population, scores):
        ...
        if scores is None :
            scores = []
            local_optimal_score = 0 if self.mode == 'max' else 90000000
            for chromosome_array in population:
                score = self.fitness(self.parse_params_array(chromosome_array))
        ...
        while len(population) < self.num_population*2:
            ...
            if self.is_inspected(offspring1) == False:
                ...
                score = self.fitness(self.parse_params_array(offspring1))
                self.fitness_call_counter += 1
#                 print("offspring1", offspring1, "score", score)
            ...
            if self.is_inspected(offspring2) == False:
                ...
                score = self.fitness(self.parse_params_array(offspring2))
                self.fitness_call_counter += 1
        ...
        while len(candidates) < 5:
            ...
            for chromo in random_population:
                ...
                if is_optimal and self.is_inspected(chromo) is False:
                    ...
                    score = self.fitness(self.parse_params_array(chromo))
#                     print("y_hat", y_hat, chromo, score, self.parse_params_array(chromo))
                    self.fitness_call_counter += 1
#                     print("cof", chromo, score)
                    scores.append(score)
...
#         print("cof max score", np.max(self.all_score))            
    ...
    def run_generations(self):
        ...
        best_score = 0 if self.mode == 'max' else 9000000
...
#                 self.update_log(None, local_max_score, -1)
                ...
                if self.optimal_score is not None and self.optimal_score == local_optimal_score:
                    print("Searched optimal early", best_chromosome)
                    break;
