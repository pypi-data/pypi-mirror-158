...
class Global_max(object):
    ...
    def Augmented_EI(self, alpha = 1, tao = 0):
        ...
        :param tao: noise standard deviation, default 1
        """
        # cal current optimal 
    ...
    def Reinterpolation_EI(self, ):
        ...
        for i in range(len(self.virtual_samples_mean)):
            Var_Z = ( self.virtual_samples_mean[i]- cur_optimal_value)/self.virtual_samples_std[i] 
            REI = (self.virtual_samples_mean[i]- cur_optimal_value) * norm.cdf(Var_Z)+ self.virtual_samples_std[i] * norm_des(Var_Z)
            REI_list.append(REI)
    ...
    def PoI(self, tao = 0):
        ...
        """
        if tao <= 0:
            print('please input a postive value of tao')
        else:
            pass
