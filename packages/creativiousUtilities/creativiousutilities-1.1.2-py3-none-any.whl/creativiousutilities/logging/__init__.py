...
class Logger:
    def __init__(self, name: str, subname: str, debug: bool = False, logfile: str = None, logAllFile: str = None):
        """Logger(
        ...
        if logAllFile is not None:
            ...
            self.allFileHandler.setFormatter(
                logging.Formatter(f'%(levelname)s [%(asctime)s] ({name}:{subname}) %(message)s',
                                  datefmt='%m/%d/%Y %H:%m:%S'))
        self.handler.setFormatter(
            colorlog.ColoredFormatter(f"%(log_color)s %(levelname)s [%(asctime)s] ({name}:{subname}) %(message)s",
                                      datefmt='%m/%d/%Y %H:%m:%S'))
        self.debug = debug
