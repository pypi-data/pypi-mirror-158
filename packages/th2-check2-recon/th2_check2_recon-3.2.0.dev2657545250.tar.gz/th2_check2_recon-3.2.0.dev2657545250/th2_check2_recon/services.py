...
class EventStore(AbstractService):
    ...
    def _get_attached_message_ids(self, *recon_msgs):
        try:
            return [json.loads(message.proto_message)['metadata']['id'] for message in recon_msgs]
        except KeyError:
            return []
...
class MessageComparator(AbstractService):
    ...
    def compare_messages(self, messages: [ReconMessage],
        ...
        compare_result = self.compare(dict_to_message(json.loads(messages[0].proto_message)), dict_to_message(json.loads(messages[1].proto_message)), settings)
