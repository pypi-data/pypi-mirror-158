...
class AscendArray():
    ...
    # 10. image groud-truth width
    @property
    def w(self):
        """Get the groundtruth width of image.

        .. Note::
            The method only support to get the width of YUV/RGB Image info.

        Args:
            None.

        Returns:
            [int] : The groundtruth width of image.
        """
        if self._format == const.PIXEL_FORMAT_YUV_SEMIPLANAR_420 or \
            self._format == const.PIXEL_FORMAT_YVU_SEMIPLANAR_420:
            return self._shape[1] 


    # 11. image groud-truth height
    @property
    def h(self):
        """Get the groundtruth height of image.

        .. Note::
            The method only support to get the height of YUV/RGB Image info.

        Args:
            None.

        Returns:
            [int] : The groundtruth height of image.
        """
        if self._format == const.PIXEL_FORMAT_YUV_SEMIPLANAR_420 or \
            self._format == const.PIXEL_FORMAT_YVU_SEMIPLANAR_420:
            return self._shape[0] * 2 // 3
    ...
    def to_numpy(self, nbytes=None):
        ...
        if self._flag != 'HOST':
            ...
            ptr = malloc(_nbytes, 'HOST')
            memcpy_d2h(ptr, self._data, _nbytes)
        else:
            ptr = self._data

        try:
            array = acl.util.ptr_to_numpy(ptr, self._shape, np_type)
        except:
            bytes_data = acl.util.ptr_to_bytes(ptr, _nbytes)
            array = np.frombuffer(bytes_data, dtype=np_type).reshape(self._shape)
