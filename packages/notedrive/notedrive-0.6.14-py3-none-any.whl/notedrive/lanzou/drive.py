...
class LanZouDrive(LanZouCloud):
    ...
    def _upload_small_file(self, *args, **kwargs) -> int:
        def callback(file_name, total_size, now_size):
            global progress
            if progress is None:
                progress = tqdm(unit='B', unit_scale=True, unit_divisor=1024, desc=file_name, total=total_size)
            progress.update(now_size)

        return super(LanZouDrive, self)._upload_small_file(*args, callback=callback, **kwargs)

    def down_file_by_url(self, *args, **kwargs) -> int:
        """通过分享链接下载文件(需提取码)
        ...
        return super(LanZouDrive, self).down_file_by_url(*args, callback=callback, **kwargs)
    ...
    def sync_files(self, path_root, folder_id, only_directory=False, overwrite=False, filter_fun=None,
                   remove_local=False):
        """
        将本地的文件同步到云端，单向同步
        ...
        for file in (os.listdir(path_root)):
            ...
            if os.path.isdir(local_path):
                ...
                file_dict.update(self.sync_files(local_path, yun_id, only_directory=only_directory,
                                                 overwrite=overwrite, filter_fun=filter_fun, remove_local=remove_local))
            else:
                # 只同步文件夹
                if only_directory:
                    continue
    ...
    def sync_directory(self, path_root, folder_id):
        return self.sync_files(path_root, folder_id, only_directory=False)
