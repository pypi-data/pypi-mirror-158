...
cpdef double addme(float[:,:] array,double[:,:,:] rgb,double ox, double oy, double dx, double dy,int jstart,int jend,int istart,int iend, int cursize, double nullvalue):
    ...
    for j in range(jstart,jend,cursize):
        for i in range(istart,iend,cursize):
            if array[i,j]!=nullvalue :            
                ...
                glBegin(GL_QUADS)
                glColor3f(rgb[i,j,0],rgb[i,j,1],rgb[i,j,2])
                glVertex2d(x,y)
...
@cython.boundscheck(False)  # Deactivate bounds checking
@cython.wraparound(False)   # Deactivate negative indexing.
cpdef double addmeall(float[:,:] array,double[:,:,:] rgb,double ox, double oy, double dx, double dy,int jstart,int jend,int istart,int iend, int cursize, double nullvalue):
