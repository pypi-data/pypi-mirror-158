...
from bson303.binary import (ALL_UUID_REPRESENTATIONS,
                         PYTHON_LEGACY,
...
class CodecOptions(_options_base):
    ...
    :Parameters:
      ...
      - `uuid_representation`: The BSON representation to use when encoding
        ...
        :data:`~bson303.binary.PYTHON_LEGACY`.
    """
    ...
    def __new__(cls, document_class=dict,
        ...
        if not issubclass(document_class, MutableMapping):
            raise TypeError("document_class must be dict, bson303.son.SON, or "
                            "another subclass of collections.MutableMapping")
        if not isinstance(tz_aware, bool):
            raise TypeError("tz_aware must be True or False")
        if uuid_representation not in ALL_UUID_REPRESENTATIONS:
            raise ValueError("uuid_representation must be a value "
                             "from bson303.binary.ALL_UUID_REPRESENTATIONS")
