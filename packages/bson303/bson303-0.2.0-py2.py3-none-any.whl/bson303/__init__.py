...
from bson303.binary import (Binary, OLD_UUID_SUBTYPE,
                         JAVA_LEGACY, CSHARP_LEGACY,
                         UUIDLegacy)
from bson303.code import Code
from bson303.codec_options import CodecOptions, DEFAULT_CODEC_OPTIONS
from bson303.dbref import DBRef
from bson303.errors import (InvalidBSON,
                         InvalidDocument,
                         InvalidStringData)
from bson303.int64 import Int64
from bson303.max_key import MaxKey
from bson303.min_key import MinKey
from bson303.objectid import ObjectId
from bson303.py3compat import (b,
                            PY3,
                            iteritems,
                            text_type,
                            string_type,
                            reraise)
from bson303.regex import Regex
from bson303.son import SON, RE_TYPE
from bson303.timestamp import Timestamp
from bson303.tz_util import utc


try:
    from bson303 import _cbson
    _USE_C = True
except ImportError:
    _USE_C = False
...
def _get_object(data, position, obj_end, opts):
    """Decode a BSON subdocument to opts.document_class or bson303.dbref.DBRef."""
...
def _get_binary(data, position, dummy, opts):
    """Decode a BSON binary to bson303.binary.Binary or python UUID."""
...
def _get_oid(data, position, dummy0, dummy1):
    """Decode a BSON ObjectId to bson303.objectid.ObjectId."""
    end = position + 12
...
def _get_regex(data, position, dummy0, dummy1):
    """Decode a BSON regex to bson303.regex.Regex or a python pattern object."""
...
def _get_ref(data, position, obj_end, opts):
    """Decode (deprecated) BSON DBPointer to bson303.dbref.DBRef."""
...
def _get_timestamp(data, position, dummy0, dummy1):
    """Decode a BSON timestamp to bson303.timestamp.Timestamp."""
    end = position + 8
...
def _get_int64(data, position, dummy0, dummy1):
    """Decode a BSON int64 to bson303.int64.Int64."""
    end = position + 8
...
def _encode_objectid(name, value, dummy0, dummy1):
    """Encode bson303.objectid.ObjectId."""
...
def _encode_regex(name, value, dummy0, dummy1):
    """Encode a python regex or bson303.regex.Regex."""
    flags = value.flags
...
def _encode_timestamp(name, value, dummy0, dummy1):
    """Encode bson303.timestamp.Timestamp."""
...
def _encode_minkey(name, dummy0, dummy1, dummy2):
    """Encode bson303.min_key.MinKey."""
    return b"\xFF" + name


def _encode_maxkey(name, dummy0, dummy1, dummy2):
    """Encode bson303.max_key.MaxKey."""
    return b"\x7F" + name
...
def decode_all(data, codec_options=DEFAULT_CODEC_OPTIONS):
    ...
    .. versionchanged:: 3.0
       ...
       expressions as :class:`~bson303.regex.Regex` objects. Use
...
class BSON(bytes):
    ...
    def encode(cls, document, check_keys=False,
        ...
        :class:`~bson303.errors.InvalidDocument` if `document` cannot be
        converted to :class:`BSON`.

        :Parameters:
          - `document`: mapping type representing a document
          - `check_keys` (optional): check if keys start with '$' or
            contain '.', raising :class:`~bson303.errors.InvalidDocument` in
            either case
          - `codec_options` (optional): An instance of
            :class:`~bson303.codec_options.CodecOptions`.
    ...
    def decode(self, codec_options=DEFAULT_CODEC_OPTIONS):
        ...
        configure a :class:`~bson303.codec_options.CodecOptions`::

            >>> import collections  # From Python standard library.
            >>> import bson303
            >>> from bson303.codec_options import CodecOptions
            >>> data = bson303.BSON.encode({'a': 1})
            >>> decoded_doc = bson303.BSON.decode(data)
            <type 'dict'>
            >>> options = CodecOptions(document_class=collections.OrderedDict)
            >>> decoded_doc = bson303.BSON.decode(data, codec_options=options)
            >>> type(decoded_doc)
