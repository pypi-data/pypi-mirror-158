...
def copy_meta_data_to_blob_storage(
        local_file, root_container, run_type, version_id):
    blob_client = get_blob_client(root_container)
    
    copy_file_to_blob(
        local_file,
        #f"dbfs:/{run_type}/{version_id}/{local_file}"
    )
