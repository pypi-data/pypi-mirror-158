...
from . import RandomStorageFixture, MPI, timeout, single_process_test
import time
...
class TestMorphologyRepository(RandomStorageFixture):
    def __init_subclass__(cls, root_factory=None, *, engine_name, **kwargs):
        super().__init_subclass__(**kwargs)
        cls._engine = engine_name
        cls._rootf = root_factory
    ...
    @single_process_test
    def test_saveload_eq(self):
        for path in get_all_morphologies():
            with self.subTest(morpho=path):
                m = Morphology.from_swc(path)
                self.mr.save("X", m, overwrite=True)
                lm = self.mr.load("X")
                self.assertEqual(m, lm, 'save+load violates equality')


    @single_process_test
    def test_ldc_mdc(self):
