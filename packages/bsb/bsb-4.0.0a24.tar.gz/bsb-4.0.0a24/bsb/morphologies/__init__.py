...
class Morphology(SubTree):
    ...
    def __eq__(self, other):
        return (
            len(self.branches) == len(other.branches)
            and all(
                b1.is_terminal == b2.is_terminal
                and (not b1.is_terminal or b1 == b2)
...
class Branch:
    ...
    def __eq__(self, other):
        if isinstance(other, Branch):
            return np.allclose(self.points, other.points) and self.labels == other.labels
        else:
        ...
        for label_arr in label_arrs:
            for k, l in label_arr.labels.items():
                if k not in merged:
                    ...
                    to_map_arrs[id(label_arr)] = label_arr
