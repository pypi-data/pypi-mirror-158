...
class APIKey:
    ...
    @staticmethod
    def create(
        name: str, expiry: Optional[Union[int, timedelta, datetime]] = None
    ) -> Dict[str, str]:
        """Create a new ApiKey
        ...
        Raises:
            ...
            ValueException: If the user tries to create a new ApiKey with an existing key name.
            ConnectionError: If the server address is invalid or not reachable.

        An example to create a new ApiKey with 1 year validity:
    ...
    def ls(
        ...
        Args:
            user: user_id/username associated with the ApiKey. Please call `User.details` method to get your user_id.
        ...
        Raises:
            ConnectionError: If the server address is invalid or not reachable.
            ValueException: If the user_id is invalid.
        ...
        ```
        """
        user_id = User.details(user=user)["id"]
        apikeys = Client._get_data(
            relative_url=f"/{user_id}/apikey/?include_disabled={include_disabled}&offset={offset}&limit={limit}"
        )
    ...
    @staticmethod
    def details(apikey: Union[int, str]) -> pd.DataFrame:
        """Return details of an ApiKey.

        Args:
            apikey: ApiKey id/name.

        Returns:
            A pandas Dataframe encapsulating the details of the ApiKey.

        Raises:
            ValueException: If the ApiKey id is invalid.
            ConnectionError: If the server address is invalid or not reachable.

        An example to get details of an ApiKey:

        ```python
        APIKey.details(apikey=1)
        ```
        """
    ...
    @staticmethod
    def revoke(
        keys: Union[int, List[int], str, List[str]],
        user: Optional[Union[int, str]] = None,
    ) -> pd.DataFrame:
        """Revoke one or more ApiKeys

        Args:
            keys: ApiKey id/name to revoke. To revoke multiple keys, please pass the ids/names as a list.
            user: user_id/username associated with the ApiKey. Please call `User.details` method to get your user_id/username.
        ...
        for key_id in _keys:
            response = Client._delete_data(relative_url=f"/{user_id}/apikey/{key_id}")
