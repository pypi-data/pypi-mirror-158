...
def main():
    ...
    # Can't use --mode=api with --output-path or --job-id:
    if args.mode == "api" and (args.output_path is not None or args.job_id is not None):
        raise SystemExit(
            "API mode (--mode=api or SCIAGRAPH_MODE=api) conflicts with both setting "
            "a process-wide job ID (--job-id or SCIAGRAPH_JOB_ID) and with setting "
            "an output path (--output-path or SCIAGRAPH_OUTPUT_PATH)."
        )

    if args.mode == "api" and args.trial_mode:
        raise SystemExit(
            "API mode (--mode=api or SCIAGRAPH_MODE=api) cannot be used with "
            "trial usage (--trial). Sign up for an account to use this feature."
        )
