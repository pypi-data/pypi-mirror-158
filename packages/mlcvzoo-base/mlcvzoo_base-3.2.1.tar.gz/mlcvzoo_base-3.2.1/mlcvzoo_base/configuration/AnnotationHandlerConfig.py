...
class AnnotationHandlerWriteOutputCSVAnnotationConfig(BaseConfigClass):
    ...
    def check_values(self) -> bool:
        if not os.path.isdir(self.csv_dir):
            ...
            logger.info(
                "Class='%s', specify correct output_dir'%s'", type(self), self.csv_dir
            )
...
class AnnotationHandlerWriteOutputConfig(BaseConfigClass):
    ...
    darknet_annotation: AnnotationHandlerWriteOutputDarknetAnnotationConfig = (
        related.ChildField(
            cls=AnnotationHandlerWriteOutputDarknetAnnotationConfig,
            required=False,
            default=None,
    ...
    bbox_snippets: Optional[
        AnnotationHandlerWriteOutputBoxSnippetsConfig
    ] = related.ChildField(
        AnnotationHandlerWriteOutputBoxSnippetsConfig, required=False, default=None
    )

    random_state: Optional[int] = related.ChildField(
        cls=int, default=None, required=False
    )
