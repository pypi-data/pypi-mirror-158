...
class Box:
    ...
    def init_format_based(
        box_format: str,
        box_list: List[int],  # TODO should this be: Tuple[int, int, int, int] ?
        ...
        if box_format == ObjectDetectionBBoxFormats.XYWH:
            ...
            raise ValueError(
                f"Format {box_format} is not supported. "
                f"Please provide any of {valid_formats}"
            )
    ...
    def scale(self, src_shape: Tuple[int, int], dst_shape: Tuple[int, int]) -> None:
        ...
        # TODO: check for correct shape values?
