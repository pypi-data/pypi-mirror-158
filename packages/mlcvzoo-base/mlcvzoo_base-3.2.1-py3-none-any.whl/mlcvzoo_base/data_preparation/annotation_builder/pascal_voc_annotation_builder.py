...
from mlcvzoo_base.data_preparation.custom_exceptions import ClassMappingNotFoundError
...
class PascalVOCAnnotationBuilder(AnnotationBuilder):
    ...
    def build(
        ...
        # TODO: We need a clear definition how xml and image paths belong together
        if not os.path.isfile(
            image_path
        ):  # TODO: This will trigger a lot of messages...
            logger.debug(
                "Use the file path from XML file because what is given is no path: '%s'"
                % image_path,
            )
        ...
        for member in root.findall("object"):
            ...
            if difficult:
                if self.use_difficult:
                    logger.debug("Found difficult box, which will be used!")
                else:
                    logger.debug(
                        "Found difficult box, which will be skipped! %s" % bounding_box
                    )
                    continue
