...
def get_blur_metrics(image_file, k=10):
    ...
    return top_sv/total_sv, get_blur_map(image_file), laplace.var(), cv2.convertScaleAbs(laplace)
...
if __name__ == '__main__':
    ...
    files = glob.glob(os.path.dirname(__file__) + '/*.jpg')
    for f in files:
        score1, map1, score2, map2 = get_blur_metrics(f)
        print (f, score1, score2)
        cv2.imwrite(f.replace('.jpg','_svd.jpg'), (1-map1)*255)
