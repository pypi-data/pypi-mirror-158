...
class MainGui(ttk.Frame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.distance_F = 1000
        self.distance_L = 1000
        self.distance_R = 1000

        '''
        # start a background distance measurement thread
...
4,5,6,7 - robotic arm
    ...
    def on_tree_dbclicked(self, event):
        ...
        if item == TEST_LED:    
            ...
            if not LCD_DRIVER_I:
                ...
                cols = 16
                l1 = 'liquidcrystal_i2c'.center(cols)
                l2 = '@hangzhou'.rjust(cols)
                for i in range(cols):
    ...
    def measure_distance(self, front_only = True):

        # while(True): # cannot do it in a thread

        # inside this loop, we judge distance to avoid collision
        ...
        if not front_only:
            ...
            print(self.distance_F, self.distance_L, self.distance_R)

        if self.distance_F < 20:
            acc.stop(0.2)
    ...
    def get_image(self):
        ...
        if acc.RUNNING == False:
            ...
            front_only = True
            self.measure_distance(front_only=front_only) # update distances
            if front_only:
                s = "Distance : " + str(self.distance_F)
            else:
                s = "Distances : " + str(self.distance_L) + ', ' + str(self.distance_F) + ',' + str(self.distance_R)
            draw = ImageDraw.Draw(img)
            ft = ImageFont.truetype('FreeMono.ttf', 25)
            draw.text((10, 10), 
            s, 
            font = ft,
            fill =(255, 255, 255))
