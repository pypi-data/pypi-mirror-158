...
class MainApp(ttk.Frame):
    ...
    def __init__(self, master):
        ...
        self.task = 'blur' # any of 'emotion', 'C1000', 'flower', 'rop', 'blur'
    ...
    def get_image(self):
        ...
        if self.task == 'emotion':
            input_img = emotion.detect_emotion(input_img)
        elif self.task == 'C1000': # image classification on 1000 classes. use pretrained model on ImageNet
            if 'c1000' not in self.__dict__ or self.c1000 is None:
                self.c1000 = Classifier()
            self.c1000.predict_cv(input_img, anno = True)
        elif self.task == 'flower': # image classification on 1000 classes. use pretrained model on ImageNet
