...
class gdo_drow:
    ...
    def get_same_user(user: str, session: str) -> str:
    	head = {'Cookie':'mid=YF55GAALAAF55lDR3NkHNG4S-vjw; ig_did=F3A1F3B5-01DB-457B-A6FA-6F83AD1717DE; ig_nrcb=1; shbid=13126; shbts=1616804137.1316793; rur=PRN; ig_direct_region_hint=ATN; csrftoken=ot7HDQ6ZX2EPbVQe1P9Nqvm1WmMkzKn2; ds_user_id=46165248972; sessionid='+str(session)}
    	azoz = requests.get(f'https://www.instagram.com/web/search/topsearch/?context=blended&query={user}',headers=head).json()
    	user_list = []
    	full_list = []
    	try:
    		se = 0
    		while True:
    			try:
    				se += 1
    				user = str(azoz['users'][se]['user']['username'])
    				name = str(azoz['users'][se]['user']['full_name'])
