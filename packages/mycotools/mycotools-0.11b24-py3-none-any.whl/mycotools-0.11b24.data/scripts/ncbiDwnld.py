...
def AcquireGenomeIDs(accession, column, database = 'assembly'):
...
# for each row in the assembly, grab the accession number, form the search term for Entrez, use Entrez,
    ...
    for index, row in ncbi_df.iterrows():
        ...
        if 'assembly_acc' in set(row.keys()):
            failure_acc = row['assembly_acc']
        else:
            failure_acc = row[column]
        if pd.isnull(row[column]):
            ass_prots[str(index)] = {
                'assembly_acc': failure_acc, 'assembly': '',
        ...
        if index in ass_prots: # add all rows that have indices associated with this
            out_df = out_df.append(row)
            icount = 1
            test = str(index) + '_' + str(icount)
            while test in ass_prots:
                count += 1
                row['genome_code'] = ass_prots[test]['genome_id']
                if 'internal_ome' in row.keys():
                    row['internal_ome'] = None # haven't assigned a mycotools ID yet
                out_df = out_df.append(row)
                test = str(index) + '_' + str(icount)
            continue

        genome_id = AcquireGenomeIDs(accession, ncbi_column, database = 'assembly')
...
# obtain the path from a summary of the ftp directory and create the standard paths for proteomes and assemblies
            ...
            if (not assembly or not gff3) and remove:
                failed.append([row['assembly_acc'], datetime.strftime(row['version'], '%Y%m%d')])
            ...
            ass_prots[str(index)] = {
                ...
                'assembly_acc': accession, 'genome_id': ID
                }
            row['genome_code'] = ID
            if icount:
                if 'internal_ome' in row:
                    row['internal_ome'] = None
            out_df = out_df.append(row)
            icount += 1
    ...
    for file_type in file_types:
        ...
        if request.status_code != 200:
            ...
            log_editor( output_dir + 'ncbiDwnld.log', str(ome), str(ome) + \
                '\t' + str(ome_prots['assembly_acc']) + '\t' + str(ome_prots['assembly']) + \
...
def main( 
    ...
    column = 'assembly_acc', ncbi_column = 'Assembly Accession'
    ):
    ...
    if 'Assembly Accession' in ncbi_df.keys() and not 'assembly_acc' in ncbi_df.keys():
        ncbi_df['assembly_acc'] = ncbi_df['Assembly Accession']
    ...
    for ome in ass_prots:
        if count >= 2:
            if not api:
                ...
                time.sleep(1)
                count = 0
        if ome not in set( ncbi_df['assembly_acc'] ):
            continue
        eprint('\t' + str(ome), flush = True)
        new_ome = ass_prots[ome]['genome_id']
        if ass_prots[ome]:
            ...
            check = ncbi_df[ncbi_df['assembly_acc'] == ome]
        ...
        ncbi_df.loc[ome, 'faa'] = output_path + 'proteome/' + \
