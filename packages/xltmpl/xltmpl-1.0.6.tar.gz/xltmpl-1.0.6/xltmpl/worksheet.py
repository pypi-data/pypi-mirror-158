...
class Worksheet(_WorkbookChild):
    ...
    def append(self, iterable, apply_styles: bool = True, place_holder='{{}}', style_row=None):
        """Custom
        ...
        * **or** append({1 : 'This is A1', 3 : 'This is C1'})
        ...
        # delete place holder
        last_row_idx = self._current_row

        if hasattr(self, 'last_row_styles'):
            row_idx = self._current_row + 1
        else:
            self.last_row_styles = {}
            if last_row_idx != 0:
                is_place_holder_row = set([str(cell.value) for cell in self[last_row_idx]]) == set([place_holder, ])
            else:
                is_place_holder_row = False
            if is_place_holder_row:
                for cell in self[last_row_idx]:
                    cell.value = None
                    self.last_row_styles[cell.col_idx] = cell._style
        ...
        if (isinstance(iterable, (list, tuple, range))
            ...
            for col_idx, content in enumerate(iterable, 1):
                ...
                if apply_styles:
                    ...
                    row = cell.row
                    column = cell.column
                    if (row - 1, column) in self._cells:
                        # cell._style = self.cell(row - 1, column)._style
                        if cell.col_idx in self.last_row_styles:
                            cell._style = self.last_row_styles[cell.col_idx]
