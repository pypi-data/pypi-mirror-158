from hdsr_wis_config_reader import FewsConfigReader
...
class LocationMapper:
    ...
    def complex_to_hoofd(self, caw_complex: str) -> List[str]:
        ...
        df_idmap_filtered = self.df_idmap.get_filtered_df(ex_loc=caw_complex)
        assert not df_idmap_filtered.empty, f"caw_complex (ex_loc) {caw_complex} not in idmapping"
        hoofd_locs = [x for x in df_idmap_filtered[IdMapCols.int_loc].unique() if IntLocChoices.is_kw_hoofd(int_loc=x)]
        hoofd_locs = list(set(hoofd_locs))
