...
def rings(features_X, features_position, criteria="k", max_radio=[200, 300, 400], max_neighbours=[200, 500, 1000], weight_mode="Simple", keep_original_value=True, smoothing=1e-08, normalize=True, leafsize=10):
    ...
    if(criteria == "k"):
        ...
        for _, r in enumerate(max_radio):
            ...
            for _, neighbourhood in enumerate(nearest_neighbours):
                for f_index, feature in enumerate(features_X.columns):
                    ...
                    features_array[f_index].append(mean_feature) 
            for f_index, feature in enumerate(features_X.columns):
                final_features_df[f"{feature}_{r}m"] = features_array[f_index]
        features_X = final_features_df
    elif(criteria == "rk"):
        for i, r in enumerate(max_radio):
            ...
            for f_index, feature in enumerate(features_X.columns):
                final_features_df[f"{feature}_{r}m{k}nb"] = features_array[f_index]
        features_X = final_features_df
    return features_X
