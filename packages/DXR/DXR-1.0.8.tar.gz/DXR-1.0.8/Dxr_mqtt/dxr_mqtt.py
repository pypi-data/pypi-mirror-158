...
def callback(topic, msg, client):
    ...
    if callback_dit[topic]['func'] is not None:
        callback_dit[topic]['func'](msg, topic)
...
def Mqtt():
    ...
    if mqttc is None:
        try:
            ...
            # 订阅该主题，QoS=0
            t = threading.Thread(target=mqttc.loop_forever, daemon=True)
            t.start()
            return mqttc
        except Exception as ex:
            print(ex)
...
class Dxr_Publisher:
    def __init__(self, topic):
        global publish_dit, topic_list_str_pub, topic_list_str_sub, callback_dit
        if isinstance(topic, str):
            self.topic = topic
            self.data_type = None
        else:
