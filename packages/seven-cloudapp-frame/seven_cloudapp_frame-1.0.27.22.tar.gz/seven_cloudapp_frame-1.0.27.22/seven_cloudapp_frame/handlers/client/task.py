...
@LastEditTime: 2022-07-07 18:55:18
...
class InviteNewUserHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        is_stat = invoke_result_data.data["is_stat"] if invoke_result_data.data.__contains__("is_stat") else True
        invoke_result_data = task_base_model.process_invite_user(app_id, act_id, module_id, user_id, login_token, from_user_id, self.__class__.__name__, self.request_code, check_user_nick, check_new_user, continue_request_expire, close_invite_limit, is_stat)
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
        ref_params = {}
        invoke_result_data = self.business_process_executed(invoke_result_data, ref_params)
