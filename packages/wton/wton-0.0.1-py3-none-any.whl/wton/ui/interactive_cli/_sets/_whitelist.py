...
class WhitelistSet(BaseSet):
    ...
    def _handle_get(self):
        questions = [
            inquirer.List(
                "name",
                message='Contact to get',
                choices=[contact.name for contact in self.ctx.whitelist.contacts],
                carousel=True
            )
        ]
        name = self._prompt(questions)["name"]

        contact = self.ctx.whitelist.get_contact(name, raise_none=True)

        addr = Address(contact.address)
        echo_success(f"Raw address: {addr.to_string(False, False, False)}", True)
        echo_success(
            f"Nonbounceable address: {addr.to_string(True, True, False)}", True)
        echo_success(f"Bounceable address: {addr.to_string(True, True, True)}", True)
    ...
    def _handle_edit(self):
        ...
        questions = [
            inquirer.Text(
                "name", message='Enter new name (enter to skip)'),
            inquirer.Text(
    ...
    def _handle_delete(self):
        ...
        self.ctx.whitelist.delete_contact(name, save=True)
