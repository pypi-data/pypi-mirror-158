...
from ..._utils import init_shared_object, SharedObject, setup_app
from wton import settings
from wton.config import Config, ConfigNotFoundError, WtonConfig, ProviderConfig
...
def init():
    """
    Initialize .wton workdir in a current directory.
    """
    wton = WtonConfig(workdir=settings.DEFAULT_LOCAL_WORKDIR)
    provider = ProviderConfig()
    config = Config(wton=wton, provider=provider)
    setup_app(config)
