...
from wton.tonsdk.crypto import generate_new_keystore, generate_keystore_key
from wton.tonsdk.utils import Address, InvalidAddressError
from wton.utils import storage
from wton.tonsdk.contract.wallet import WalletVersionEnum, Wallets
...
class KeyStores:
    ...
    def restore_keystore(self, name: str, filepath: str, password: Optional[str] = None):
        name = self.__keystore_name(name)
    ...
    def get_keystore(self, keystore_name: Optional[str], raise_none: bool = False) -> Optional[KeyStore]:
        if keystore_name is None:
            raise KeyStoreIsNotSpecifiedError(
                "wton.keystore_name is not specified.")
