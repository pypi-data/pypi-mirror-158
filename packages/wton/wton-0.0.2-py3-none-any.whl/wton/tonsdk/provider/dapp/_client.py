...
class DAppClient:
    def __init__(self, graphql_url: str, broadcast_url: str, api_key: str):
        ...
        self.transport = AIOHTTPTransport(
            url=graphql_url, headers={"API-KEY": api_key})
    ...
    async def broadcast(self, queries: List[BroadcastQuery]) -> List[Any]:
        results = []
        timeout = aiohttp.ClientTimeout(total=20)

        async with aiohttp.ClientSession(timeout=timeout) as session:
            for query in queries:
                async with session.post(self.broadcast_url, json=query.dict(), headers=self.__headers()) as resp:
    ...
    async def __parse_response(self, resp):
        try:
            ...
            raise DAppWrongResult(ErrorMsg(resp.reason, resp.status))
