...
class BaseExperiment:
    ...
    def fit(
        ...
        **config        
        ...
        if self.data_module is None:
            self.data_module = self.get_data_module(stratify=self.stratify)
        if self.logger:
            self.logger.close()
            del self.logger
            self.logger = None
    ...
    def delete_saved_model(
        ...
        **hparams
        ...
        del_ckpt_path = f'{self.log_path}/checkpoints/{model_type.__name__}' 
        del_json_path = f'{self.log_path}/jsonlogs/{model_type.__name__}'
        del_tensorboard_path = f'{self.log_path}/tensorboardlogs/{model_type.__name__}' 
        ...
        if version > -1:
            del_ckpt_path = f"{del_ckpt_path}/version_{version}"
            del_json_path = f"{del_json_path}/version_{version}"
            del_tensorboard_path = f"{del_tensorboard_path}/version_{version}"
        ...
        if os.path.exists(ckpt_dir):
            shutil.rmtree(del_ckpt_path, ignore_errors=True)
            success.update({ckpt_dir:1})
        if os.path.exists(jsonlog_dir):
            shutil.rmtree(del_json_path, ignore_errors=True)
            success.update({jsonlog_dir:1})
        if os.path.exists(tensorboard_dir):
            shutil.rmtree(del_tensorboard_path, ignore_errors=True)
