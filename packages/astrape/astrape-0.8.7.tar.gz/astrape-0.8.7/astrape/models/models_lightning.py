...
class _MLP(nn.Module):
    """Multilayer perceptron with identical # of hidden units.
...
class MLP(BaseNet):
    """Multi-layer perceptron with identical number of hidden units for each layer.

    The number of hidden units for each layer should be specified in `n_hidden_units` and the number of layers should
    be specified in `n_layers`.
...
class ContractingMLP(BaseNet):
    """Multi-Layer perceptron with number of hidden units for each layer contracting with `contraction_ratio`.
...
class _CustomMLP(nn.Module):
    """Mutli-layer perceptron with specified hidden units for each hidden layer as a list.

    CustomMLP defines an MLP with number of hidden units for hidden layers 
    ...
    def __init__(
        ...
        n_hidden_units_list : Optional[List[int]] = [],
        momentum : float=0.9,
        sample_weight : Optional[Union["torch.Tensor", "np.ndarray"]] = None
    ) -> None:
        ...
        if self.n_hidden_units_list is None or self.n_hidden_units_list == []:
            if self.n_classes is None:
                layers.append(nn.Linear(self.dims, 1))
            elif self.n_classes > 2:
                layers.append(nn.Linear(self.dims, self.n_classes))
...
class CustomMLP(BaseNet):
    ...
    Example::
        ...
        >>> model = CustomMLP(dims=(1,8,8), n_classes=10, n_hidden_units_list=[30,20,10])

    Args:
        
        dims (Union[int, Tuple[int]]): The dimension of the data. e.g., 4, (1,8,8)

        n_classes (Optional[int]) : Number of classes (labels) in the classification task.
        
        n_hidden_units_list : Optional[List[int]] : List of all n_hidden_units for all hidden layers.
                                                    If not specified, it would be a single-layer perceptron.
    ...
    def __init__(
        ...
        **hparams : Any
        ...
        for idx in range(1, len(hparams['n_hidden_units_list'])+1):
            key_str = 'layer_' + str(idx) +'_dim'
            hparams.update({key_str : hparams['n_hidden_units_list'][idx-1]}) 
        # save information of **hparams to self.hparams.
        self.save_hyperparameters(*hparams)
        # n_hidden_units_list
        for n_hidden_units in hparams['n_hidden_units_list']:
            if n_hidden_units <=0:
    ...
    def __init__(
        ...
        **hparams
        ...
        super(ResNet18, self).__init__(
            ...
            n_classes=n_classes,
            sample_weight=sample_weight,
            schduler_config=scheduler_config,
            **hparams
        )
