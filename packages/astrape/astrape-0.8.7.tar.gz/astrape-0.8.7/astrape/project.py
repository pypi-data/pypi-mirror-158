...
class BaseProject:
    ...
    def update_project_metadata(
        ...
        """Updates metadata of the project.
    ...
    def delete_empty_exps(
        self
    )->None:
        """Delete empty experiments (if any). 

        This will delete empty {path}/{project_name}/FIT/random_state_{random_state} folder.

        """
        path = f"{self.path}/{self.project_name}/FIT"
        for exp in os.listdir(path):
            if len(exp) == 0:
                log.info(f"logs folder has not been created in {exp}. Deleting {exp}...")
                shutil.rmtree(f"{path}/{exp}")
            jsonlog_path = f"{path}/{exp}/logs/jsonlogs"
            if len(os.listdir(jsonlog_path)) == 0:
                log.info(f"{exp} is an empty experiment. Deleting {exp}...")
                shutil.rmtree(f"{path}/{exp}")  
    ...
    def create_experiments(
        ...
        Example::
            ...
            >>> project.create_experiments(amount=10)
    ...
    def add_experiment(
        ...
        """Add an individual ``Experiment`` to ``exps``.
    ...
    def set_models(
        ...
        **hparams
        ...
        """Set identical models across ``Experiments``.
    ...
    def set_trainers(
        ...
        **trainer_config
        ...
        """Set identical trainers across ``Experiments``.
    ...
    def fit_experiments(
        ...
        **trainer_config
        ...
        """Fit models in ``Experiments`` stacked in ``exps``.
    ...
    def save_ckpts(
        ...
        """Save checkpoints in ``Experiments`` stacked in ``exps``.
    ...
    def save_stacks(
        ...
        """Apply ``save_stack()`` for ``Experiments`` in ``exps``.
    ...
    def get_available_random_states(
        ...
        """Returns list of directories of ``Experiments`` that has been fitted and logged.
    ...
    def search_standard(
        ...
        """Search method for finding the model type/structure with best validation performance.
        ...
        for random_state_dir in random_state_dir_list:
            if len(os.listdir(random_state_dir)) == 0:
                continue
            if len(os.listdir(f"{random_state_dir}/logs")) == 0:
                continue
            jsonlog_dir = f"{random_state_dir}/logs/jsonlogs"
            if len(os.listdir(f"{jsonlog_dir}")) == 0:
                continue
    ...
    def search_with_model_type(
        ...
        """Search method for finding validation performances for a specific model type.
    ...
    def search_with_model_type_hparams(
        ...
        **hparams
        ...
        """Search method for finding validation performances of a specific model structure.
    ...
    def search_all_models(
        ...
        """Method for searching validation performances of all saved models.
...
class Project(BaseProject):
    """Project class that plots results of the ML project.
    ...
    def plot_best_performances(
        ...
        """Plots the best validation performance for each ``Experiment``.
    ...
    def plot_best_model_frequencies(
        ...
        """Plots pie graphs showing the frequencies of 1)best model type and 2)best model structure for each ``Experiment``.
    ...
    def plot_identical_model_type(
        ...
        """Plots validation performances across different random states for a given model type.
    ...
    def plot_identical_model_structure(
        ...
        **hparams
        ...
        """Plots validation performances across different random states for a given model structure.
    ...
    def generate_marker_style(amount)->List[str]: 
        """Returns a string specifying the marker style
    ...
    def plot_all_model_structures(
        ...
        """Plots validation performances across different random states for all model structures saved. 
