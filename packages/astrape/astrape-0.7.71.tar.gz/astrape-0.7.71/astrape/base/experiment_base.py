...
class BaseExperiment:
    ...
    def get_data_module(
        ...
        data_module = None
        if self.n_classes is None and stratify:
    ...
    def fit(
        ...
        **config        
        ...
        Args:
            ...
            stratify (bool): Whether to use label-stratification in data splitting. Forced to be `False` when it is a regression task.
                Default: ``True``
        ...
        if self.data_module is None:
            self.data_module = self.get_data_module(stratify=stratify)
        if self.logger:
            self.logger.close()
            del self.logger
            self.logger = None
