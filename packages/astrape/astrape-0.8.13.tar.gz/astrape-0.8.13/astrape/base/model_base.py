...
class BaseNet(pl.LightningModule):
    """BaseNet. Use this for models that don't deal with image data. 

    Descendants of BaseNet are typically a multi-layered perceptron and takes flattened
    data as an input. 
    ...
    Note:
        ...
        compatible with other MLPs that are written in pytorch-lightning.
    ...
    Args: 
        ...
        scheduler_config (``Dict``, optional): Dictionary specifying the LearningRate scheduler. It should include the class of the scheduler and its configurations. 
            Default: {'scheduler' : None, 'scheduler_config' : None}
    ...
    def __init__(
        ...
        scheduler_config : Optional[Dict[str, Any]] = {'scheduler' : None, 'scheduler_config' : {}},
        **config : Any
    ) -> None:
        super(BaseNet, self).__init__()

        if n_classes <= 1:
            raise ValueError("n_classes can't be small than or equal to 1.")
        ...
        if list(self.scheduler_config.keys()) != ['scheduler', 'scheduler_config']:
            raise ValueError(f"Wrong format of dictionary for scheduler_config. The keys for the argument should be 'scheduler' and 'scheduler_config'. Got {list(self.scheduler_config.keys())} instead.")
    ...
    def configure_optimizers(self):
        ...
        scheduler_dict = {
            "scheduler" : self.get_scheduler(optimizer=optimizer, scheduler_class=self.scheduler_config["scheduler"], **(self.scheduler_config["scheduler_config"])),
            "interval" : "step"
        }
...
class ConvNet(pl.LightningModule):
    """ConvNet. Use this when using CNNs. 

    Note:

        ConvNet is the parent class for CNNs. It is also compatible with other CNNs
        that are written in pytorch-lightning. 
    ...
    def criterion(self):   
        ...
        Returns:
            ...
            Criterion. See astrape.utilities.utils for details.
...
class SegNet(pl.LightningModule):
    """SegNet. Use this when using CNNs to perform Image Segmentation. 
