...
def rearrange_dims(dims, dataformats="NCHW"):
    if isinstance(dims, int):
        ...
        if dataformats == "NCHW":
            dims = dims
        elif dataformats == "NHCW":
            height, channels, width = dims
            dims = (channels, height, width)
        elif dataformats == "NHWC":
            height, width, channels = dims
            dims = (channels, height, width)
        elif dataformats == "NCWH":
            channels, width, height = dims
            dims = (channels, height, width)
        elif dataformats == "NWHC":
            width, height, channels = dims
            dims = (channels, height, width)
        elif dataformats == "NWCH":
            width, channels, height = dims
            dims = (channels, height, width)
        else:
            raise ValueError(f"Invalid dataformats {dataformats}. It should start with 'N' followed by permutation of 'HWC'.")
    else:
        raise ValueError(f"Wrong dimension {dims} for an input.")
    return dims
