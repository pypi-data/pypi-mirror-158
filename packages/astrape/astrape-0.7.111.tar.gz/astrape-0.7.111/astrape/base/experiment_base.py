...
class BaseExperiment:
    ...
    def get_data_module(
        ...
        if isinstance(self.model, ConvNet) or isinstance(self.model, SegNet):
            ...
            if self.X_val is not None and self.X_test is not None:
                data_module = initialize_datamodule(
                    batch_size=batch_size, 
                    X=self.X, 
                    y=self.y,
                    X_val=self.X_val.reshape(-1, *(self.X.shape[1:])),
                    y_val=self.y_val,
                    X_test=self.X_test.reshape(-1, *(self.X.shape[1:])),
                    y_test=self.y_test,
                    test_size=self.test_size,
                    random_state=self.random_state,
                    stratify=self.stratify
                )
            elif self.X_val is not None and self.X_test is None:
                data_module = initialize_datamodule(
                    batch_size=batch_size, 
                    X=self.X, 
                    y=self.y,
            ...
            elif self.X_val is None and self.X_test is None:
                data_module = initialize_datamodule(
                    batch_size=batch_size, 
                    X=self.X, 
                    y=self.y,
    ...
    def delete_saved_model(
        self,
        model_type : Union["pl.LightningModule", "nn.Module", "BaseEstimator"],
        version : int = -1, 
        **hparams
    ):
        if not isinstance(version, int) or version <-1:
            raise ValueError(f"Wrong value for version. Expected to be an integer greater than or equal to -1. Got {version} instead.")
        if BaseNet in inspect.getmro(model_type) or ConvNet in inspect.getmro(model_type) or SegNet in inspect.getmro(model_type):
            pseudomodel = model_type(dims=self.dims, n_classes=self.n_classes, **hparams)
        else:
        ...
        if version > -1:
            ckpt_dir = f"{ckpt_dir}/version_{version}"
            jsonlog_dir = f"{jsonlog_dir}/version_{version}"
            tensorboard_dir = f"{tensorboard_dir}/version_{version}"
        
        shutil.rmtree(path=ckpt_dir)
        shutil.rmtree(path=jsonlog_dir)
