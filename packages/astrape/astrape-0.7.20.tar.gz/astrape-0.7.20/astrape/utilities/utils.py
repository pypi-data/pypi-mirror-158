...
def rearrange_dims(dims, in_format, dataformats="NCHW"):
    def find_mapping(in_format, dataformats):
        ...
        c_to_i = {"N" : 0, "C" : 1, "H" : 2, "W" : 3}
        in_format_array = np.array([])
        dataformat_array = np.array([])
        for c_in, c_out in zip(in_format, dataformats):
            in_format_array = np.concatenate([in_format_array, np.array([c_to_i[c_in]])]).astype(int)
            dataformat_array = np.concatenate([dataformat_array, np.array([c_to_i[c_out]])]).astype(int)
        for mapping in mappings:
            mapped = np.transpose(in_format_array, mapping)
    ...
    if isinstance(dims, int):
        ...
        dims = tuple(np.transpose(temp, mapping))
