...
def split_data(
    ...
    X_train_tensor = torch.Tensor(X_train).float().reshape(-1, *(X.shape[1:])) if not isinstance(X_train, torch.Tensor) else X_train.float().reshape(-1, *(X.shape[1:]))
    y_train_tensor = torch.Tensor(y_train).float() if not isinstance(y_train, torch.Tensor) else y_train.float()
    X_val_tensor = torch.Tensor(X_val).float().reshape(-1, *(X.shape[1:])) if not isinstance(X_val, torch.Tensor) else X_val.float().reshape(-1, *(X.shape[1:]))
    ...
    def setup(self, stage):
        if self.X_val is not None and self.y_val is not None and self.X_test is not None and self.y_test is not None:
            ...
            if not isinstance(self.X, torch.Tensor) and not isinstance(self.X_val, torch.Tensor):
                self.X_predict_tensor = torch.Tensor(np.concatenate([self.X, self.X_val.reshape(-1,*(self.X.shape[1:]))])).float()
            else:
