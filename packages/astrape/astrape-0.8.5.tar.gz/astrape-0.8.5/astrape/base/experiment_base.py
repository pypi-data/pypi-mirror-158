...
class BaseExperiment:
    ...
    Attributes:
        ...
        stratify (bool): Whether to use label-stratification in splitting data into train/val/test set.
            Default: ``True``
    ...
    def __init__(
        ...
        Example::
            ...
            >>> exp = Experiment(project_name="MNIST", X=X, y=y, n_classes=10)
        ...
        if n_classes is None and self.stratify:
            logger.info(f"Cannot perform label stratification in regression tasks. Setting stratify to False.")
            self.stratify = False
        self.path = str(path).replace(".",str(os.getcwd()))
        if self.path[-1] == "/":
    ...
    def update_exp_metadata(self)->None:
        """Updates metadata for the experiment.

        Experiment metadata contains the following::
            - number of checkpoints(models) saved
            - file size of the ``Experiment``(in MB).
            - random state governing the experiment. 
    ...
    def set_model(
        ...
        **hparams : Any
        ...
        """Method for setting a model in the ``Experiment``.
    ...
    def get_data_module(
        ...
        """Method for getting a datamodule.

        ``self.data_module`` would be `LightningDataModule` if ``self.model`` is a `LightningModule` object. Tuple of 
        (X_train, y_train, X_val, y_val, X_test, y_test) would be ``self.data_module`` if ``self.model`` is a `BaseEstimator` 
        object.
        ...
        data_module = None
        if self.n_classes is None and self.stratify:
            logger.info(f"Unable to stratify continuous targets. Will split the data without label stratification.")
            self.stratify = False
            ...
            if self.X_test is None and self.y_test is None:
                if self.n_classes is None:
                    if self.stratify:
                        logger.info(f"Unable to stratify continuous targets. Will split the data without label stratification.")
                        self.stratify = False
                    X_, X_test, y_, y_test = train_test_split(self.X, self.y, test_size=self.test_size)
                    X_train, X_val, y_train, y_val = train_test_split(X_, y_, test_size=self.test_size)
                else:
                ...
                data_module = (X_train, X_val, y_train, y_val, self.X_test, self.y_test)
    ...
    def set_trainer(self, **trainer_config)->"pl.Trainer":
        """Method for getting trainer for the lightning model.
    ...
    def fit(
        ...
        **config        
        ...
        ``self.model`` will be trained on `.fit()`. If ``self.model`` is a `LightningModule`, real-time tracking with 
        TensorBoard is supported. If ``self.model`` is a `BaseEstimator`, only the result of the training would be
        available.
        ...
        if BaseEstimator in inspect.getmro(self.model.__class__):
            self.X_train, self.X_val, self.y_train, self.y_val, self.X_test, self.y_test = self.data_module
    ...
    def fit_sklearn(
        ...
        **config
        ...
        self.X_train, self.X_val, self.X_test = self.X_train.reshape(len(self.y_train),-1), self.X_val.reshape(len(self.y_val),-1), self.X_test.reshape(len(self.y_test),-1)
        ...
        if self.n_classes:
            train_auc, val_auc, test_auc = math.nan, math.nan, math.nan
            ...
            if self.n_classes == 2:
                ...
                train_auc = roc_auc_score(y_true=self.y_train, y_score=y_train_predict_proba, multi_class='ovo')
                val_auc = roc_auc_score(y_true=self.y_val, y_score=y_val_predict_proba, multi_class='ovo')   
                test_auc = roc_auc_score(y_true=self.y_test, y_score=y_test_predict_proba, multi_class='ovo')
    ...
    def save_ckpt(
        ...
        """Method for saving checkpoints. The logs are saved as json file as well.
        ...
        # saving results
        if isinstance(model, BaseEstimator):
            tensorboard_logdir = self.logger.log_dir
        else:
            tensorboard_logdir = self.logger.experiment.get_logdir()

        self.saved_model_types.update({model.__class__.__name__: model.__class__})
    ...
    def save_to_stack(self)->None:
        """Method for stacking a model to ``stack``. If one of the ancestors of the model is ``BaseEstimator``, we only stack the model and its validation performance.
    ...
    def save_stack(self)->None:
        """Method for saving all model checkpoints in ``stack``.
        ...
        Example::
            ...
            [{'model': MLP(
                ...
                'trainer': <pytorch_lightning.trainer.trainer.Trainer at 0x7f719df2cf10>,
                'val_metrics': {'val/acc': 0.2898876368999481,
                'val/loss': 2.158809185028076,
                'val/auc': 0.7340054512023926}}]
    ...
    def best_ckpt_thus_far(
        ...
        Example::
            ...
            >>> exp.best_ckpt_thus_far(val_metric="val/auc")
            (MLP(
                (model): _MLP(
                    (layers): Sequential(
                    (0): Linear(in_features=64, out_features=10, bias=True)
        ...
        if not fetch_only_dir:
            ...
            for ckpt in os.listdir(ckpt_path):
                ...
                elif pl.LightningModule in inspect.getmro(self.saved_model_types[model_type]):
                    if BaseNet in inspect.getmro(self.saved_model_types[model_type]) or ConvNet in inspect.getmro(self.saved_model_types[model_type]):
                        best_ckpt = self.saved_model_types[model_type].load_from_checkpoint(dims=self.dims, n_classes=self.n_classes, checkpoint_path=ckpt_dir)
                    else:    
                        best_ckpt = self.saved_model_types[model_type].load_from_checkpoint(checkpoint_path=ckpt_dir)
                else:
    ...
    def best_ckpt_in_stack(
        ...
        """Method for returning the best saved checkpoint from the stack as per the validation metric.
    ...
    def set_model_metadata(
        ...
        if len(model_metadata) >= 255:
            if hparams not in self.hyperparameter_setting:
                self.hyperparameter_setting.append(hparams)

            hparams_str = f"hyperparameter_setting-{self.hyperparameter_setting.index(hparams)}"
            model_metadata = f"{model.__class__.__name__}-{hparams_str}" 
        return model_metadata
   
    def get_hparams(self, model):
        """Gets hyperparameter or model specs.
