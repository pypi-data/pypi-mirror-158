...
def _mk_intp(t, v):
    ...
    vv = v[i]
    lims = (vv[0], vv[-1])
    f = interpolate.interp1d(t[i], vv, fill_value=lims, bounds_error=False)
    return f
...
class Telescope():
    ...
    def __init__(self, speedup_factor=1, t_start = astropy.time.Time.now(), use_realtime=True, UPDATE_INTERVAL = .2, do_write_history = False):
        ...
        self.data = { "acu.time.internal_time": self.__t_internal.mjd,
    ...
    def update(self, stepsize=None):
        ...
        with self.dataLock:
            ...
            t_old = self.__t_internal
            if stepsize is not None:
                self.__t_internal += stepsize
                dt = float(stepsize)
            elif self.use_realtime:
                self.__t_internal = astropy.time.Time.now()
                dt = (self.__t_internal - t_old).to_value('s')  # convert to u.s (seconds)
            else:
                self.__t_internal += u.s * self.UPDATE_INTERVAL
            ...
            if self.data["acu.general_management_and_controller.state"] == "TRACK":
                self.data["acu.azimuth.p_set"] = self.faz(self.__t_internal.mjd)
                self.data["acu.elevation.p_set"] = self.fel(self.__t_internal.mjd)
            ...
            row = [self.__t_internal] + [self.data[v] for v in datalog_cols if 'internal_time' not in v]
