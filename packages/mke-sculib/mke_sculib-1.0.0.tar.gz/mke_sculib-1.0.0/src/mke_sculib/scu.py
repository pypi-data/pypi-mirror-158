...
class scu():
    ...
    def waitForPos(self, axis, value, timeout=600, query_delay=.25, tolerance=None):
        """
        ...
        key = dc1[axis.lower()]
        if key == 'feed_indexer' and isinstance(value, str):
            bands = {'Band 1': 1, 'Band 2': 2, 'Band 3': 3, 'Band 4': 4, 'Band 5a': 5, 'Band 5b': 6, 'Band 5c': 7}      
            dc_band_positions = {
                1: 99.85156,
                2: -103.4874,
            ...
            v = bands[value]
            value = dc_band_positions[v]
            tolerance = 0.01

        path = f'acu.{key}.p_act'
        self.waitForStatusValue(path, value, timeout, query_delay, tolerance)
    ...
    def waitForStatusValue(self, path, value, timeout=600, query_delay=.25, tolerance=None):
        ...
        while time.time() - starttime < timeout:
            ...
            if tolerance is None and v == value:
                return True
            elif tolerance is not None and abs(v - value) < abs(tolerance):
                return True
            self.wait_duration(query_delay)                
    ...
    def stow(self):
        ...
        self.waitForStatusValue("acu.stow_pin_controller.azimuth_status", "3")
        self.waitForStatusValue("acu.stow_pin_controller.elevation_status", "3")
    ...
    def unstow(self):
        ...
        self.waitForStatusValue("acu.stow_pin_controller.azimuth_status", "1")
        self.waitForStatusValue("acu.stow_pin_controller.elevation_status", "1")        
    ...
    def upload_track_table(self, t, az, el):
...
#        log('logger state ')
    ...
    def get_session_as_df(self, interval_ms=1000, session = 'last'):
        ...
        log('Attempt export of session: {} at rate {:.0f} ms'.format(session, interval_ms))
        if session == 'last':
            #get all logger sessions, may be many
            r=self.logger_sessions()
            #[-1] for end of list, and ['uuid'] to get id of last session in list
        ...
        df = pd.read_csv(StringIO(file_txt), sep=';', skiprows=nskip)
