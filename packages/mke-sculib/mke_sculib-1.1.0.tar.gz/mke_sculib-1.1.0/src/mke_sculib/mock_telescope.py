    ...
    def update(self, stepsize=None):
        ...
        with self.dataLock:
            ...
            if self.data["acu.general_management_and_controller.state"] == "TRACK":
                self.data["acu.azimuth.p_set"] = self.faz(self.t_internal.mjd)
                self.data["acu.elevation.p_set"] = self.fel(self.t_internal.mjd)
            ...
            row = [self.t_internal] + [self.data[v] for v in datalog_cols if 'internal_time' not in v]
