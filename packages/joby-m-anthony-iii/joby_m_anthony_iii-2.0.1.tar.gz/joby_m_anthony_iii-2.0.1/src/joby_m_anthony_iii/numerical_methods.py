...
def condition_number(
	...
	See Also
	--------
	norms().l_two() : Method that yields the l_two norm.
...
class EigenValues:
	...
	__BadVectorDataError = lambda matrix_name, matrix_size, vector_name, vector_size: "ERROR! {matrix_name} of size {matrix_size} and {vector_name} of size {vector_size} must be the same length!"
	...
	def power_method(
		...
		if np.sum(np.array(x).shape) - np.array(x).shape[0] > 1: raise IndexError(f"Systems vector, {self.__vector_name} must be n x 1 or 1 x n array!")
		...
		while errors[-1] > self.tol and k <= self.max_iter:
			...
			for yi in y:
				if abs(yi) == norms(y).l_infinity(): yp = float(yi)
			mu.append(yp)
	...
	def steepest_descent(
		...
		if np.sum(np.array(b).shape) - np.array(b).shape[0] > 1: raise IndexError(f"Systems vector, {self.__vector_name} must be n x 1 or 1 x n array!")
		if len(x) != len(self.A): raise IndexError(EigenValues.__BadVectorDataError(self.__matrix_name, len(self.A), self.__solution_name, len(b)))
		self.b = b = np.array(b)
		k, eigenvectors, errors = 1, [x], [self.tol*10]
		while errors[-1] > self.tol and k <= self.max_iter:
			r = b - np.matmul(self.A, x)
...
class SingleVariableIteration:
	...
	def __init__(
		...
		"""
		self.__function_name = function_name = _retrieve_name(function)[0]
		# if isinstance(function, (str)):
		# 	function_str = ex.fast_parse_latex(function)
		# 	function = lambda x: ex.fast_eval_latex(function_str, {variable: x})
...
class MultiVariableIteration:
	...
	def __init__(
		...
		self.__vector_name = _retrieve_name(x)
		self.__solution_name = _retrieve_name(b)
		BadMatrixError = lambda matrix_name: f"ERROR! Matrix, '{matrix_name}' must be square!"
		BadVectorError = lambda vector_name: f"ERROR! Systems vector, '{vector_name}' must be n x 1 or 1 x n array!"
		...
		self.is_tridiagonal = tridiagonality(A)
		self.eigen_values = EigenValues(A, power=power).conjugate_gradient(x, b)
	...
	def gauss_seidel(
		...
		norms().l_two() : Will find the l_two norm between x0 and xi.
		...
		while errors[-1] > self.tol and k <= self.max_iter:
			...
			for i in range(n):
				y1 = y2 = 0
				for j in range(i): y1 += self.A[i][j]*xi[j]
				for j in range(i+1, n): y2 += self.A[i][j]*x[j]
				xi[i] = (-y1 - y2 + self.b[i])/self.A[i][i]
			# xi = self.__find_xk(x0)
			if self.norm_type == "l_infinity":
				norm = norms(xi, x).l_infinity()
			elif self.norm_type == "l_two":
				norm = norms(xi, x).l_two()
	...
	def jacobi(
		...
		while errors[-1] > self.tol and k <= self.max_iter:
			...
			for i in range(n):
				y = 0
				for j in range(n):
					if j != i: y += self.A[i][j]*x[j]
				xi[i] = (-y + self.b[i])/self.A[i][i]
			if self.norm_type == "l_infinity":
				norm = norms(xi, x).l_infinity()
			elif self.norm_type == "l_two":
				norm = norms(xi, x).l_two()
...
class cubic_spline:
	...
	def __init__(
		...
		BadDomainError : string
			If `domain` is neither n x 1 nor 1 x n array.

		BadFunctionError : string
			If `function` is not an expression or function and is not an n x 1 or 1 x n array.
		...
		"""
		self.__domain_name, self.__function_name = _retrieve_name(domain)[0], _retrieve_name(function)[0]
		self.domain = X = np.array(domain)
		if np.sum(X.shape) > X.shape[0]: raise ValueError(cubic_spline.__BadDomainError(self.__domain_name))
		# if isinstance(function, (str)):
		...
		else: raise TypeError(cubic_spline.__BadFunctionError(self.__function_name))
		if np.sum(self.domain.shape) != np.sum(self.mapped.shape) or self.domain.shape[0] != self.mapped.shape[0]:
			raise IndexError(cubic_spline.__BadDataError(self.__domain_name, self.__function_name))
		self.variable = variable
	
	__BadDomainError = lambda domain_name: f"ERROR! Input domain, '{domain_name}' was neither an n x 1 nor a 1 x n array."
	__BadFunctionError = lambda function_name: f"ERROR! Input range, '{function_name}' was neither function of type string nor an n x 1 or 1 x n array."
	...
	def clamped(
		...
		def algorithm(g, gp):
			...
			for i in range(1, n+1):
				...
				D[j] = (C[j+1] - C[j])/(3*H[j])
			return Y, A, B, C, D
		try: self.__function_derivative_name = _retrieve_name(function_derivative)[0]
		except IndexError: self.__function_derivative_name = "fp"
		BadDerivativeError = lambda fp_name: f"ERROR! Derivative range, '{fp_name}' was neither function nor expression and not an n x 1 or 1 x n array."
...
def hermite(
	...
	BadFunctionError : string
		If `function` is neither n x 1 nor 1 x n array.

	BadDataError : string
		If `domain` and `function` are of unequal length.

	BadFunctionDerivativeError : string
		If `function_derivative` is not an expression or function and is not an n x 1 or 1 x n array.
	...
	BadFunctionDerivativeError = lambda function_derivative_name: "ERROR! Input function derivative range, 'function_derivative_name}'was neither function nor expression and not an n x 1 or 1 x n array."
...
def lagrange(
	...
	domain_name, function_name = _retrieve_name(domain)[0], _retrieve_name(function)[0]
	domain, function = np.array(domain), np.array(function)
	BadDomainError = lambda domain_name: f"ERROR! Input domain, '{domain_name}' was neither an n x 1 nor a 1 x n array."
	BadFunctionError = lambda function_name: f"ERROR! Input range, '{function_name}' was neither an n x 1 nor a 1 x n array or LaTeX string expression."
	BadDataError = lambda domain_name, function_name: f"ERROR! Arrays '{domain_name}' and '{function_name}' must be of equal length."
...
class integrate:
	...
	def __init__(
		...
		Notes
		-----
		`domain = None` if not a list nor n x 1 or 1 x n array.
...
def richard_extrapolation(
	...
	"""
	function_name = _retrieve_name(function)[0]
	...
	#	else: raise ValueError(BadDataError(_retrieve_name(domain)[0], function_name))
	else:
...
class ivp(__ode):
	...
	def forward_euler(self) -> pd.DataFrame:
		...
		self.domain = np.arange(self.a, t+h, h)
	...
	def trapezoidal(
		...
		for i in range(1, self.steps):
			...
			#w = obj.newton_raphson(k1)["Approximations"][-1]
			Y.append(w)
			increments.append(abs(w - w0))
			t, w0 = self.a + i*h, w
		self.iterations = np.arange(self.steps)
