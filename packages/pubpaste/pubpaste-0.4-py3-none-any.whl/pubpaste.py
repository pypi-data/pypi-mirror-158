...
class GtkClipboard(AbstractClipboard):
    ...
    def put(self, data):
        ...
        if pid:  # parent
            logging.info(
                "forked child %d to hold onto %d bytes of clipboard data",
                pid,
                len(data),
            )
            ...
            logging.debug("setting clipboard text to %r", data)
            self._cb.set_text(data, len(data))
...
class XclipClipboard(AbstractClipboard):
    def __init__(self, selection):
        assert selection in ("CLIPBOARD", "PRIMARY"), "invalid clipboard selection used"
        self.commnad = ("xclip", "-selection", selection.lower())
...
def main():
    ...
    pasted = selection_clipboard.put(uri)
