...
class Changelog(object):
    ...
    def current_version(self) -> VersionBlock:
        ...
        if len(self.versions) > 1:
            return max(list(self.versions)[1:])
    ...
    def difference(self, other: "Changelog") -> VersionCollection:
        ...
        if unreleased not in res:
            ...
            for change_type in other.unreleased.change_blocks:
                if change_type not in unreleased.change_blocks:
                    unreleased.change_blocks[change_type] = other.unreleased.change_blocks[change_type]
                    continue
                for change in other.unreleased.change_blocks[change_type].changes:
                    if change not in unreleased.change_blocks[change_type].changes:
