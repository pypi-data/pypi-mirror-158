...
def create_stat_message(database):
...
# GUI - Функция реализующая заполнение таблицы историй логирования
def create_stat_login(database):
    # Список записей из базы
    ...
    list_table = QStandardItemModel()
    list_table.setHorizontalHeaderLabels(
        ['Имя Клиента', 'Дата входа', 'ip адрес', 'порт'])
    for row in hist_list:
        user, last_seen, addr, port = row
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.login_history_button = QAction('История подключений', self)

        # Кнопка вывести историю подключений
        self.message_history_button = QAction('Статистика сообщений', self)
        ...
        self.toolbar.addAction(self.login_history_button)
        self.toolbar.addAction(self.message_history_button)
...
# Класс окна с историей подключений
class LoginHistoryWindow(QDialog):
    ...
    def initUI(self):
        # Настройки окна:
        self.setWindowTitle('Статистика подключений')
