...
def assign_times(course_list, input_json, season):
    ...
    for course in course_list:
        ...
        while(score_index < len(course.score_list)):
            ...
            #course.score_list[score_index] = course.score_list[score_index]*2  
            score_index += 1
    ...
    start_time = time()
    print("Running branch and bound algorithm to assign time slots with ", COURSE_CAPACITY, "courses. Maximum # of tree levels is 54. Maximum number of nodes being expanded at each level is ", MAX_NUM_CHILD_NODES, ".")
    number_of_nodes = assign_time_slots(course_list[0:COURSE_CAPACITY]) # run course to time slots branch and bound
    end_time = time()
    print("Time slot assignment complete in ", (end_time - start_time), " seconds")
    ...
    return format_output(input_json, season)