import copy, sys, os, timeit
sys.path.append(os.path.dirname(os.path.dirname(os.path.realpath(__file__)))) # have to add parent to module path to import tools
...
class Professor():
    def __init__(self, id, name, preference, workload):
        self.id = id
        self.name = name
...
def assign_profs_to_courses_branch_and_bound(input_sorted_by_prof_interest, historical_year=None):   
    ...
    print('Found ' + str(len(courses)) + ' courses that can have professors assigned.')
    ...
    solution_limit = 1
    i = 1
    print('Looking for ' + str(solution_limit) + ' solutions')
    while current_node is not None:
        ...
        if len(current_node.data.courses) == 0:
            ...
            for professor in course.professors:                
                ...
                new_cost = current_node.data.cost + (195 - professor.preference) # TODO: Add in semester preference multiplier to cost
    ...
    print('Generated ' + str(len(prof_course_assignments)) + ' solutions in ' + str(stop - start) + 's')
