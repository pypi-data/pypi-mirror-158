...
import neuralspace.nlu.converters.rasa.shared.data
import neuralspace.nlu.converters.rasa.shared.io
from neuralspace.nlu.converters.rasa import util
from neuralspace.nlu.converters.rasa.data import (
    is_likely_json_file,
...
from neuralspace.nlu.converters.rasa.shared.nlu.constants import (
    ACTION_NAME,
    ENTITIES,
...
from neuralspace.nlu.converters.rasa.shared.utils import lazy_property
...
class TrainingData:
    ...
    def _load_lookup_table(lookup_table: Dict[Text, Any]) -> Dict[Text, Any]:
        ...
        if Path(potential_file).is_file():
            try:
                lookup_table[
                    "elements"
                ] = neuralspace.nlu.converters.rasa.shared.io.read_file(potential_file)
                return lookup_table
    ...
    def fingerprint(self) -> Text:
        ...
        return neuralspace.nlu.converters.rasa.shared.io.deep_container_fingerprint(
            relevant_attributes
        )
    ...
    def nlu_as_json(self, **kwargs: Any) -> Text:
        ...
        from neuralspace.nlu.converters.rasa.formats.rasa import RasaWriter
    ...
    def nlg_as_markdown(self) -> Text:
        ...
        from neuralspace.nlu.converters.rasa.formats.markdown_nlg import (
            NLGMarkdownWriter,
        )
    ...
    def nlg_as_yaml(self) -> Text:
        ...
        """
        from neuralspace.nlu.converters.rasa.formats.rasa_yml import RasaYAMLWriter
    ...
    def nlu_as_markdown(self) -> Text:
        ...
        from neuralspace.nlu.converters.rasa.formats.markdown import MarkdownWriter
    ...
    def persist_nlu(self, filename: Text = DEFAULT_TRAINING_DATA_OUTPUT_PATH) -> None:
        if is_likely_json_file(filename):
            neuralspace.nlu.converters.rasa.shared.io.write_text_file(
                self.nlu_as_json(indent=2), filename
            )
        elif is_likely_markdown_file(filename):
            neuralspace.nlu.converters.rasa.shared.io.write_text_file(
                self.nlu_as_markdown(), filename
            )
        elif is_likely_yaml_file(filename):
            neuralspace.nlu.converters.rasa.shared.io.write_text_file(
                self.nlu_as_yaml(), filename
            )
        else:
            raise ValueError(
    ...
    def persist_nlg(self, filename: Text) -> None:
        if is_likely_yaml_file(filename):
            neuralspace.nlu.converters.rasa.shared.io.write_text_file(
                self.nlg_as_yaml(), filename
            )
        elif is_likely_markdown_file(filename):
            nlg_serialized_data = self.nlg_as_markdown()
            if nlg_serialized_data:
                neuralspace.nlu.converters.rasa.shared.io.write_text_file(
                    nlg_serialized_data, filename
                )
        else:
            raise ValueError(
    ...
    def get_nlg_persist_filename(nlu_filename: Text) -> Text:
        ...
        if is_likely_json_file(nlu_filename):
    ...
    def sorted_intent_examples(self) -> List[Message]:
        ...
        neuralspace.nlu.converters.rasa.shared.io.raise_warning(
