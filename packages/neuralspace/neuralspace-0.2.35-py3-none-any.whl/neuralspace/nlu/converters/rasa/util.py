...
import neuralspace.nlu.converters.rasa.shared.common
import neuralspace.nlu.converters.rasa.shared.data
import neuralspace.nlu.converters.rasa.shared.io
from neuralspace.nlu.converters.rasa import loading
from neuralspace.nlu.converters.rasa.data import (
    markdown_file_extension,
    yaml_file_extension,
)
from neuralspace.nlu.converters.rasa.shared.constants import UTTER_PREFIX
...
def get_file_format_extension(resource_name: Text) -> Text:
    ...
    files = neuralspace.nlu.converters.rasa.shared.io.list_files(resource_name)
    ...
    known_file_formats = {
        loading.MARKDOWN: markdown_file_extension(),
        loading.RASA_YAML: yaml_file_extension(),
    }
    fformat = file_formats[0]
    if all(f == fformat for f in file_formats):
        return known_file_formats.get(fformat, yaml_file_extension())
