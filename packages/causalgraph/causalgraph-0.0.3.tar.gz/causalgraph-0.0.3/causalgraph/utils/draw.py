...
class Draw():
    ...
    def nx(self, path = None, graph_dict = None) -> None:
        """Plots the graph with NetworkX methods. Parse a path (e.g. "dir/plot.png") to save
        the plot as an image. By default, a plot of the whole graph will be generated, unless
        you parse another graph_dict explicitly.

        :param path: Directory where the plot should be saved, defaults to None
        :type path: str, optional
        :param graph_dict: A properties dict that describes a graph with its nodes, edges and
        creators, defaults to None
        ...
        G_nx = self.graph.readwrite.nx.write()
        edge_labels = {}
        ...
        for individual in graph_dict:
            ...
            if individual_type == "CausalEdge":
                ...
                timelag = graph_dict[individual].get("time_lag_s", None)
                confidence = graph_dict[individual].get("confidence", None)
    ...
    def html(self, directory, graph_dict = None) -> None:
        ...
        """
        file = open(f"{directory}/vis.html", "w")
        file.write(html)
        file.close()
