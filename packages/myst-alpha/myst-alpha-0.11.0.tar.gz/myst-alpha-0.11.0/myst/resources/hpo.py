...
from myst.adapters.utils import UNSET, _Unset, drop_unset_values
from myst.adapters.utils_test import get_resource_uuid
...
from myst.models.types import OptionalParam, UUIDOrStr, to_uuid
...
from myst.openapi.models.search_algorithm_create import SearchAlgorithmCreate
...
def _to_search_algorithm_create(hyper_opt: OptionalParam[Hyperopt]) -> OptionalParam[SearchAlgorithmCreate]:
    if hyper_opt is None:
        return None
    elif isinstance(hyper_opt, _Unset):
        return UNSET
    else:
        return SearchAlgorithmCreate(object="SearchAlgorithm", **hyper_opt.dict())


def _to_search_space_create(
    search_space: OptionalParam[Dict[str, Optional[Union[Uniform, QUniform, LogUniform, QLogUniform]]]]
) -> OptionalParam[Dict[str, Optional[Union[UniformModel, QUniformModel, LogUniformModel, QLogUniformModel]]]]:
    if search_space is None:
        return None
    elif isinstance(search_space, _Unset):
        return UNSET
...
class HPO(Resource):
    ...
    def update(
        self,
        title: OptionalParam[str] = UNSET,
        model: OptionalParam[Union[Model, UUIDOrStr]] = UNSET,
        test_start_time: OptionalParam[Time] = UNSET,
        test_end_time: OptionalParam[Time] = UNSET,
        fit_start_timing: OptionalParam[TimeRangeBoundary] = UNSET,
        fit_end_timing: OptionalParam[TimeRangeBoundary] = UNSET,
        fit_reference_timing: OptionalParam[AbsoluteOrCronTiming] = UNSET,
        predict_start_timing: OptionalParam[TimeRangeBoundary] = UNSET,
        predict_end_timing: OptionalParam[TimeRangeBoundary] = UNSET,
        predict_reference_timing: OptionalParam[CronTiming] = UNSET,
        search_algorithm: OptionalParam[Hyperopt] = UNSET,
        search_space: OptionalParam[Dict[str, Optional[Union[Uniform, QUniform, LogUniform, QLogUniform]]]] = UNSET,
        description: OptionalParam[str] = UNSET,
    ) -> "HPO":
        """Updates an HPO."""
        hpo_update: Dict[str, Any] = dict(
            ...
            title=title,
            description=description,
            model=model if model is None or isinstance(model, _Unset) else str(get_resource_uuid(model)),
            test_start_time=test_start_time
            if test_start_time is None or isinstance(test_start_time, _Unset)
            else test_start_time.to_iso_string(),
            test_end_time=test_end_time
            if test_end_time is None or isinstance(test_end_time, _Unset)
            else test_end_time.to_iso_string(),
            fit_start_timing=to_timing_create(from_time_range_boundary(fit_start_timing)),
            fit_end_timing=to_timing_create(from_time_range_boundary(fit_end_timing)),
            fit_reference_timing=to_timing_create(fit_reference_timing),
            predict_start_timing=to_timing_create(from_time_range_boundary(predict_start_timing)),
