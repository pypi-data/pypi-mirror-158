...
import pythonvideoannotator_module_patheditor.constants
config=load_config(pythonvideoannotator_module_patheditor.constants)
...
class Module(object):
	...
	def process_frame_event(self, frame):
		...
		if self._player.video_index is not None:
			...
			self._project.draw(frame, index)
